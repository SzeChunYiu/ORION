# QG23 — n=2 witness frontier: genuine geometry vs the t_c = 1 boundary (V1)

## Aim

QG22 established (exhaustive in instances at n=1, 41-objective margin grid) that
...
QG22's registered open question.

## Frozen machinery (identical to QG22, imported not copied)
...
1. **Objective grid** (13 objectives: 6 `H_main` + 6 `H_scale` + 1 anchor):
   - `H_main` (t_r = 2): `(dc,dnc) in {-3,-2,-1} x {0,1}` -> 6 objectives
     ...
     witness-free column QG22 could only sample); `dc = -1` shallow controls.
...
2. **Instance panels**:
   - `P1` EXHAUSTIVE letter-alphabet n=2: all 729 instances with each of the 6
     targets one of `{(1,0),(1,1),(0,1)}` (QG22's n=1 letters, embedded at
   ...
   - `P3` witness continuation: every serialized QG22 witness row (cap 20)
     re-evaluated on all 11 objectives.
   - `P4` discovery (seed 20260903, `numpy.default_rng`, QG22's generator
...
3. **Registered questions** (authored before the run):
   - **Q1 (primary):** does any n=2 witness exist at an objective with
     ...
     Prediction: YES (QG22's isolated 2-gap points at `dc = -2` had `t_c = 2`)
   ...
   - **Q4 (continuation stability):** each QG22 serialized witness, evaluated
...
4. **Gates (hard):**
   ...
   - `G4`: independent n=1 brute cross-check (as QG22) at the two most
     witness-bearing n=2-discovered points, on 12 fixed letter instances.
   - `G5`: anti-instrument import gate (as QG22).
   - `G6`: O1-anchor replay — frozen QG2 O1 witnesses reproduce exactly
     `C_DP = 11`, `C_Dxx = 13`.
   - `G7` (NEW, QG22 receipt binding): `QG22_CONE_EXACTNESS_RESULTS.json`
     sha256 recorded; its terminal must be
     `QG22_REGION_STRICTLY_CONTAINS_CONE_AT_N1`; every serialized QG22
     ...
     same `C_DP`/`C_Dxx`.
5. **Artifacts:** `QG23_N2_FRONTIER_RESULTS.json` (schema
   `ORION.QG.QG23.N2Frontier.v1`: grid, per-objective aggregates
   ...
   `g4_brute_report`, `qg22_receipt_sha256`, gate verdicts, witness rows
   capped at 20), `RUN_QG23.log`.

## Terminals

- `QG23_FRONTIER_IS_GEOMETRY` — Q1 YES with witnesses at `t_c >= 2` in at
  least two distinct margin cells OR any P1/P3 witness at `t_c >= 2`: the
  n=2 frontier is margin geometry, not a `t_c = 1` artifact.
- `QG23_TC1_ARTIFACT_DOMINANT` — Q1 NO: zero witnesses at `t_c >= 2` across
...
- `QG23_CONSISTENCY_FAILURE` — any gate violated (exit 3, no claims).
