...
STAGE1_PATH = ARTIFACT_DIR / "orion-qg-qg3-stage1.json"
DUAL_PATH = ARTIFACT_DIR / "orion-qg-qg3-dual-admission.json"
GENERIC_WS = REPO_ROOT / ".orion-qg-qg3-generic"
...
def _run_local(workspace: ResearchWorkspace, code: str, timeout: int = 120):
    ...
    if not result.success:
        raise RuntimeError(f"local harness capability failed: {result.error}")
    if not isinstance(result.output, dict) or result.output.get("returncode") != 0:
        raise RuntimeError("local harness process did not exit cleanly")
...
def main() -> int:
    for path in (GENERIC_WS, NATIVE_WS):
        if path.exists():
            shutil.rmtree(path)
    for path in (STAGE1_PATH, DUAL_PATH):
        path.unlink(missing_ok=True)
    ARTIFACT_DIR.mkdir(parents=True, exist_ok=True)

    # Lane A / generic harness custody: execute stage-1 selection and then an
    # independently implemented host-side admission check. Both become immutable receipts.
    ...
    if not STAGE1_PATH.is_file():
        raise FileNotFoundError("QG-3 stage-1 artifact missing after generic harness run")
    stage1 = json.loads(STAGE1_PATH.read_text(encoding="utf-8"))
    if stage1_summary.get("stage1_digest") != stage1.get("stage1_digest"):
        raise ValueError("stage-1 stdout digest does not match stage-1 artifact")

    admission_code = (
        "import runpy; runpy.run_path('development/orion-qg-regime-geometry/"
        "qg3_generic_admission.py', run_name='__main__')"
    )
    generic_request, generic_result = _run_local(generic_ws, admission_code)
    generic_decision = _token(str(generic_result.output.get("stdout", "")), GENERIC_PREFIX)
    if generic_decision.get("stage1_digest") != stage1.get("stage1_digest"):
        raise ValueError("generic admission is not bound to current stage-1 packet")

    # Lane B / native ORION-Q typed campaign: load the same stage-1 evidence and
    # let production campaign control select OPEN / NO_POSITIVE / INVALID.
    ...
    final_state = CampaignState.from_dict(
        native_ws.load_latest_campaign_state(
            QG3_POSITIVE_FORECAST_CAMPAIGN_MANIFEST["campaign_id"]
        )
    )
    phase_to_decision = {
        "OPEN_RECORDED": "OPEN",
        "NO_POSITIVE_RECORDED": "NO_POSITIVE",
        "INVALID_RECORDED": "INVALID",
    }
    native_decision = phase_to_decision.get(final_state.phase_id, "INCOMPLETE")
    native_stage1_digest = final_state.observation_map.get("QG3_STAGE1_DIGEST", "")
    if native_stage1_digest != stage1.get("stage1_digest"):
        raise ValueError("native campaign is not bound to current stage-1 packet")

    both_open = generic_decision.get("decision") == "OPEN" and native_decision == "OPEN"
    dual = {
        "schema": "ORION.QG.QG3.DualAdmission.v1",
        "issue": "SzeChunYiu/ORION#745",
        "stage1_digest": stage1.get("stage1_digest"),
        "positive_found": stage1.get("positive_found"),
        "generic_lane": {
            "decision": generic_decision.get("decision"),
            "decision_payload": generic_decision,
            "stage1_request": stage1_request.as_dict(),
            "stage1_result": stage1_result.as_dict(),
            "admission_request": generic_request.as_dict(),
            "admission_result": generic_result.as_dict(),
        },
        "native_lane": {
    ...
    print(json.dumps({
        ...
        "dual_admission": str(DUAL_PATH),
        "positive_found": stage1.get("positive_found"),
        "generic_decision": generic_decision.get("decision"),
