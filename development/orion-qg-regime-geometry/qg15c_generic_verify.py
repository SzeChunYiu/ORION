...
def main():
 ...
 checks={'schema':a.get('schema')=='ORION.QG.QG15C.EnlargedVocabulary.v1','digest':obs==hashlib.sha256(canonical(u).encode()).hexdigest(),'protocol':a.get('protocol_sha256')==hashlib.sha256(PROTOCOL.read_bytes()).hexdigest(),'feature_count':all(len(v)==a.get('feature_count') for v in cells),'training_1146':sum(x['instances'] for x in per.values())==1146,'unique_cells':len(cells)==a.get('unique_feature_cells'),'mixed_count':len(mixed)==a.get('mixed_cell_count'),'floor':floor==a.get('irreducible_error_floor'),'per_n':all(per[k]==a['domain'][k] for k in per),'no_overclaim':a.get('novelty_authority') is False and a.get('physical_quantum_advantage_claim') is False}
