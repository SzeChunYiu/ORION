...
def decide(stage1: dict[str, Any], novelty_text: str) -> dict[str, Any]:
    ...
    checks = {
        ...
        "no_dp_calls": stage1.get("no_dp_calls") is True and int(stage1.get("dp_call_count", -1)) == 0,
    ...
    if checks["prediction_payload_present"]:
        checks["predicted_cost_strictly_below_donor"] = int(prediction["predicted_C_DP"]) < int(prediction["C_R6L"])
        checks["positive_regime"] = prediction.get("predicted_regime") in {"split", "borrow"}
    else:
        checks["predicted_cost_strictly_below_donor"] = False
        checks["positive_regime"] = False

    decision = "OPEN" if all(checks.values()) else (
        "NO_POSITIVE" if stage1.get("positive_found") is False and stage1.get("admission_gates_pass") is True else "INVALID"
    )
    return {
        "schema": "ORION.QG.QG3.GenericAdmission.v1",
        "decision": decision,
        "stage1_digest": str(stage1.get("stage1_digest", "")),
        "checks": checks,
        "novelty_threat_sha256": _sha256(NOVELTY_PATH),
