# QG22 — Cone exactness: exhaustive n=1 support-2 region vs the QG8 cone (V1)
...
5. **Artifacts:** `QG22_CONE_EXACTNESS_RESULTS.json` (schema
   `ORION.QG.QG22.ConeExactness.v1`: grid, per-objective aggregates
...
- `QG22_REGION_STRICTLY_CONTAINS_CONE_AT_N1` — Q1 YES (exit 0): the cone is
  NOT the exact support-2 region at n=1 on the grid; R_1 ∩ grid ⊋ C ∩ grid.
- `QG22_CONE_EXACT_ON_GRID_AT_N1` — Q1 NO (exit 0): every outside grid point
  has an n=1 witness; finite-n evidence FOR cone = region.
- `QG22_CONSISTENCY_FAILURE` — G1/G2/G3/G4/G5 violated (exit 3, no claims).
