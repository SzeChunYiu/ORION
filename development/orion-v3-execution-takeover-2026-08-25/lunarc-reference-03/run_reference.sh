...
expected_source_sha=$(env -u PYTHONPATH python3 -c "import json; print(json.load(open('EXECUTION_PROTOCOL.json'))['inputs']['source_archive_sha256'])")
actual_source_sha=$(sha256sum source.tar.gz | awk '{print $1}')
test "$actual_source_sha" = "$expected_source_sha"
echo "ORION_SOURCE_ARCHIVE_HASH_GREEN sha256=$actual_source_sha"
