#!/usr/bin/env python3
"""Execute the once-frozen P3 V20 BERTMap and common-scoring contract."""
...
PREFLIGHT = ROOT / "RUNTIME_PREFLIGHT_V20.json"
LOCK = ROOT / "ATTEMPT_LOCK_V20.json"
STDOUT = ROOT / "BERTMAP_STDOUT_V20.log"
STDERR = ROOT / "BERTMAP_STDERR_V20.log"
PARSER_STDOUT = ROOT / "PARSER_STDOUT_V20.log"
PARSER_STDERR = ROOT / "PARSER_STDERR_V20.log"
PARSER_RECEIPT = ROOT / "PARSER_RECEIPT_V20.json"
RESULT = ROOT / "BERTMAP_RESULT_V20.json"
TERMINAL = ROOT / "TERMINAL_V20.txt"
ORIGINAL = ROOT / "runtime-interface/ORIGINAL_WRAPPED_REPAIRED_MAPPINGS_V20.tsv"
RUNTIME_ROOT = Path("/Volumes/P3V17_RUNTIME")
PYTHON = RUNTIME_ROOT / "venv/bin/python"
SOURCE_SCRIPT = ROOT / "bertmap_struct_entrypoint_v20.py"
OUTPUT_ROOT = RUNTIME_ROOT / "output-v20/bertmap-out"
...
def decode_repaired(protocol: dict[str, object]) -> dict[str, object]:
    universe = json.loads((ROOT / "UNIVERSE_MANIFEST_V20.json").read_text())
    sets = {
    ...
    parser_command = [
        ...
        str(decoded_dir),
        "--manifest",
        str(ROOT / "UNIVERSE_MANIFEST_V20.json"),
        "--write-receipt",
        str(PARSER_RECEIPT),
    ]
...
def main() -> int:
    ...
    if any(path.exists() for path in stale) or any((ROOT / "decoded-interface").iterdir()):
        raise SystemExit("REFUSE_RERUN_OR_STALE_V20_ARTIFACT")
    ...
    check("runner_identity", sha256(Path(__file__)) == protocol["frozen_code"]["run_bertmap_v20.py"])
    for name, expected in protocol["frozen_code"].items():
        if name == "run_bertmap_v20.py":
            continue
    ...
    preflight = {
        "schema_version": "orion.p3.oaei-bertmap.runtime-preflight.v20",
        ...
        "terminal": "P3_V20_RUNTIME_AND_OUTCOME_BLIND_IDENTITY_PREFLIGHT_PASS" if authorized else "P3_V20_RUNTIME_OR_IDENTITY_PREFLIGHT_FAIL",
    }
    ...
    command = [
        ...
        str(SOURCE_SCRIPT),
        "-s",
        str(ROOT / "inputs/SOURCE_BASE_BOUND_V20.rdf"),
        "-t",
        str(ROOT / "inputs/TARGET_BASE_BOUND_V20.rdf"),
        "-c",
        str(ROOT / "BERTMAP_CONFIG_V20.yaml"),
        "-u",
        str(ROOT / "UNIVERSE_MANIFEST_V20.json"),
    ]
    started = now()
    LOCK.write_text(
        json.dumps(
            {
                "schema_version": "orion.p3.oaei-bertmap.attempt-lock.v20",
                "protocol_sha256": sha256(PROTOCOL),
    ...
    class_surface_bindings = [
        line for line in stdout_text.splitlines() if line.startswith("ORION_P3_V20_CLASS_SURFACE_BOUND=")
    ]
    ...
    class_surface_ok = class_surface_bindings == [
        "ORION_P3_V20_CLASS_SURFACE_BOUND=source:37_TO_36:OWL_THING_ONLY",
        "ORION_P3_V20_CLASS_SURFACE_BOUND=target:37_TO_36:OWL_THING_ONLY",
    ]
    native_success = exit_code == 0 and not timed_out and launch_error is None and five and direct_ok and class_surface_ok
    ...
    if native_success and typed_pass:
        terminal = "P3_V20_BERTMAP_NATIVE_AND_TYPED_DECODER_PASS__SAME_UNIVERSE_OUTPUT_FROZEN__COMMON_REFERENCE_SCORING_AUTHORIZED"
    elif native_success:
        terminal = "P3_V20_BERTMAP_NATIVE_PASS__TYPED_DECODER_OR_STRUCTURAL_CONTRACT_FAIL__COMMON_SCORING_NOT_AUTHORIZED"
    else:
        terminal = "P3_V20_BERTMAP_NATIVE_ATTEMPT_FAIL__NO_RETRY__COMMON_SCORING_NOT_AUTHORIZED"
    result = {
        "schema_version": "orion.p3.oaei-bertmap.execution-result.v20",
        ...
        "class_surface_bindings": class_surface_bindings,
        "class_surface_binding_pass": class_surface_ok,
    ...
    evaluator = subprocess.run(
        [str(PYTHON), str(ROOT / "common_pair_evaluator_v20.py")],
        cwd=ROOT,
        capture_output=True,
        text=True,
        timeout=120,
    ...
    (ROOT / "COMMON_SCORING_STDOUT_V20.log").write_text(evaluator.stdout)
    (ROOT / "COMMON_SCORING_STDERR_V20.log").write_text(evaluator.stderr)
