#!/usr/bin/env python3
"""Execute the frozen eight-world SYNERGY relative-effect confirmation."""
...
def execute(protocol_path: Path, data_root: Path, out: Path) -> None:
    ...
    if len(worlds) == 8:
        ...
        for world in worlds:
            active = world["arms"]["ACTIVE_LOGREG"]
            ...
            paired.append({
                ...
                "recall_at_010_difference": active["recall_at_010"] - world["arms"][recall_comparator]["recall_at_010"],
                "wss_comparator": wss_comparator,
                "wss_at_95_difference": active["wss_at_95"] - world["arms"][wss_comparator]["wss_at_95"],
                "active_minus_random_recall_at_010": active["recall_at_010"] - world["arms"]["RANDOM"]["recall_at_010"],
            })
    ...
    gates = {
        ...
        "G2_INCREMENTAL": len(worlds) == 8 and uncertainty["paired_recall_gain_macro_mean"] >= 0.10,
        "G3_REPLICATION": len(worlds) == 8 and wins >= 7 and uncertainty["exact_one_sided_sign_flip_p"] <= 0.05,
        "G4_WORK_SAVING": len(worlds) == 8 and all(w["arms"]["ACTIVE_LOGREG"]["wss_at_95"] > 0 for w in worlds) and wss_gain >= 0.10,
        "G5_NO_MATERIAL_HARM": len(worlds) == 8 and all(item["active_minus_random_recall_at_010"] >= -0.05 for item in paired),
    }
    ...
    result = {"schema_version": protocol["schema_version"], "claim_scope": protocol["claim_scope"],
              "terminal": terminal, "gates": gates, "active_strict_wins_worlds": wins,
