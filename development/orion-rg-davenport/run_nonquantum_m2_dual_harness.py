#!/usr/bin/env python3
"""Run source, generic, and native lanes for non-quantum math M2."""
...
SOURCE = ROOT / "research/orion-rg/NONQUANTUM_M2_SATURATION_DEFECT_REPLAY_RESULTS_2026-08-24.json"
GENERIC = ROOT / "development/orion-rg-davenport/NONQUANTUM_M2_SATURATION_DEFECT_REPLAY_GENERIC_2026-08-24.json"
DUAL = ROOT / "development/orion-rg-davenport/NONQUANTUM_M2_SATURATION_DEFECT_REPLAY_DUAL_2026-08-24.json"
POSITIVE = (
...
def _run(generic_path: Path, native_path: Path) -> int:
    ...
    source_execution = run_python(
        generic_workspace, "research/orion-rg/nonquantum_m2_saturation_defect_replay.py"
    )
    generic_execution = run_python(
        generic_workspace, "development/orion-rg-davenport/nonquantum_m2_generic_verify.py"
    )
    ...
    campaign_id = NONQUANTUM_M2_SATURATION_DEFECT_CAMPAIGN_MANIFEST["campaign_id"]
    ...
    native_decision = {
        "ACCEPT_RECORDED": "ACCEPT_SATURATION_DEFECT_SUPPORT_LE9",
        "REJECT_RECORDED": "REJECT",
    }.get(state.phase_id, "INCOMPLETE")
    both_accept = (
        source.get("terminal") == POSITIVE
        and generic.get("decision") == "ACCEPT_SATURATION_DEFECT_SUPPORT_LE9"
        and native_decision == "ACCEPT_SATURATION_DEFECT_SUPPORT_LE9"
    )
    terminal = POSITIVE if both_accept else "NONQUANTUM_M2_GENERIC_NATIVE_DISAGREEMENT"
    cycles = outcome.get("cycles", []) if isinstance(outcome, dict) else []
    dual: dict[str, Any] = {
        "schema": "ORION.NonQuantumMath.M2.DualHarness.v1",
        "terminal": terminal,
        ...
        "support8_normalized_supports_per_c_engine": 564,
        "support9_nodes_per_c_engine": 6_537_270,
        "support8_solutions_per_c_engine": 0,
        "support9_solutions_per_c_engine": 0,
        "result_owner": "NON_QUANTUM_MATH",
        "bounded_support_le9_theorem_authority": True,
...
def main() -> int:
    paths: list[Path] = []
    try:
        generic_path = Path(tempfile.mkdtemp(prefix="orion-nonquantum-m2-generic-", dir="/tmp"))
        paths.append(generic_path)
        native_path = Path(tempfile.mkdtemp(prefix="orion-nonquantum-m2-native-", dir="/tmp"))
