#!/usr/bin/env python3
"""Execute the frozen safe query-conditioned acquisition successor."""
...
def execute(protocol_path: Path, data_root: Path, out: Path) -> None:
    ...
    for world in protocol["worlds"]:
        ...
        worlds.append({
            ...
            "arms": {"RANDOM": random_mean, "STATIC_SEED_CENTROID": order_metrics(static_order, y),
                     ...
                     "SAFE_RANK_FUSION": order_metrics(safe, y)},
        })
    paired: list[dict[str, Any]] = []
    if len(worlds) == 8:
        arms = ("RANDOM", "STATIC_SEED_CENTROID", "STATIC_REVIEW_QUERY",
                "ACTIVE_LOGREG", "SAFE_RANK_FUSION")
        ...
        for world in worlds:
            candidate = world["arms"]["SAFE_RANK_FUSION"]
            comparators = ("RANDOM", "STATIC_SEED_CENTROID", "STATIC_REVIEW_QUERY", "ACTIVE_LOGREG")
            recall_comparator = max(
                comparators,
                key=lambda arm: world["arms"][arm]["recall_at_010"],
            )
            wss_comparator = max(
                comparators,
                key=lambda arm: world["arms"][arm]["wss_at_95"],
            )
            paired.append({
                "world_id": world["world_id"],
                "recall_comparator": recall_comparator,
                "recall_at_010_difference": candidate["recall_at_010"] - world["arms"][recall_comparator]["recall_at_010"],
                "wss_comparator": wss_comparator,
                "wss_at_95_difference": candidate["wss_at_95"] - world["arms"][wss_comparator]["wss_at_95"],
                "candidate_minus_random_recall_at_010": candidate["recall_at_010"] - world["arms"]["RANDOM"]["recall_at_010"],
            })
        differences = np.array([item["recall_at_010_difference"] for item in paired])
        ...
        wins = sum(item["recall_at_010_difference"] > 0 for item in paired)
        wss_gain = float(np.mean([item["wss_at_95_difference"] for item in paired]))
    else:
        aggregate, uncertainty, wins, wss_gain = {}, {}, 0, float("nan")
    gates = {
        "G1_BINDING": binding_ok and len(worlds) == 8 and all(w["missing_label_work_joins"] == 0 for w in worlds),
        "G2_INCREMENTAL": len(worlds) == 8 and uncertainty["paired_recall_gain_macro_mean"] >= 0.05,
        "G3_REPLICATION": len(worlds) == 8 and wins >= 6 and uncertainty["exact_one_sided_sign_flip_p"] <= 0.05,
        "G4_WORK_SAVING": len(worlds) == 8 and all(w["arms"]["SAFE_RANK_FUSION"]["wss_at_95"] > 0 for w in worlds) and wss_gain >= 0.05,
        "G5_NO_MATERIAL_HARM": len(worlds) == 8 and all(item["candidate_minus_random_recall_at_010"] >= -0.05 for item in paired),
    }
    ...
    result = {"schema_version": protocol["schema_version"], "claim_scope": protocol["claim_scope"],
              "terminal": terminal, "gates": gates, "candidate_strict_wins_worlds": wins,
              "uncertainty": uncertainty, "paired_world_differences": paired,
              "macro_wss_gain_over_stronger_per_world_comparator": wss_gain,
