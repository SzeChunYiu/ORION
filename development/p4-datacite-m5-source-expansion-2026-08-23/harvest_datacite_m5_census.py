#!/usr/bin/env python3
"""Execute the frozen outcome-blind P4 DataCite M5 metadata census."""
...
def main() -> int:
    ...
    for query_number, query in enumerate(protocol["queries"], start=1):
        ...
        for item in data:
            ...
            if (
                not client_id
                or client_id in excluded_clients
                or publisher.casefold() in excluded_publishers
                or doi.startswith(excluded_prefixes)
                or doi in prior_zenodo_dois
            ):
                counts["provider_or_root_disjointness_exclusions"] += 1
                continue
            ...
            candidates.append(
                {
                    "schema_version": "orion.p4.datacite-m5-candidate.v1",
    ...
    if not transport_schema_ok or not pagination_ok:
        ...
        source_signal_terminal = "P4_DATACITE_M5_DISJOINT_PROVIDER_COMBINED_48_SIGNAL_GATE_PASSED__NO_CASES_ADJUDICATED"

    result: dict[str, Any] = {
        "schema_version": "orion.p4.datacite-m5-disjoint-provider-census-result.v1",
        "date": "2026-08-23",
        "captured_at_utc": dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat(),
