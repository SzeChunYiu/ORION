...
MARKER_PREFIX = "ORION_P10_STATE::"
HELPER = """
namespace ORION_P10_Trace
open Lean Elab Tactic
elab "orion_p10_marker " s:str : tactic => do
  logInfo s.getString
end ORION_P10_Trace
""".strip("\n")
...
def state_features(state: str) -> tuple[dict[str, Any], dict[str, Any], str]:
    ...
    for names, rhs in declarations:
        ...
        for source in names:
            for target in targets:
                ...
                referenced_decls.add(source)
                indegree[target] += 1
    goal_refs = set(IDENT.findall(goal_text)) & context_set

    token_count = len(TOKEN.findall(state))
    depth = visible_depth(state)
    flags = {
    ...
    state_vector = {
        ...
        "context_shape_histogram": {key: context_hist.get(key, 0) for key in (
        ...
        **flags,
        "state_token_bucket": bucket(token_count, (32, 64, 128, 256, 512)),
        "visible_depth_bucket": bucket(depth, (3, 6, 10)),
    }
    dependency_vector = {
        ...
        "referencing_declaration_fraction": (
            len(referenced_decls) / len(context_names) if context_names else 0.0
        ),
    }
...
def parse_messages(output: str) -> list[tuple[str, str]]:
    ...
    for index, match in enumerate(matches):
        ...
        if rest.startswith("\n"):
            rest = rest[1:]
        content = first + ("\n" + rest.rstrip("\n") if rest else "")
        messages.append((match.group(1), content.strip()))
    return messages
...
def instrument(text: str, events: list[dict[str, Any]]) -> str:
    lines = text.splitlines()
    by_line: dict[int, list[dict[str, Any]]] = {}
    for event in events:
        by_line.setdefault(int(event["line_zero_index"]), []).append(event)
    for line_index in sorted(by_line, reverse=True):
        original = lines[line_index]
        indent = original[: len(original) - len(original.lstrip())]
        inserts: list[str] = []
        for event in by_line[line_index]:
            marker = MARKER_PREFIX + event["transition_id"]
            inserts.extend([
                f'{indent}orion_p10_marker {json.dumps(marker)}',
                f"{indent}trace_state",
            ])
        lines[line_index:line_index] = inserts
    import_indices = [idx for idx, line in enumerate(lines) if line.lstrip().startswith("import ")]
    helper_at = (max(import_indices) + 1) if import_indices else 0
    helper_lines = ["", *HELPER.splitlines(), ""]
    lines[helper_at:helper_at] = helper_lines
    return "\n".join(lines) + ("\n" if text.endswith("\n") else "")


def pair_states(messages: list[tuple[str, str]]) -> dict[str, str]:
    states: dict[str, str] = {}
    for idx, (kind, content) in enumerate(messages):
        if kind != "info" or not content.startswith(MARKER_PREFIX):
            continue
        transition_id = content[len(MARKER_PREFIX):].splitlines()[0].strip()
        for next_kind, next_content in messages[idx + 1:]:
            if next_kind == "info" and next_content.startswith(MARKER_PREFIX):
                break
            if next_kind == "info" and ("⊢" in next_content or "no goals" in next_content.lower()):
                states[transition_id] = next_content
                break
    return states
...
def main() -> None:
    ...
    for item in selected:
        ...
        instrumented = instrument(original, events)
        ...
        try:
            ...
            original_restored = sha_file(path) == item["sha256"]
        ...
        states = pair_states(parse_messages(combined))
