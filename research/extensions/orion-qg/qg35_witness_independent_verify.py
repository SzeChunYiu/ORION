"""Independent re-derivation of the headline witness, recomputing bulk, spectrum
and the full 384-response from the primitives rather than reading the cache."""
...
for A,B in [("IXIYXZ","IXIYYZ"),("IXXYYZ","IXXYZY"),("IXIXIY","IXIXYI")]:
    ta,tb=parse(A),parse(B)
    ra,rb=response(ta),response(tb)
    ba=[baseline(ta,p) for p in PS[:4]]; bb=[baseline(tb,p) for p in PS[:4]]
    sa,sb=sorted(ra),sorted(rb)
    ma,mb=min(ra),min(rb)
    ga={p for p,v in enumerate(ra) if v==ma}; gb={p for p,v in enumerate(rb) if v==mb}
    print(f"{A} vs {B}")
    print(f"   bulk equal      : {ba==bb}")
    print(f"   spectrum equal  : {sa==sb}")
    print(f"   optimal value   : {ma} vs {mb}  equal: {ma==mb}")
    print(f"   optimal frames  : {sorted(ga)[:6]} vs {sorted(gb)[:6]}")
    print(f"   frame sets equal: {ga==gb}   shared frames: {len(ga&gb)}")
    print()
