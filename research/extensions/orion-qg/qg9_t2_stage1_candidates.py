...
def main():
    ...
    states,actions_by_type,by_desc,concrete=v3.build_types()
    reverse={tuple(row):key for key,rows in states.items() for row in rows}
    local=sorted(reverse)
    ...
    for q0 in local:
        for q1 in local:
            ...
            if v3.safe_profile_move((k0,k1),actions_by_type) is not None:continue
            raw_accepted_unsafe+=1
            cols=min((q0,q1),(q1,q0))
            ck=canonical(cols)
