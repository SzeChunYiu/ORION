...
Protocol: development/orion-qg-regime-geometry/QG43_CONE_EXACTNESS_PROTOCOL_V1.md
...
r6m = qg2.r6m
PROTOCOL = REPO_ROOT / "development/orion-qg-regime-geometry/QG43_CONE_EXACTNESS_PROTOCOL_V1.md"
QG2_PATH = QG_DIR / "QG2_OBJECTIVE_ROBUSTNESS_RESULTS.json"
DEFAULT_OUTPUT = QG_DIR / "QG43_CONE_EXACTNESS_RESULTS.json"
SEED = 20260824
RANDOM_N2 = 60
RANDOM_N3 = 40
...
def ob_name(t_r: int, dc: int, dnc: int, t_tag: int) -> str:
    return f"Q43G_tr{t_r}_dc{dc}_dnc{dnc}_tag{t_tag}"
...
def build_grid() -> list[qg2.Objective]:
    ...
    grid.append(qg2.Objective("Q43G_O1ANCHOR", 7, 1, 4, 3, 0))  # exact O1
...
def n1_brute_check(tp: tuple, ob: qg2.Objective) -> None:
    ...
    if brute_global != dp_global:
        raise AssertionError({"qg43_n1_brute_mismatch": [ob.name, dp_global, brute_global]})
...
def run() -> dict[str, Any]:
    ...
    for ob in grid:
        ...
        is_main = ob.name.startswith("Q43G_tr2_") and ob.name.endswith("_tag2")
    ...
    ray_pool = [ob for ob in witness_bearing
                if ob.name.startswith("Q43G_tr2_") and ob.name.endswith("_tag2")]
    ...
    o1_anchor = qg2.Objective("Q43G_O1ANCHOR", 7, 1, 4, 3, 0)
    qg2.clear_caches()
    o1_replay_ok = True
    o1_replay_rows = []
    for name, n, tp in p3:
...
def main() -> int:
    ...
    return 0 if result["terminal"] != "QG43_CONSISTENCY_FAILURE" else 3
