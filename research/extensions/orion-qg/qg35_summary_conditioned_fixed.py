...
def exact_minimum(entries,npairs,fixed_ok):
 ...
 covers=[x[0] for x in entries];reps=[x[1] for x in entries];U=(1<<npairs)-1
 pair_cands=[]
 ...
 for target in range(1,5):
  ...
  def search(rem,slots):
   ...
   for i in choices:
    nr=rem&~covers[i]
    if nr==rem:continue
    tail=search(nr,slots-1)
    if tail is not None:return (i,)+tail
   return None
  sol=search(U,target);attempts[str(target)]={"stats":dict(stats),"cache":search.cache_info()._asdict(),"found":sol is not None}
  if sol is not None:
   ps=tuple(sorted({reps[i] for i in sol}));return len(ps),ps,{"depth_attempts":attempts,"selected_depth":target,"search_state":"REMAINING_PAIRS_AND_SLOTS_ONLY"}
 if not fixed_ok:raise AssertionError('earned universal five-probe upper bound failed on class')
 return 5,FIXED,{"depth_attempts":attempts,"selected_depth":5,"fallback":"QG32_CERTIFIED_UNIVERSAL_FIVE_PROBE_BASIS","search_state":"REMAINING_PAIRS_AND_SLOTS_ONLY"}
...
def main():
 ...
 for ci,g in enumerate(z['joint']):
  members=tuple(g)
  if len(members)<=1:
   m,ps,proof=0,(),{"depth_attempts":{},"selected_depth":0};pairs=[];entries=[];dom=0
  else:
   pairs,_physical,entries,dom=class_cover_problem(z['mat'],members);m,ps,proof=exact_minimum(entries,len(pairs),separates(z['mat'],members,FIXED))
