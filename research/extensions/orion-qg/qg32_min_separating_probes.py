#!/usr/bin/env python3
"""QG-32 production analyzer: exact minimum fixed probe cardinality above joint bulk+spectrum summaries."""
...
def coverage_classes(pairs,mat):
 ...
 for ridx,(a,b) in enumerate(pairs):
  ...
  if len(diff)==0:raise AssertionError(("indexed collision",a,b))
  bit=1<<ridx
  for p in diff:bits[int(p)]|=bit
 groups=defaultdict(list)
 for p,b in enumerate(bits):
  if b:groups[b].append(p)
 covers=sorted(groups,key=lambda b:(min(groups[b]),b))
 representatives=[min(groups[b]) for b in covers]
 return bits,covers,representatives,groups

def incidence_from_cover_bits(covers,npairs):
 rows=[];cols=[]
 for j,b in enumerate(covers):
  x=b
  while x:
   lsb=x&-x;ridx=lsb.bit_length()-1;rows.append(ridx);cols.append(j);x-=lsb
 return csr_array(coo_array((np.ones(len(rows),dtype=float),(np.array(rows,dtype=int),np.array(cols,dtype=int))),shape=(npairs,len(covers))))
def solve_minimum(A,time_limit=75.0):
 n=A.shape[1]
...
def main():
 ...
 pairs=pairs_from_groups(joint_groups);physical_bits,covers,reps_for_cover,cover_groups=coverage_classes(pairs,resp);A=incidence_from_cover_bits(covers,len(pairs));res=solve_minimum(A,75.0)
 milp_ok=res.status==0 and res.x is not None
 if milp_ok:
  m=int(round(float(res.fun)));chosen_classes=[j for j,x in enumerate(res.x) if x>0.5];selected=tuple(sorted(reps_for_cover[j] for j in chosen_classes))
 else:m=None;selected=()
 ...
 out={"schema":"ORIONQG.QG32.MinSeparatingProbes.v1","issue":"SzeChunYiu/ORION#911","terminal":term,"protocol_sha256":shaf(PROTO),"parent_hashes":{"qg31":shaf(QG31)},"parent_checks":parent,"universe":{"orbits":len(reps),"physical_probes":resp.shape[1],"distinct_pair_coverage_classes":len(covers)},"joint_partition":{"class_count":len(joint_groups),"class_size_histogram":hist(joint_groups),"largest_class":largest(joint_groups),"unresolved_pair_count":len(pairs)},"minimum_probe_cardinality":m,"selected_probe_indices":list(selected),"selected_probe_details":selected_details,"selected_separates_715":separates,"selected_canonicalization":"MINIMUM_CARDINALITY_MILP_OPTIMUM__SORTED_MIN_PHYSICAL_REP_PER_SELECTED_COVERAGE_CLASS__NO_LEX_AUTHORITY","milp":{"status":int(res.status),"message":str(res.message),"objective":float(res.fun) if res.fun is not None else None,"optimal":milp_ok},"lower_bound_packing":{"attempted":False,"size":0,"closes_minimum":False,"pairs":[]},"ablations":abl,"JOINT_PARTITION_RECONSTRUCTED":all(parent.values()),"JOINT_SEPARATING_PROBE_UPPER_BOUND_CERTIFIED":bool(separates),"PRODUCTION_MILP_MINIMUM_OPTIMAL":bool(milp_ok),"PRODUCTION_PACKING_LOWER_BOUND_CLOSES":False,"MINIMUM_FULL_FINITE_OPTIMUM_PROBES":False,"HARDWARE_MEASUREMENT_MINIMUM":False,"QG28_GLOBAL_STATE_MINIMALITY":False,"ADAPTIVE_TREE_OPTIMALITY":False,"novelty_authority":False,"physical_quantum_advantage_claim":False};raw=canon(out);out["result_digest"]=hashlib.sha256(raw.encode()).hexdigest();ns.output.parent.mkdir(parents=True,exist_ok=True);ns.output.write_text(json.dumps(out,indent=2,sort_keys=True)+"\n");print(TOKEN+canon({"terminal":term,"joint_classes":len(joint_groups),"unresolved_pairs":len(pairs),"coverage_classes":len(covers),"m_joint":m,"selected":list(selected),"ablations":{k:{"greedy":v["greedy_probe_upper_bound"],"lower":v["safe_cardinality_lower_bound"]} for k,v in abl.items()},"result_digest":out["result_digest"]}));return 0
