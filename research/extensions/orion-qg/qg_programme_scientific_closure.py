...
PROTOCOL=DEV/'QG_PROGRAMME_SCIENTIFIC_CLOSURE_PROTOCOL_V1.md'; INDEX=DEV/'QG_PROGRAMME_CLOSURE_EVIDENCE_V1.json'; W1=DEV/'QG_WAVE1_CLOSURE_PACKET.md'; W2=DEV/'QG_WAVE2_RECORD.md'
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--output',type=Path,default=OUT); args=ap.parse_args(); idx=json.loads(INDEX.read_text()); e=idx['evidence']; p={k:json.loads(v.read_text()) for k,v in PARENTS.items()}
 expected={'portfolio','qg18','qg17r','qg15c','qg10c','qg11c','qg14c','qg7d'}
 terminals={k:v['terminal'] for k,v in e.items()}
 gates={
  'protocol_and_index':PROTOCOL.exists() and INDEX.exists(),
