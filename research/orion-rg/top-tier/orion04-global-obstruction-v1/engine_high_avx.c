...
static int sel[40],mul[40],ns;static long long nodes,leaves,sol;static int A1,B2,C4,seed1,seed2,plane2;
...
static void dfs(int stage,int got,int start,const Reach*R,int sum){nodes++;if(stage==2){int q=neg(sum);leaves++;if(!q||used(q)||lconf(q))return;if(A1-seed1-1>0&&q<=sel[ns-1])return;Reach z;if(!addm(R,q,1,&z))return;sol++;return;}int need=stage==0?(B2-seed2):(A1-seed1-1),m=stage==0?2:1;if(got==need){dfs(stage+1,0,1,R,sum);return;}for(int v=start;v<N;v++){if(stage==0&&plane2&&v%5!=0)continue;if(!v||used(v)||lconf(v))continue;Reach z;if(!addm(R,v,m,&z))continue;sel[ns]=v;mul[ns]=m;ns++;dfs(stage,got+1,v+1,&z,addsum(sum,v,m));ns--;if(sol)return;}}
...
int main(int ac,char**av){if(ac!=8)return 2;A1=atoi(av[1]);B2=atoi(av[2]);C4=atoi(av[3]);int sm[3]={atoi(av[4]),atoi(av[5]),atoi(av[6])};plane2=atoi(av[7]);if(A1+2*B2+4*C4!=31||A1<1||plane2<0||plane2>1)return 2;seed1=seed2=0;int seed4=0;for(int i=0;i<3;i++){if(sm[i]==1)seed1++;else if(sm[i]==2)seed2++;else if(sm[i]==4)seed4++;else if(sm[i]!=0)return 2;}if(seed1>A1||seed2>B2||seed4>C4||A1-seed1<1)return 2;init();Reach R;memset(&R,0,sizeof(R));R.r[0][0]=1U;int sum=0,base[3]={25,5,1};for(int i=0;i<3;i++)if(sm[i]){Reach z;if(!addm(&R,base[i],sm[i],&z))return 3;R=z;sel[ns]=base[i];mul[ns]=sm[i];ns++;sum=addsum(sum,base[i],sm[i]);}dfs(0,0,1,&R,sum);printf("a1=%d b2=%d c4=%d seeds=%d,%d,%d plane2=%d nodes=%lld leaves=%lld solutions=%lld\n",A1,B2,C4,sm[0],sm[1],sm[2],plane2,nodes,leaves,sol);return sol?1:0;}
