/* X1-K: exact normalized support check for the C0(31) / Property-C branch.
 *
 * Mathematical role (NO novelty authority):
 ...
 * -sigma(U) in U.
 *
 * Expected current discovery result (must be replayed independently before
 * theorem authority): 564 normalized supports, 0 satisfying -sigma(U) in U.
 */
...
int main(void) {
    ...
    printf("{\n");
    printf("  \"schema\": \"ORION.RG.X1K.PropertyCSupportCheck.v1\",\n");
    printf("  \"authority\": {\"novelty_claim\": false, \"independent_external_replay_required\": true},\n");
