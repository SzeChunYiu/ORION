...
| NR-13 | P12 | Robustness stress V1: `price_axis=BROKEN` (zero-regret regime list empty over all 36 case records) and `distribution_shift_axis=BROKEN` (B1/B2 mixes fail) for unchanged `P12_TRANSFER_ALLOCATOR_V1` (#1006, receipt `P12_ROBUSTNESS_STRESS_RESULT_RECEIPT_V1`, run 32672856998; V1 FLAT zero-regret replicates; hidden-parameterization audit GREEN) | allocator's frozen greedy-by-q selection is price-invariant by construction — it optimizes the FLAT-charged objective, so any skew between build/serve prices moves realized cost without moving the selection; attribution is one-stage (selection rule, not oracle/outputs — RG1 exactness green everywhere) | price-and-shift-aware successor selector pre-registered against the same frozen 27-case pool + regimes/mixes (selection rule may read the same price vector the runner charges), re-run full battery with two implementations; FLAT replication must be preserved as a constraint | P12 price-aware allocator |
