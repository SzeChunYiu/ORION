...
| **Re-Test Plan** | **Path 2a (margin relaxation):** <br>1. Amend PROTOCOL_V1.json with H2_margin_revision from +0.02 to +0.10 <br>2. Update `assess_hypothesis` in statistics.py to reference the revised margin <br>3. Re-run hypothesis evaluation on existing frozen results <br>4. Report: "H2 at +0.02 is NOT_SUPPORTED; exploratory analysis at +0.10 shows [CONCLUSION]" <br><br>**Path 2b (control expansion):** <br>1. Design 14+ new evidence-only/execution-only control cases across 6 task families <br>2. Freeze expanded TEST_N at pilot→test split before execution <br>3. Execute full campaign on expanded suite <br>4. Re-assess H2 at original +0.02 margin |
...
| **Est. Effort** | M (margin change) OR L (control expansion) |
...
| **1** | **#001 — P1 H2 Power** | H × M | Execute p1-expand (TIER_B, n=385) |
| **2** | **#003 — P2 Baselines** | H × M | Verify PR #148 merge, execute frozen campaign |
| **2** | **#002 — P2 Deep Probe** | M × M | ✅ DIAGNOSTIC COMPLETE — Regenerate candidates with corrected retrieval |
...
|---------|------|---------|
| V1.1 | 2026-08-17 | Corrections applied: (1) P1 H2 lever changed to EXPANSION ONLY — margin relaxation forbidden (outcome tuning). (2) P2 Deep probe diagnostic COMPLETED — added PR #266 evidence: exact 0 / substring 0 / token≥0.5 = 8, 36 empty candidate lists, candidate generation is failing stage. |
...
2. **Execute top items** — CI flakiness fix + P1 H2 margin relaxation (parallel, independent)
3. **Execute P2 items** — Deep probe diagnostic + baselines execution (after PR #148)
