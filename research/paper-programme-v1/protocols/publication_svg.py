...
def _axes(lines: list[str], *, y_min: float, y_max: float, y_label: str) -> None:
...
def bar_chart(rows: Iterable[dict[str, object]], *, label_key: str, value_key: str, title: str, y_label: str) -> str:
    ...
    for i, (label, value) in enumerate(data):
        ...
        lines.append(f'<rect x="{cx-bar_w/2:.2f}" y="{top:.2f}" width="{bar_w:.2f}" height="{max(1.0,bottom-top):.2f}" fill="#666"/>')
...
def heatmap(rows: list[dict[str, object]], *, row_key: str, col_key: str, value_key: str, title: str) -> str:
    if not rows:
