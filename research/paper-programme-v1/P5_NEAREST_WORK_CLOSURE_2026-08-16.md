...
| Ratchet (`arXiv:2605.22148`) | Self-evolving skill lifecycle management already combines self-authored skills, outcome-driven retirement, bounded active capacity, held-out transfer and eight mechanism ablations. | `ADAPT` | Skill retention/retirement, bounded accumulation and held-out transfer are not standalone novelty; use Ratchet or a faithful equivalent when runnable. |
| Verifier-as-Gatekeeper / skill-contamination study (`arXiv:2608.05810`) | Harmful skill admission can contaminate later self-evolution irreversibly; heterogeneous pre-commit critics and frozen-pool transfer are explicitly tested. | `ADOPT` | Pre-commit gating and cross-backbone/benchmark transfer checks are prior-art pressure. P5 must distinguish external evaluator custody, causal licensing, negative-history integrity and host authority rather than claim gating itself. |
| RewardHackingAgents (`arXiv:2603.11337`) | Evaluator tampering and held-out leakage are directly benchmarked with patch/access telemetry and evaluator locking. | `ADOPT` | Evaluator integrity/access telemetry is a mandatory parent baseline and threat model, not standalone novelty. |
...
| Current 2026+ self-evolving-agent frontier — MOSS, Ratchet, VaG, MEGA (`arXiv:2608.10504`) and `Self-Evolving Coding Agents` survey (`arXiv:2608.03392`) | MOSS pressures source rewrite/replay/promotion; Ratchet pressures lifecycle hygiene and held-out transfer; VaG pressures pre-commit gating and contamination prevention; MEGA pressures durable evolving optimization knowledge and controlled effect attribution; the survey makes reliability, overfitting, safety, cost and generalization first-class field concerns. | `ADAPT` | At execution freeze, select the strongest runnable current arm(s) under matched resources. Re-run this row within 14 days of submission; no fixed 2026 snapshot may be treated as permanently current. |

## Claim contraction after the fresh audit

P5 does **not** claim novelty for any of the following in isolation: persistent issue state, failure-driven improvement, multi-hypothesis diagnosis, causal attribution, source-level self-rewriting, archive persistence, replay validation, persistent behavioral rules, skill retirement/capacity hygiene, pre-commit skill gating, durable optimization knowledge, sandboxing, rollback, evaluator-guided search, evaluator locking/access telemetry, or externally gated deployment.
