...
def scientific_terminal(native_valid, type_flags, narrowing_ok, blocker, support_a, support_b, protected_coercion):
    if not native_valid:
    ...
    if not (support_a or support_b):
        return "BLOCK"
    if not (all(type_flags) or protected_coercion):
        return "BLOCK"
    return "DISCHARGE"
...
#
# X3's T1 is "scientific lifting does not alter the donor-native verdict", and it
# spans a wider donor set than X2: the six security/delegation/evidence families
# plus four scientific claim/release authorities whose *own* verification and
# adjudication results the theorem promises to leave intact. This file published
...
# evaluated 30,720 times, satisfied 0 times, and 0 under every theory of
...
#
# X3's T9 is "a decentralized product given the identical scientific type,
# narrowing, blocker, support and coercion rules ties the shared calculus
# exactly", and this file published `ideal_product_mismatches: 0` for it from
#
...
#
# The construction T9 names is neither the rule itself nor a copy of it: the
...
def main():
    ...
    # 30,720. Read off the signature rather than counted, because it is a
    ...
    result = {
        "schema": "P8.X3.AuthorityLiftingResult.v1",
        ...
        "terminal": (
            ...
            and type_separations == 50
            and coercion_successes == 50
            and unprotected_countermodels == 50
            and support_survivals == 20
            and all_support_broken == 10
            and chain_successes == 100
            and chain_widening_countermodels == 100
            else "FAIL"
        ),
    }
