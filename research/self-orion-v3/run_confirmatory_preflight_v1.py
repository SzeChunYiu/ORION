...
DEFAULT_BASELINES = ROOT / "research" / "self-orion-v3" / "BASELINE_STRUCTURAL_BINDINGS_V1.json"
...
def derive(protocol_path: Path = DEFAULT_PROTOCOL, baseline_path: Path = DEFAULT_BASELINES) -> dict[str, object]:
    ...
    return {
        "schema": "SelfOrionV3ConfirmatoryPreflightCommand.v1",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Report or enforce Self-ORION V3 confirmatory readiness.")
    ...
    if args.require_ready and result["status"] != ConfirmatoryPreflightStatus.READY_TO_FREEZE_CONFIRMATORY.value:
        raise SystemExit("Self-ORION V3 confirmatory execution is not ready; see blockers above")
