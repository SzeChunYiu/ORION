...
def execute(holdout_dir: Path, execution_freeze_path: Path, outdir: Path) -> dict[str, Any]:
    ...
    policy_by_id = {policy.__name__: policy for policy in MATCHED_ARMS}
    matched_arm_ids = tuple(str(item) for item in freeze.get("matched_arm_ids", ()))
    if not matched_arm_ids:
        raise ValueError("execution freeze does not bind matched-arm identities")
    if len(set(matched_arm_ids)) != len(matched_arm_ids):
        raise ValueError("execution freeze matched-arm identities are duplicated")
    missing_arm_ids = sorted(set(matched_arm_ids) - set(policy_by_id))
    if missing_arm_ids:
        raise ValueError(f"bound matched-arm implementation missing:{missing_arm_ids}")
    policies = tuple(policy_by_id[arm_id] for arm_id in matched_arm_ids)
    if freeze.get("primary_arm") not in matched_arm_ids:
        raise ValueError("primary arm is absent from bound matched arms")
    if freeze.get("primary_comparator") not in matched_arm_ids:
        raise ValueError("primary comparator is absent from bound matched arms")
