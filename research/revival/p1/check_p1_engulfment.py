...
def validate_engulfment(
    ...
    for component in components:
        ...
        if not isinstance(matrix_rows, list):
            ...
            continue
        extra_donors = component.get("extra_donors")
        if not isinstance(extra_donors, list):
            errors.append(f"component_extra_donors_not_list:{component_id}")
            extra_donors = []
        normalized_extra_donors = [str(item).strip() for item in extra_donors]
        if any(not item for item in normalized_extra_donors):
            errors.append(f"component_extra_donor_empty:{component_id}")
        if len(set(normalized_extra_donors)) != len(normalized_extra_donors):
            errors.append(f"component_extra_donor_duplicate:{component_id}")
        if normalized_extra_donors and mode not in ACTIVE_MODES:
            errors.append(f"active_extra_donor_not_integrated:{component_id}:{mode}")
