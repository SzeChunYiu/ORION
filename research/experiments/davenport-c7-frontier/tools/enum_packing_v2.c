...
#define MAXS 12
static int p, L, cap, s, kmax, need_zs=0, planecap=1000000, count_only=0, G;
...
static void dfs(int i){
    nodes++; if(progress && (nodes&((1LL<<26)-1))==0){ fprintf(stderr,"progress nodes=%lld leaves=%lld found=%lld\n",nodes,leaves,found); }
    if(total==L){
        leaves++;
    ...
    if(i>=3){ mult[i]=0; dfs(i+1); cur_i=i; }
    for(int m=1;m<=cmax;m++){
        ...
        mult[i]=m; total+=1;
        int planebad=0; for(int q=0;planes_of[i][q]>=0;q++){ planecnt[planes_of[i][q]]++; if(planecnt[planes_of[i][q]]>planecap) planebad=1; }
        if(!planebad) dfs(i+1);
        cur_i=i;
        for(int q=0;planes_of[i][q]>=0;q++) planecnt[planes_of[i][q]]--;
        if(planebad) break;
    }
    if(mult[i]){ total-=mult[i]; mult[i]=0; }
...
int main(int argc,char**argv){
    ...
    for(int a=6;a<argc;a++){ if(!strcmp(argv[a],"--zerosum")) need_zs=1; else if(!strcmp(argv[a],"--planecap")) planecap=atoi(argv[++a]); else if(!strcmp(argv[a],"--count-only")) count_only=1; else if(!strcmp(argv[a],"--progress")) progress=1; }
