...
The branch workflow compiles and runs the independent verifier in two modes:
...
The audit grants no novelty credit and does not advance the frontier beyond `c>=3`. Its role is to make the finite `p=13` authority and the arithmetic-resonance isolation independently reproducible before the campaign moves to `c>=3`.
