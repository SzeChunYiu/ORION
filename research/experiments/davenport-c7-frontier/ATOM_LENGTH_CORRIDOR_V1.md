...
then `z(B)=3`, and every factorization of `B` into three atoms has, after sorting lengths, one of exactly six length triples:
...
The complement `C=BA^{-1}` is total-zero, has packing number at most two, and has length `29`, `28`, or `27`. Since all three lengths exceed `D(G)=19`, `C` is not an atom and hence factors into exactly two atoms `U,V`. Because `A` was shortest, `|U|,|V|>=|A|`, and both are at most 19.
...
This is a corridor, not a solution of `D_3(C_7^3)`. The six patterns are necessary for a length-37 total-zero packing obstruction; none is claimed to exist. The inputs `D_2=29`, Zhao's lemma, Olson's `D=19`, and Zhang's short-zero theorem are donor-owned.
