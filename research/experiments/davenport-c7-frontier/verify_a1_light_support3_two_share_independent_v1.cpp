...
struct P13Engine {
    ...
    bool exact_pair_compatible(const V3 &x, const V3 &y) const {
    ...
    std::tuple<int, int, int> run() const {
        int structural = 0;
        int singleton = 0;
        int compatible = 0;
        for (int a = 0; a < p; ++a) {
            for (int b = 0; b < p; ++b) {
                for (int c = 0; c < p; ++c) {
                    ...
                    if (1 + rho[enc(neg(x))] >= m && 1 + rho[enc(neg(y))] >= m)
                        ++singleton;
                    if (exact_pair_compatible(x, y)) ++compatible;
                }
            }
        }
        return {structural, singleton, compatible};
    }
};
...
int main() {
    ...
    const auto [structural, singleton, compatible] = engine.run();
    ...
    std::cout << "{\"status\":\"A1_LIGHT_SUPPORT3_TWO_SHARE_INDEPENDENT_GREEN\","
              ...
              << "\"p13_exact_pair_survivors\":" << compatible << "}\n";
}
