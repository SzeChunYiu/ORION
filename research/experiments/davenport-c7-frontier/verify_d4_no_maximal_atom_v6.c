...
static unsigned char reps[8000][13]; static int nreps;
...
static void canon(int *A, unsigned char *best){
    ...
    for(int q=0;q<13;q++) if(!seen[A[q]]++) sup[ns++]=A[q];
    int have=0; unsigned char cur[13];
    for(int i=0;i<ns;i++)for(int j=0;j<ns;j++){ if(j==i) continue;
      for(int k=0;k<ns;k++){ if(k==i||k==j) continue;
        ...
        for(int x=1;x<13;x++){ unsigned char t=cur[x]; int y=x-1;
            while(y>=0&&cur[y]>t){cur[y+1]=cur[y];y--;} cur[y+1]=t; }
        if(!have||memcmp(cur,best,13)<0){ memcpy(best,cur,13); have=1; }
      }}
}

static unsigned long long h64(unsigned char*p){
    unsigned long long h=1469598103934665603ULL;
    for(int i=0;i<13;i++){h^=p[i];h*=1099511628211ULL;}
    return h;
}
static int insert(unsigned char*c){
    unsigned long long h=h64(c); size_t i=h&(HS-1);
    while(used[i]){ if(!memcmp(tab+13*i,c,13)) return 0; i=(i+1)&(HS-1); }
    used[i]=1; memcpy(tab+13*i,c,13); hcount++; return 1;
}
...
static void dfs(int start,int len,unsigned long long S[2]){
    if(len==12){
        ...
        if(!w) return;
        natom++;
        int A[13]; for(int q=0;q<12;q++) A[q]=chosen[q]; A[12]=w;
        unsigned char c[13]; canon(A,c);
        if(insert(c)){ norbit++; if(nreps<8000) memcpy(reps[nreps++],c,13); }
        return;
    }
...
int main(void){
    ...
    clock_t t0=clock();
    dfs(0,3,S);
    printf("maximal atoms %lld -> GL(3,5) ORBITS %lld   (%.1fs)\n",
