...
def _assert_pair_collides_until_semantic_target_is_exposed(pair):
...
def test_entanglement_pair_collides_until_semantic_target_is_exposed():
    ...
    _assert_pair_collides_until_semantic_target_is_exposed(pair)
...
def test_clifford_magic_pair_collides_until_semantic_target_is_exposed():
    pair = generate_quantum_hostile_pair(
        QuantumHostileFamily.CLIFFORD_MAGIC,
        seed="orion-q-clifford-hostile",
    )
    _assert_pair_collides_until_semantic_target_is_exposed(pair)
...
def test_balanced_quantum_information_lattice_has_exact_half_half_one_ceilings():
    ...
    assert surface["sample_count"] == 30
    assert surface["distinct_view_count"] == 15
    assert surface["collision_bucket_count"] == 15
    assert surface["max_deterministic_accuracy"] == 0.5

    assert contract["sample_count"] == 30
    assert contract["distinct_view_count"] == 15
    assert contract["collision_bucket_count"] == 15
    assert contract["max_deterministic_accuracy"] == 0.5

    assert semantic["sample_count"] == 30
    assert semantic["distinct_view_count"] == 30
...
def test_model_views_exclude_evaluator_family_case_ids_and_gold_for_every_family():
    for family in QuantumHostileFamily:
        pair = generate_quantum_hostile_pair(
            family,
            seed=f"orion-q-no-gold-leakage-{family.value}",
        )
        case = pair.cases[0]
