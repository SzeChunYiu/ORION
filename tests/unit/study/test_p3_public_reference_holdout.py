...
def _case(case_id: str, discipline: str = "d", relation: str = "COMPATIBLE") -> dict[str, object]:
    return {
...
def test_build_holdout_fails_closed_on_overlap(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(mod, "scifact_claim_cases", lambda _: [_case("b0", "e", "CONTRADICTORY"), _case("b1", "e", "CONTRADICTORY")])
    monkeypatch.setattr(mod, "scischema_identity_cases", lambda _: [_case("c0", "f"), _case("c1", "f")])
...
def test_build_holdout_is_ready_when_disjoint_and_supported(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(mod, "scifact_claim_cases", lambda _: [_case("b0", "d2", "CONTRADICTORY"), _case("b1", "d2", "CONTRADICTORY")])
    monkeypatch.setattr(mod, "scischema_identity_cases", lambda _: [_case("c0", "d3"), _case("c1", "d3")])
