...
from orion.study.p1_causal import CausalContext, CausalDependency, dual_slice_causal_context
...
def test_runtime_component_registry_matches_engulfment_executable_ids() -> None:
    ...
    causal = runtime["CAUSAL_DIAGNOSIS"]
    assert "CausalRepair" in causal.donor_keys
    assert {
        "CausalDependency",
        "CausalContext",
        "dual_slice_causal_context",
    }.issubset(causal.executable_surface)
    assert CausalContext is not None
    assert CausalDependency is not None
    assert callable(dual_slice_causal_context)
