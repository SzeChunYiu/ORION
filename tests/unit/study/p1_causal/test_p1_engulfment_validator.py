...
def test_extra_donor_cannot_be_parked_outside_an_active_component() -> None:
    coverage, engulfment = _payloads()
    mutated = copy.deepcopy(engulfment)
    component = next(item for item in mutated["components"] if item["extra_donors"])
    component["integration_mode"] = "DEFERRED_WATCHLIST"
    errors = checker.validate_engulfment(coverage, mutated)
    assert (
        f"active_extra_donor_not_integrated:{component['component_id']}:DEFERRED_WATCHLIST"
        in errors
    )


def test_extra_donor_identity_cannot_be_blank_or_duplicated() -> None:
    coverage, engulfment = _payloads()
    mutated = copy.deepcopy(engulfment)
    component = next(item for item in mutated["components"] if item["extra_donors"])
    donor = component["extra_donors"][0]
    component["extra_donors"] = ["", donor, donor]
    errors = checker.validate_engulfment(coverage, mutated)
    assert f"component_extra_donor_empty:{component['component_id']}" in errors
    assert f"component_extra_donor_duplicate:{component['component_id']}" in errors
