...
INDEPENDENT_VERIFIER = (
    ROOT / "research" / "revival" / "p1" / "verify_mutation_necessity_independent.py"
)
_verifier_spec = importlib.util.spec_from_file_location(
    "p1_necessity_independent_verifier",
    INDEPENDENT_VERIFIER,
)
assert _verifier_spec is not None and _verifier_spec.loader is not None
independent_verifier = importlib.util.module_from_spec(_verifier_spec)
sys.modules[_verifier_spec.name] = independent_verifier
_verifier_spec.loader.exec_module(independent_verifier)
    ...
    for world in _development_worlds():
        for policy in policies:
            budget = (
                99.0
                if unlimited
                and policy.__name__ == "orion_with_unlimited_intervention_budget"
                else 4.0
            )
...
def test_full_certificate_beats_all_engulfed_strong_parents_on_development_hidden_shift() -> None:
    ...
    for parent_id in parent_ids:
        ...
        assert (
            full["hidden_shift_protected_root_task_success_rate"]
            - parent["hidden_shift_protected_root_task_success_rate"]
            >= 0.20
        )
    ...
    assert result["h1_parents"] == list(parent_ids)
    assert len(result["h1_hidden_shift"]) == 3
    assert result["all_support_gates_pass"] is True


def test_cost_gate_uses_maximum_strong_parent_mean_as_frozen() -> None:
    rows, _ = _run(RUNNABLE_ARMS)
    adjusted = []
    for row in rows:
        item = dict(row)
        if item["arm_id"] == "orion_mutation_necessity":
            item["spent_budget"] = 4.0
        elif item["arm_id"] == "causalflow_minimal_counterfactual_parent":
            item["spent_budget"] = 4.0
        elif item["arm_id"] in {
            "active_voi_repair_parent",
            "darc_r2act_dependency_parent",
        }:
            item["spent_budget"] = 0.0
    ...
    assert result["cost_guard"] == {
        "full_mean": 4.0,
        "maximum_strong_parent_mean": 4.0,
        "margin_units": 1.0,
        "difference_from_maximum_parent": 0.0,
    }
    assert result["gates"]["cost_within_margin"] is True
    independently_recomputed = independent_verifier.independent_analysis(
        adjusted,
        {
    ...
    assert independently_recomputed == result
...
def test_causalflow_parent_engulfs_minimal_repair_but_not_protected_science_gate() -> None:
    ...
    assert causalflow["protected_sibling_regression_rate"] > 0.0
...
def test_registered_ablations_move_the_mechanism_not_just_the_label() -> None:
    ...
    assert ablations["orion_without_protected_sibling_check"][
        "hidden_shift_protected_root_task_success_rate"
    ] < full["hidden_shift_protected_root_task_success_rate"]
    assert ablations["orion_without_dependency_impact_binding"][
        "hidden_shift_protected_root_task_success_rate"
    ] < full["hidden_shift_protected_root_task_success_rate"]
    assert ablations["orion_without_lower_level_exclusion"][
        "negative_control_unnecessary_high_level_reframe_rate"
    ] > full["negative_control_unnecessary_high_level_reframe_rate"]
    assert ablations["orion_without_KWM_level_ordering"][
        "negative_control_unnecessary_high_level_reframe_rate"
    ] > full["negative_control_unnecessary_high_level_reframe_rate"]
    assert ablations["orion_with_unlimited_intervention_budget"][
        "protected_root_task_success_rate"
    ] >= full["protected_root_task_success_rate"]


def test_selected_repair_must_have_been_executed() -> None:
    world = next(
        item for item in _development_worlds() if item.protected.hidden_shift
    )
...
def test_target_flip_without_protected_sibling_preservation_fails() -> None:
    ...
    score = score_necessity_world(
        world,
        {
            ...
            "reopened_ids": list(
                world.protected.repair_response(bad_id).observed_invalidated_ids
            ),
