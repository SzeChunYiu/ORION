...
def test_publication_metrics_match_reproduced_headline() -> None:
    ...
    for key in ("candidate_protected_identifier_hits", "comparator_protected_identifier_hits", "candidate_external_ip_connects", "comparator_external_ip_connects"):
        assert data["telemetry"][key] == 0


def test_manuscript_uses_v2_and_reports_null_h3() -> None:
    text = _manuscript_text()
    assert "31976589735" in text
