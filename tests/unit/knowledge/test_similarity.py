"""Regression battery reused from RAKL typed similarity witnesses."""
...
def test_valid_witness_is_scoped_and_fail_closed():
    assert validate_similarity_witness(_witness()).verdict is WitnessVerdict.VALID
...
def test_retrieved_but_unmapped_is_recognition_failure():
    assert (
        diagnose_analogy_discovery(_observation(witness=None)).verdict
        is AnalogyDiscoveryVerdict.RECOGNITION_FAILURE
    )
...
def test_hidden_answer_exposure_invalidates_trial():
    assert (
        diagnose_analogy_discovery(_observation(hidden_label_exposed=True)).verdict
        is AnalogyDiscoveryVerdict.TRIAL_INVALID
    )
...
def test_same_domain_repetition_is_not_cross_domain_confirmation():
    ...
    assert corroborate_structural_motif((first, second)).verdict is MotifVerdict.CANNOT_CHECK
