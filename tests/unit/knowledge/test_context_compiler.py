"""Regression battery carried over from RAKL context_compiler.

Donor tests:
- RAKL@70f5f7c4a6771ffd1158765b42ac9f8aee8a270f:tests/test_context_compiler.py
- RAKL@70f5f7c4a6771ffd1158765b42ac9f8aee8a270f:tests/test_context_compiler_budget_guard.py
"""
...
def test_mechanism_authority_prerequisites_fail_closed_when_not_materialized():
    items = [
...
def test_negative_history_guard_is_kept_outside_target_fiber_when_mandatory():
    items = [
        ContextItem(
            "old-falsifier",
            3,
...
def test_empty_relevant_set_does_not_add_filler():
    items = [ContextItem("other", 2, ("other",), fiber_ids=("other-fiber",))]
    report = compile_epistemic_context(
        items,
...
def test_h30_budget_excludes_all_relevant_context_is_cannot_compile():
    items = [
        ContextItem(f"r{i}", 400, (f"a{i}",), fiber_ids=("f",))
        for i in range(30)
    ]
    report = compile_epistemic_context(
        items,
        ContextCompileRequest(budget_tokens=5, target_fibers=("f",)),
    )
...
def test_h30_partial_fit_remains_normal_compilation():
    items = [
        ContextItem("small", 4, ("a",), fiber_ids=("f",)),
        ContextItem("big", 400, ("b",), fiber_ids=("f",)),
    ]
    report = compile_epistemic_context(
        items,
        ContextCompileRequest(budget_tokens=5, target_fibers=("f",)),
...
def test_h30_zero_marginal_candidate_is_not_over_budget():
