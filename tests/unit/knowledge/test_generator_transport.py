"""Regression battery reused from RAKL generator transport tests."""
...
def test_category_only_relation_is_discovery_hint_not_transfer():
    ...
    report = evaluate_generator_relation(
        _witness(relation=GeneratorRelation.SHARES_CATEGORY_WITH, source_lift=source, target_lift=target)
    )
...
def test_shared_parent_is_retrieval_hint_only():
    ...
    report = evaluate_generator_relation(
        _witness(relation=GeneratorRelation.SHARES_PARENT_WITH, source_lift=source, target_lift=target)
    )
...
def test_abstract_schema_is_analogy_proposal_only():
    ...
    report = evaluate_generator_relation(
        _witness(relation=GeneratorRelation.SHARES_ABSTRACT_SCHEMA_WITH, source_lift=source, target_lift=target)
    )
...
def test_mechanism_family_can_create_transfer_hypothesis_but_not_authority():
    ...
    report = evaluate_generator_relation(
        _witness(relation=GeneratorRelation.SHARES_MECHANISM_FAMILY_WITH, source_lift=source, target_lift=target)
    )
...
def test_required_core_feature_contradiction_rejects_relation():
    ...
    report = evaluate_generator_relation(_witness(target_lift=target))
    assert report.verdict is GeneratorRelationVerdict.REJECT
    assert "required_core_feature_explicitly_not_preserved:feedback" in report.reasons
...
def test_category_evidence_cannot_escalate_to_generator_claim():
    ...
    target = _lift(instance_id="banana", evidence_scope=EvidenceScope.CATEGORY_ONLY)
    report = evaluate_generator_relation(
        _witness(relation=GeneratorRelation.SHARES_GENERATOR_WITH, source_lift=source, target_lift=target)
    )
    assert report.verdict is GeneratorRelationVerdict.REJECT
...
def test_posthoc_relation_expansion_invalidates_trial():
    report = evaluate_generator_relation(_witness(relation_declared_before_outcomes=False))
    assert report.verdict is GeneratorRelationVerdict.TRIAL_INVALID
...
def test_same_lineage_support_is_not_independent_corroboration():
    ...
    report = assess_generator_family(
        GeneratorFamilyCandidate(
            candidate_id="candidate-1", generator_id="ripening-generator",
            question_or_qoi="what controls the state transition?",
            abstraction_level=AbstractionLevel.L3,
            required_core_features=("state_transition", "feedback"), lifts=lifts,
            declared_before_outcomes=True, hidden_labels_exposed=False,
        )
    )
...
def test_multi_lineage_multi_environment_support_is_proposal_only():
    lifts = (
        ...
        _lift(instance_id="banana", evidence_ids=("paper-2",), evidence_lineage_ids=("lineage-B",), environment_id="env-B"),
    )
    report = assess_generator_family(
        GeneratorFamilyCandidate(
            candidate_id="candidate-2", generator_id="ripening-generator",
            question_or_qoi="what controls the state transition?",
            abstraction_level=AbstractionLevel.L3,
            required_core_features=("state_transition", "feedback"), lifts=lifts,
            declared_before_outcomes=True, hidden_labels_exposed=False,
...
def test_outlier_sibling_is_preserved_not_forced_into_family():
    lifts = (
        ...
        _lift(instance_id="banana", evidence_ids=("paper-2",), evidence_lineage_ids=("lineage-B",), environment_id="env-B"),
        _lift(
            instance_id="stone", instance_domain="materials", generator_id="ripening-generator",
            evidence_scope=EvidenceScope.ABSTRACT_SCHEMA, preserved=("state_transition",),
            not_preserved=("feedback",), evidence_ids=("paper-3",),
            evidence_lineage_ids=("lineage-C",), environment_id="env-C",
        ),
    )
    report = assess_generator_family(
        GeneratorFamilyCandidate(
            candidate_id="candidate-3", generator_id="ripening-generator",
            question_or_qoi="what controls the state transition?", abstraction_level=AbstractionLevel.L3,
            required_core_features=("state_transition", "feedback"), lifts=lifts,
            declared_before_outcomes=True, hidden_labels_exposed=False,
        )
    )
...
def test_unknown_sibling_is_preserved_as_unknown():
    lifts = (
        ...
        _lift(instance_id="banana", evidence_ids=("paper-2",), evidence_lineage_ids=("lineage-B",), environment_id="env-B"),
        _lift(
            instance_id="pear", preserved=("state_transition",),
            not_preserved=("species_specific_substrate",), evidence_ids=("paper-3",),
            evidence_lineage_ids=("lineage-C",), environment_id="env-C",
        ),
    )
    report = assess_generator_family(
        GeneratorFamilyCandidate(
            candidate_id="candidate-4", generator_id="ripening-generator",
            question_or_qoi="what controls the state transition?", abstraction_level=AbstractionLevel.L3,
            required_core_features=("state_transition", "feedback"), lifts=lifts,
            declared_before_outcomes=True, hidden_labels_exposed=False,
        )
...
def test_target_refutation_preserves_generator_witness():
    report = project_through_generator(_trial(target_tested=True, target_passed=False))
    assert report.verdict is GeneratorTransportVerdict.TARGET_REFUTED_GENERATOR_WITNESS_PRESERVED
    assert report.relation_report is not None
    assert report.relation_report.verdict is GeneratorRelationVerdict.TRANSFER_HYPOTHESIS_ONLY


def test_target_pass_still_requires_separate_promotion():
    report = project_through_generator(_trial(target_tested=True, target_passed=True))
    assert report.verdict is GeneratorTransportVerdict.TARGET_TEST_PASSED_SEPARATE_PROMOTION_REQUIRED
    assert report.activates_canonical_knowledge is False
