from orion.knowledge.promotion_attestation import (
...
def test_validation_document_cannot_override_missing_active_ancestry():
    assert verdict(main_descends_from_candidate=False, validation_doc_present=True) == PromotionAttestationVerdict.REFUTED_CLAIM


def test_exact_candidate_active_main_can_be_confirmed():
    assert verdict(observed_main_sha=CANDIDATE) == PromotionAttestationVerdict.ACTIVE_PROMOTION_CONFIRMED
...
def test_claimed_promoted_sha_mismatch_requires_explicit_supersession():
    assert verdict(claimed_promoted_sha="d" * 40) == PromotionAttestationVerdict.REFUTED_CLAIM
    assert verdict(claimed_promoted_sha="d" * 40, explicit_supersession=True) == PromotionAttestationVerdict.ACTIVE_PROMOTION_CONFIRMED
...
def test_postpromotion_exact_main_validation_is_required():
    assert verdict(postpromotion_ci_exact_active_main=False) == PromotionAttestationVerdict.REFUTED_CLAIM
    assert verdict(postpromotion_ci_exact_active_main=None) == PromotionAttestationVerdict.CANNOT_CHECK
