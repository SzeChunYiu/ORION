"""Regression battery reused from RAKL measurement-aware similarity tests."""
...
def _witness(
    relation: SimilarityRelation = SimilarityRelation.SAME_OBSERVABLE,
) -> SimilarityWitness:
    return SimilarityWitness(
        ...
        probe_family=ProbeFamily(
            family_id="measurement-probes-v1", probe_ids=("p1", "p2")
        ),
    )
...
def _trial(
    ...
    **overrides,
    ...
    values = dict(
        ...
        probes=(
            ObservationProbe("p1", 300.00, 300.03),
            ObservationProbe("p2", 302.00, 302.04),
        ),
...
def test_m01_same_measurand_operator_calibrated_is_supported_proposal_only():
    ...
    assert not report.activates_scientific_authority
    assert not report.activates_target_authority
    assert not report.implies_same_mechanism
...
def test_m05_posthoc_measurement_transform_invalidates_trial():
    assert (
        ...
        is MeasurementRelationVerdict.TRIAL_INVALID
    )
...
def test_m06_unknown_operator_compatibility_cannot_check():
    ...
    report = evaluate_measurement_relation(
        _trial(
            target=target,
            mapping=_mapping(observation_operator_compatibility=None),
        )
    )
...
def test_m11_frozen_probe_family_can_support_observational_equivalence():
    report = evaluate_measurement_relation(
        _trial(SimilarityRelation.OBSERVATIONALLY_EQUIVALENT)
    )
...
def test_m16_same_observable_never_implies_same_mechanism():
    assert not evaluate_measurement_relation(_trial()).implies_same_mechanism
...
def test_generic_similarity_witness_is_structural_not_measurement_certificate():
    generic = validate_similarity_witness(
        _witness(SimilarityRelation.OBSERVATIONALLY_EQUIVALENT)
    )
...
def test_non_measurement_relation_is_rejected_by_measurement_extension():
    report = evaluate_measurement_relation(
        _trial(SimilarityRelation.RELATIONALLY_ANALOGOUS)
    )
    assert report.verdict is MeasurementRelationVerdict.REJECT
