...
def test_transition_plan_declares_guarded_nondeterministic_and_retry_semantics():
    plan = default_transition_plan(MechanicCell("SEARCH.test", "Find evidence.", "fixture"))
    assert any(item.determinism is TransitionDeterminism.EXTERNAL_NONDETERMINISTIC for item in plan.rules)
    assert any(item.retry_safety is RetrySafety.SIDE_EFFECT_GUARD_REQUIRED for item in plan.rules)
...
def test_transition_plan_closes_lifecycle_relation_but_keeps_step_specific_dynamics_open():
    ...
    assert "TRANSITION_MODEL" not in dimensions
    assert "MATHEMATICS" in dimensions
    assert any("step-specific transition relation" in item for item in updated.empirical_open_coordinates)
