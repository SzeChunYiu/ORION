...
def test_current_program_advances_to_transition_model():
    metrics = observe_current_mechanics_program()
    counts = dict(metrics.open_by_dimension)
    for closed in (MechanicDimension.VERIFICATION, MechanicDimension.FAILURE, MechanicDimension.OBSERVABILITY, MechanicDimension.HANDOFF, MechanicDimension.STATE):
        assert closed.value not in counts
    assert counts[MechanicDimension.TRANSITION_MODEL.value] == metrics.mechanic_count
...
def test_global_scheduler_is_breadth_first_not_one_cell_deep():
    ...
    assert all(item.dimension is MechanicDimension.TRANSITION_MODEL for item in questions)
