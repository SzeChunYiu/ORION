...
def test_default_verification_plan_separates_plan_from_evidence():
    ...
    assert any(
        item.stage is VerificationStage.EMPIRICAL_PROMOTION for item in plan.obligations
    )
...
def test_attaching_generic_verification_plan_keeps_step_specific_question_open():
    cell = MechanicCell("SEARCH.test", "Find evidence.", "fixture")
    before = {item.dimension.value for item in generate_mechanic_questions(cell)}
    after = {
        item.dimension.value
        for item in generate_mechanic_questions(apply_default_verification_plan(cell))
    }
    assert "VERIFICATION" in before
    assert "VERIFICATION" in after
