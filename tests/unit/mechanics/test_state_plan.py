...
def test_state_plan_binds_replay_identity_and_transition_controlled_working_state():
    ...
    assert any(item.mutability is StateMutability.TRANSITION_CONTROLLED for item in plan.variables)
    assert "hidden mutation" in plan.mutation_rule


def test_state_plan_closes_execution_state_question_but_keeps_step_specific_state_open():
    ...
    assert "STATE" not in dimensions
    assert "TRANSITION_MODEL" in dimensions
    assert any("step-specific scientific/algorithmic state" in item for item in updated.empirical_open_coordinates)
