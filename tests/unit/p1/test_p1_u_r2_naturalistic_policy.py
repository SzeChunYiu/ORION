...
def test_orion_r2_reaches_every_registered_responsibility_before_holdout():
    ...
    cases = [
        (
            "SEARCH_OR_EVIDENCE",
            case("The analysis reports missing evidence and insufficient data coverage; additional data may resolve the result.", "SEARCH_OR_EVIDENCE"),
        ),
        (
            "REPRESENTATION_OR_INTERFACE",
            case("The same objective behaves differently after serialization; a representation encoding interface mapping is suspected.", "REPRESENTATION_OR_INTERFACE"),
        ),
        (
            "IMPLEMENTATION_OR_ENVIRONMENT",
            case("A software pipeline configuration dependency and environment version bug makes the result fail.", "IMPLEMENTATION_OR_ENVIRONMENT"),
        ),
        (
            "MEASUREMENT_OR_EVALUATOR",
            case("The benchmark metric changes after calibration of the instrument and evaluator scoring procedure.", "MEASUREMENT_OR_EVALUATOR"),
        ),
        (
            "OBJECTIVE_OR_MODEL_CLASS",
            case("Persistent residual error indicates a misspecified hypothesis and inadequate model class for the mechanism.", "OBJECTIVE_OR_MODEL_CLASS"),
        ),
        (
            "PROBLEM_BOUNDARY",
            case("An omitted process outside scope changes the system boundary and the relevant population context.", "PROBLEM_BOUNDARY"),
        ),
        (
            "NO_HIGH_LEVEL_REFORMULATION",
            case("The adequate model is reproduced and stable; a control shows no change is needed and the same objective remains sufficient.", no_high=True),
        ),
        (
            "UNRESOLVED",
            case("Conflicting evidence leaves several plausible explanations with no decisive diagnostic available.", unresolved=True),
        ),
    ]
...
def test_no_policy_receives_gold_label_field():
    ...
    # Policies intentionally ignore evaluator-only gold.
