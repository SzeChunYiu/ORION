...
def test_missing_or_tampered_bytes_are_cannot_check(tmp_path) -> None:
    for mutation in ("missing", "tampered"):
        root, result, receipt_path, _, receipt_hash, document_hash = _write_fixture(tmp_path / mutation)
        if mutation == "missing":
            result.unlink()
        else:
        ...
        assert any(
            blocker.startswith("artifact_missing:") or blocker.startswith("artifact_hash_mismatch:")
...
def test_host_selected_subject_evaluator_and_epoch_dominate_receipt(tmp_path) -> None:
    ...
    for substitution in substitutions:
        report = _audit(receipt_path, root, receipt_hash, document_hash, **substitution)
        assert report.status == "CANNOT_CHECK"
        assert report.blockers
...
def test_cli_emits_machine_readable_non_authorizing_result(tmp_path, capsys) -> None:
    ...
    exit_code = main(
        [
            ...
            "--subject-revision-hash", SUBJECT,
            "--evaluator-artifact-hash", EVALUATOR,
            "--evaluation-epoch-id", EPOCH,
        ]
    )
