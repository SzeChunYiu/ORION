...
def test_stale_receipt_hash_is_rejected_even_if_document_hash_is_recomputed(tmp_path) -> None:
    ...
    without_document_hash = {key: value for key, value in document.items() if key != "document_hash"}
...
def test_unknown_document_or_artifact_fields_fail_closed_after_hash_recomputation(tmp_path) -> None:
    from orion.self_orion import evidence_admission_io as io
