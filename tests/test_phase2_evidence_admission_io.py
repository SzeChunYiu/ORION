...
def test_stale_receipt_hash_is_rejected_even_if_document_hash_is_recomputed(
...
def test_unknown_document_or_artifact_fields_fail_closed(tmp_path) -> None:
    for location in ("document", "artifact"):
        ...
        # Hashes are deliberately irrelevant here: shape validation must reject
        # undeclared authority-bearing coordinates rather than silently ignore them.
        path.write_text(json.dumps(document))
