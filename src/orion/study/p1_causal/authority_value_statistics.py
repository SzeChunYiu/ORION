...
def wilson_interval(k: int, n: int, *, z: float = 1.959963984540054) -> tuple[float, float]:
    ...
    # Avoid platform-dependent positive/negative rounding residue at the two
    # exact binomial boundaries.  The Wilson bound is mathematically zero for
    # k=0 and one for k=n, so preserve those identities byte-for-byte.
    low = 0.0 if k == 0 else max(0.0, center - margin)
    high = 1.0 if k == n else min(1.0, center + margin)
    return (low, high)
