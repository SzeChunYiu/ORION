...
def predict_relation_explicit(task: MechanicRankingTask) -> str:
    """Select from typed relation semantics + declared candidate effects only."""
    ...
    if len(decisive) != 1:
        return AMBIGUOUS
    relation = next(iter(decisive))
    required_effect = {
        "SUPPORTS": "ASSIMILATE_EVIDENCE",
        "DEFEATS": "REOPEN_CLAIM",
    }[relation]
    matches = [
        candidate_id
        for candidate_id, effects in _candidate_effects(task).items()
        if required_effect in effects
    ]
    return matches[0] if len(matches) == 1 else AMBIGUOUS


def predict_history_explicit(task: MechanicRankingTask) -> str:
    """Exclude explicitly failed candidates; otherwise use unique visible min cost."""
...
def run_a2_a4_explicit(
    *,
    seed: str = "p9-a2-a4-d0-explicit-v1",
    relation_pairs: int = 256,
    history_pairs: int = 256,
    ...
    for mode in ViewMode:
        ...
        for index in range(relation_pairs):
            ...
            for world in pair.worlds:
                ...
                if not isinstance(task, MechanicRankingTask):
                    raise TypeError("relation family produced non-ranking task")
                prediction = predict_relation_explicit(task)
                relation_rows.append(
                    {
                        "world_id": world.world_id,
                        "gold": task.target_candidate_id,
        ...
        for index in range(history_pairs):
            ...
            for world in pair.worlds:
                ...
                if not isinstance(task, MechanicRankingTask):
                    raise TypeError("history family produced non-ranking task")
                prediction = predict_history_explicit(task)
        ...
        metrics["hidden_view_pair_prediction_equal"] = (
            all(pair_consistency) if pair_consistency else None
        )
        history_views[mode.value] = metrics

    relation_ok = all(
        relation_views[mode.value]["full_task_accuracy"] == 1.0
        for mode in (ViewMode.TYPED, ViewMode.CURRENT, ViewMode.SEMANTIC)
    )
    relation_weak_ambiguous = all(
    ...
    result: dict[str, object] = {
        "schema": "P9.A2A4D0ExplicitResult.v1",
        "protocol": "P9.A2A4D0ExplicitProtocol.v1",
...
__all__ = [
    "AMBIGUOUS",
    "predict_history_explicit",
    "predict_relation_explicit",
    "run_a2_a4_explicit",
]
