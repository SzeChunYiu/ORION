...
def inspect_paper(repo_root: Path, directory: Path) -> PaperBinding:
    ...
    Reads only committed artifacts --- the digest file and, when present, the
    manifest's ``bound_files`` --- and never derives what the manifest ought to
    say. Deriving it is the generator's job; agreeing with a derivation is not
    the same as agreeing with the bytes.
    """
