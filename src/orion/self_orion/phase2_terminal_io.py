...
def load_failure_replay_receipt(path: Path | str) -> FailureReplayReceipt:
    ...
    entries = tuple(
        FailureReplayEntry(
            ...
            fresh_transfer_artifact_hash=item.get(
                "fresh_transfer_artifact_hash", ""
            ),
            blocking_fibre_artifact_hash=item.get(
                "blocking_fibre_artifact_hash", ""
            ),
    ...
    receipt = FailureReplayReceipt(
        ...
        frozen_failure_index_artifact_hash=raw[
            "frozen_failure_index_artifact_hash"
        ],
...
def load_final_integration_receipt(path: Path | str) -> FinalIntegrationReceipt:
    ...
    receipt = FinalIntegrationReceipt(
        ...
        external_manifest_artifact_hash=raw[
            "external_manifest_artifact_hash"
        ],
        papers_claim_ledger_artifact_hash=raw[
            "papers_claim_ledger_artifact_hash"
        ],
...
def load_repository_subject_attestation(
    ...
    if not isinstance(raw, dict) or raw.get("schema") != SUBJECT_ATTESTATION_SCHEMA:
        raise ValueError(
            f"repository subject schema must be {SUBJECT_ATTESTATION_SCHEMA}"
        )
