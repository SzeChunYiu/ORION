"""Typed similarity and analogy witnesses reused from RAKL.

Donor: SzeChunYiu/RAKL@70f5f7c4a6771ffd1158765b42ac9f8aee8a270f
       src/rakl/similarity.py

No ORION novelty is claimed for this donor mechanic.
"""
...
def validate_similarity_witness(witness: SimilarityWitness) -> WitnessReport:
    ...
    if witness.mapping_admissibility.declared_before_fit is None:
        return WitnessReport(WitnessVerdict.CANNOT_CHECK, ("mapping_family_chronology_unknown",))
    if witness.mapping_admissibility.declared_before_fit is False:
        return WitnessReport(WitnessVerdict.REJECT, ("posthoc_mapping_family_expansion",))
    if witness.mapping_admissibility.null_calibration_passed is None:
        return WitnessReport(WitnessVerdict.CANNOT_CHECK, ("null_calibration_unknown",))
    if witness.mapping_admissibility.null_calibration_passed is False:
        return WitnessReport(WitnessVerdict.REJECT, ("structure_null_not_rejected",))
    if witness.mapping_admissibility.constraint_violations:
        return WitnessReport(
            WitnessVerdict.REJECT,
            tuple(f"mapping_constraint_violation:{item}" for item in witness.mapping_admissibility.constraint_violations),
        )
    if set(witness.preserved).intersection(witness.not_preserved):
        return WitnessReport(WitnessVerdict.REJECT, ("same_structure_marked_preserved_and_not_preserved",))
    ...
    if witness.relation is SimilarityRelation.CAUSALLY_ANALOGOUS and "causal_direction" not in witness.mapping_admissibility.constraints:
        return WitnessReport(WitnessVerdict.CANNOT_CHECK, ("causal_direction_constraint_missing",))
    if witness.relation is SimilarityRelation.MATHEMATICALLY_ISOMORPHIC and "type_or_unit_compatibility" not in witness.mapping_admissibility.constraints:
        return WitnessReport(WitnessVerdict.CANNOT_CHECK, ("type_or_unit_compatibility_constraint_missing",))
    return WitnessReport(
...
def diagnose_analogy_discovery(observation: AnalogyDiscoveryObservation) -> AnalogyDiscoveryReport:
    if observation.hidden_label_exposed is True:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.TRIAL_INVALID, ("hidden_answer_or_relevance_label_exposed",))
    if observation.query_frozen_before_labels is False:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.TRIAL_INVALID, ("query_or_abstraction_modified_after_label_inspection",))
    if observation.hidden_label_exposed is None:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.CANNOT_CHECK, ("hidden_label_exposure_status_unknown",))
    if observation.query_frozen_before_labels is None:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.CANNOT_CHECK, ("query_freeze_chronology_unknown",))
    if not observation.designated_source_id:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.CANNOT_CHECK, ("evaluator_designated_source_missing",))
    source_id = observation.designated_source_id
    if source_id not in observation.corpus_candidate_ids:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.CORPUS_COVERAGE_FAILURE, ("designated_analogue_absent_from_frozen_corpus",))
    if source_id not in observation.retrieved_candidate_ids:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.RETRIEVAL_FAILURE, ("designated_analogue_present_but_not_retrieved",))
    if observation.witness is None:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.RECOGNITION_FAILURE, ("designated_analogue_retrieved_but_witness_missing",))
    if observation.witness.source_id != source_id:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.RECOGNITION_FAILURE, ("witness_points_to_wrong_source",))
    if observation.witness.target_id != observation.target_id:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.RECOGNITION_FAILURE, ("witness_points_to_wrong_target",))
    witness_report = validate_similarity_witness(observation.witness)
    if witness_report.verdict is WitnessVerdict.REJECT:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.RECOGNITION_FAILURE, ("retrieved_candidate_failed_witness_validation",), witness_report)
    if witness_report.verdict is WitnessVerdict.CANNOT_CHECK:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.CANNOT_CHECK, ("witness_validation_incomplete",), witness_report)
    if observation.target_transfer_tested is None:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.CANNOT_CHECK, ("target_transfer_test_status_unknown",), witness_report)
    if observation.target_transfer_tested is False:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.WITNESSED_ANALOGY, ("structural_witness_valid_target_transfer_not_tested",), witness_report)
    if observation.target_transfer_passed is None:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.CANNOT_CHECK, ("target_transfer_outcome_unknown",), witness_report)
    if observation.target_transfer_passed is False:
        return AnalogyDiscoveryReport(AnalogyDiscoveryVerdict.TARGET_TRANSFER_REFUTED, ("target_test_refuted_transfer_structural_witness_preserved",), witness_report)
...
def corroborate_structural_motif(witnesses: Tuple[SimilarityWitness, ...]) -> StructuralMotifReport:
    if len(witnesses) < 2:
        return StructuralMotifReport(MotifVerdict.CANNOT_CHECK, (), frozenset(w.source_domain for w in witnesses), ("at_least_two_witnesses_required",))
    ...
    if any(report.verdict is not WitnessVerdict.VALID for report in reports):
        return StructuralMotifReport(MotifVerdict.CANNOT_CHECK, (), frozenset(w.source_domain for w in witnesses), ("one_or_more_witnesses_not_validated",))
    domains = frozenset(w.source_domain for w in witnesses)
    if len(domains) < 2:
        return StructuralMotifReport(MotifVerdict.CANNOT_CHECK, (), domains, ("cross_domain_confirmation_not_established",))
    common = set(witnesses[0].preserved)
    for witness in witnesses[1:]:
        common.intersection_update(witness.preserved)
    if not common:
        return StructuralMotifReport(MotifVerdict.NO_COMMON_MOTIF, (), domains, ("no_shared_preserved_structure",))
    return StructuralMotifReport(
        MotifVerdict.CORROBORATED_PROPOSAL_ONLY,
        tuple(sorted(common)),
        domains,
        ("shared_structure_found_across_distinct_source_domains", "corroboration_is_not_target_validation"),
    )


__all__ = [
    "AnalogyDiscoveryObservation", "AnalogyDiscoveryReport", "AnalogyDiscoveryVerdict",
    "DistinguishingProbeCertificate", "MappingAdmissibility", "MotifVerdict", "ProbeFamily",
    "SimilarityRelation", "SimilarityWitness", "StructuralMotifReport", "WitnessReport",
    "WitnessVerdict", "corroborate_structural_motif", "diagnose_analogy_discovery",
