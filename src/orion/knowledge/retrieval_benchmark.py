"""RAKL-derived factorized retrieval benchmark.

Provenance: SzeChunYiu/RAKL@70f5f7c4a6771ffd1158765b42ac9f8aee8a270f
``src/rakl/retrieval_benchmark.py``.

This complements ORION's recall-first evaluation harness.  It separates corpus
coverage, conditional retrieval recall, structural-witness ground truth and
transfer-validity ground truth so a successful retrieval metric cannot silently
mint structural or transfer authority.
"""
...
def validate_corpus_artifact(artifact: CorpusArtifactIdentity) -> CorpusArtifactReport:
    missing = tuple(
        f"{name}_missing"
        for name, value in (
    ...
    if artifact.size_bytes <= 0:
        return CorpusArtifactReport(BenchmarkVerdict.REJECT, ("artifact_size_must_be_positive",))
    if artifact.identity_observed is None:
        return CorpusArtifactReport(BenchmarkVerdict.CANNOT_CHECK, ("artifact_identity_observation_unknown",))
    if artifact.identity_observed is False:
        return CorpusArtifactReport(BenchmarkVerdict.CANNOT_CHECK, ("pinned_artifact_identity_not_observed",))
    return CorpusArtifactReport(BenchmarkVerdict.VALID, ("source_revision_and_content_identity_pinned",))


def validate_ground_truth_factorization(factorization: GroundTruthFactorization) -> GroundTruthReport:
    if not factorization.retrieval_relevance_label_id:
        return GroundTruthReport(BenchmarkVerdict.CANNOT_CHECK, (), ("retrieval_relevance_authority_missing",))
    if factorization.labels_collapsed_to_one_authority:
        return GroundTruthReport(BenchmarkVerdict.REJECT, (), ("retrieval_structure_transfer_ground_truth_collapsed",))
    ...
    return GroundTruthReport(BenchmarkVerdict.VALID, tuple(available), tuple(reasons))


def evaluate_retrieval_route_trial(trial: RetrievalRouteTrial) -> RetrievalRouteReport:
    required = (
    ...
    if missing:
        return RetrievalRouteReport(RouteTrialVerdict.CANNOT_CHECK, *empty, missing)
    if trial.top_k <= 0:
        return RetrievalRouteReport(RouteTrialVerdict.TRIAL_INVALID, *empty, ("top_k_must_be_positive",))
    if trial.hidden_labels_exposed is None:
        return RetrievalRouteReport(RouteTrialVerdict.CANNOT_CHECK, *empty, ("hidden_label_exposure_status_unknown",))
    if trial.hidden_labels_exposed:
        return RetrievalRouteReport(RouteTrialVerdict.TRIAL_INVALID, *empty, ("hidden_relevance_labels_exposed",))
    if trial.query_frozen_before_labels is None:
        return RetrievalRouteReport(RouteTrialVerdict.CANNOT_CHECK, *empty, ("query_freeze_chronology_unknown",))
    if trial.query_frozen_before_labels is False:
        return RetrievalRouteReport(RouteTrialVerdict.TRIAL_INVALID, *empty, ("posthoc_query_or_abstraction_modification",))
    if trial.execution_observed is None:
        return RetrievalRouteReport(RouteTrialVerdict.CANNOT_CHECK, *empty, ("execution_observation_unknown",))
    if trial.execution_observed is False:
        return RetrievalRouteReport(RouteTrialVerdict.CANNOT_CHECK, *empty, ("retrieval_route_not_executed",))
    if trial.resource_delta_declared is None:
        return RetrievalRouteReport(RouteTrialVerdict.CANNOT_CHECK, *empty, ("resource_delta_declaration_unknown",))
    if not trial.designated_relevant_ids:
        return RetrievalRouteReport(RouteTrialVerdict.CANNOT_CHECK, *empty, ("evaluator_relevance_labels_missing",))
    if len(trial.retrieved_candidate_ids) > trial.top_k:
        return RetrievalRouteReport(RouteTrialVerdict.TRIAL_INVALID, *empty, ("retrieved_candidate_count_exceeds_top_k",))
    if len(trial.retrieved_candidate_ids) != len(set(trial.retrieved_candidate_ids)):
        return RetrievalRouteReport(RouteTrialVerdict.TRIAL_INVALID, *empty, ("duplicate_candidate_ids_in_ranked_output",))
    ...
    conditional_recall = (
        sum(1 for item in covered if item in trial.retrieved_candidate_ids) / len(covered)
        if covered else None
    )
    rank_lookup = {candidate: index + 1 for index, candidate in enumerate(trial.retrieved_candidate_ids)}
    reciprocal_ranks = [1.0 / rank_lookup[item] if item in rank_lookup else 0.0 for item in covered]
...
def compare_retrieval_routes(trials: Tuple[RetrievalRouteTrial, ...]) -> RouteComparisonReport:
    if len(trials) < 2:
        return RouteComparisonReport(RouteComparisonVerdict.CANNOT_CHECK, tuple(t.route_id for t in trials), ("at_least_two_routes_required",))
    route_ids = tuple(t.route_id for t in trials)
    if len(route_ids) != len(set(route_ids)):
        return RouteComparisonReport(RouteComparisonVerdict.TRIAL_INVALID, route_ids, ("duplicate_route_id",))
    reports = tuple(evaluate_retrieval_route_trial(t) for t in trials)
    if any(r.verdict is RouteTrialVerdict.TRIAL_INVALID for r in reports):
        return RouteComparisonReport(RouteComparisonVerdict.TRIAL_INVALID, route_ids, ("one_or_more_route_trials_invalid",))
    if any(r.verdict is RouteTrialVerdict.CANNOT_CHECK for r in reports):
        return RouteComparisonReport(RouteComparisonVerdict.CANNOT_CHECK, route_ids, ("one_or_more_route_trials_incomplete",))
    reference = trials[0]
    matched_fields = (
        "corpus_id", "corpus_revision", "corpus_artifact_sha", "task_packet_id",
        "query_packet_id", "top_k", "base_model_id", "base_model_config_id",
        "output_contract_id", "evaluator_id", "corpus_candidate_ids",
        "designated_relevant_ids",
    )
    mismatches = []
    for field in matched_fields:
        baseline = getattr(reference, field)
        if any(getattr(t, field) != baseline for t in trials[1:]):
            mismatches.append(f"matched_contract_mismatch:{field}")
    if mismatches:
        return RouteComparisonReport(RouteComparisonVerdict.TRIAL_INVALID, route_ids, tuple(mismatches))
    resource_sets = tuple(frozenset(t.resource_ids) for t in trials)
    resources_differ = any(resources != resource_sets[0] for resources in resource_sets[1:])
    if resources_differ:
        if any(t.resource_delta_declared is not True for t in trials):
            return RouteComparisonReport(RouteComparisonVerdict.TRIAL_INVALID, route_ids, ("undeclared_resource_delta_between_routes",))
        return RouteComparisonReport(
            RouteComparisonVerdict.SYSTEM_LEVEL_WITH_RESOURCE_DELTA,
            route_ids,
            ("matched_nonresource_contract", "resource_delta_explicit", "comparison_cannot_establish_pure_model_utilization_gain"),
        )
    return RouteComparisonReport(
        RouteComparisonVerdict.MATCHED_SAME_RESOURCE,
        route_ids,
        ("matched_corpus_task_query_model_topk_and_evaluator", "same_resource_set", "no_automatic_route_activation"),
    )


__all__ = [
    "BenchmarkVerdict", "CorpusArtifactIdentity", "CorpusArtifactReport",
    "GroundTruthFactorization", "GroundTruthLayer", "GroundTruthReport",
    "RetrievalRouteReport", "RetrievalRouteTrial", "RouteComparisonReport",
    "RouteComparisonVerdict", "RouteTrialVerdict", "compare_retrieval_routes",
    "evaluate_retrieval_route_trial", "validate_corpus_artifact",
