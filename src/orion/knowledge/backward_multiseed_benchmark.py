"""Frozen hidden-path benchmark for backward/multi-seed search.

Reused from RAKL issue #141 at
SzeChunYiu/RAKL@70f5f7c4a6771ffd1158765b42ac9f8aee8a270f.

The benchmark is intentionally evaluator-side and observation-only.  It cannot
generate arm outcomes, cannot activate a search arm, and cannot establish a
scientific capability claim.  Contamination ceilings are hard admissibility
gates and dominate solve rate.
"""
...
class MatchedBudget:
    ...
    def __post_init__(self) -> None:
        if any(value < 1 for value in (self.max_expansions, self.max_connection_tests, self.max_tool_calls, self.max_tokens)):
            raise ValueError("matched budget limits must be positive")
        if self.max_wall_clock_seconds <= 0:
            raise ValueError("matched wall-clock budget must be positive")
...
class HiddenPathCase:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.frozen_evidence_ids:
            raise ValueError("hidden-path cases require frozen evidence")
...
class TaskArmObservation:
    ...
    def __post_init__(self) -> None:
        ...
        if self.root_obligations_verified_closed > self.root_obligations_declared:
            raise ValueError("closed root obligations cannot exceed declared obligations")
        if self.root_progress_claims_upheld > self.root_progress_claims:
            raise ValueError("upheld root progress cannot exceed progress claims")
        if self.connection_tests_confirmed_by_checker > self.connection_tests_claimed_verified:
            raise ValueError("confirmed connections cannot exceed verified claims")
        if self.waypoints_used_in_verified_connection + self.waypoints_judged_irrelevant_by_evaluator > self.waypoints_emitted:
            raise ValueError("classified waypoints cannot exceed emitted waypoints")
...
class FrozenBenchmarkSpec:
    ...
    def __post_init__(self) -> None:
        if not self.benchmark_id or not self.cases:
            raise ValueError("benchmark identity and cases are required")
        if len({case.case_id for case in self.cases}) != len(self.cases):
            raise ValueError("case ids must be unique")
        if len(self.arms) < 2 or len(set(self.arms)) != len(self.arms):
            raise ValueError("benchmark requires at least two unique arms")
    ...
    def spec_hash(self) -> str:
        payload = "|".join((
            ...
            f"{self.budget.max_expansions}:{self.budget.max_connection_tests}:{self.budget.max_tool_calls}:{self.budget.max_tokens}:{self.budget.max_wall_clock_seconds}",
            f"{self.ceilings.max_false_root_progress_rate}:{self.ceilings.min_connection_test_precision}:{self.ceilings.max_repeated_known_failure_rate}",
...
def evaluate_arm(spec: FrozenBenchmarkSpec, observations: Iterable[TaskArmObservation], arm: SearchArm) -> ArmReport:
    ...
    rows = tuple(row for row in observations if row.arm is arm and row.benchmark_id == spec.benchmark_id)
    if not rows:
        return ArmReport(ArmVerdict.INSUFFICIENT_HISTORY, None, ("no_observations_supplied_for_arm", "harness_does_not_generate_arm_results"))
    if len(rows) < spec.minimum_cases_per_arm:
        return ArmReport(ArmVerdict.INSUFFICIENT_HISTORY, None, (f"observed_cases:{len(rows)}", f"required_cases:{spec.minimum_cases_per_arm}"))
    ...
    if unknown:
        return ArmReport(ArmVerdict.TRIAL_INVALID, None, ("observation_case_not_in_frozen_spec:" + ",".join(unknown),))
    ...
    for row in rows:
        ...
        for condition, reason in (
            (case.evaluator_knows_solution is None, f"evaluator_solution_status_unknown:{row.case_id}"),
            (case.gold_path_hidden_from_solver is None, f"gold_path_hidden_status_unknown:{row.case_id}"),
            (case.evaluator_separate_from_solver is None, f"evaluator_separation_unknown:{row.case_id}"),
            (row.gold_path_exposed is None, f"gold_path_exposure_unknown:{row.case_id}"),
            (row.budget_frozen_before_run is None, f"budget_freeze_chronology_unknown:{row.case_id}"),
            (row.verified_solved is None, f"solve_outcome_unknown:{row.case_id}"),
        ):
            if condition:
                integrity_unknown.append(reason)
        ...
        if case.evaluator_separate_from_solver is False:
            invalid.append(f"evaluator_not_separate:{row.case_id}")
        if row.budget_frozen_before_run is False:
            invalid.append(f"budget_not_frozen_before_run:{row.case_id}")
        for used, maximum, label in (
            (row.expansions_used, spec.budget.max_expansions, "expansions"),
            (row.connection_tests_run, spec.budget.max_connection_tests, "connection_tests"),
            (row.tool_calls_used, spec.budget.max_tool_calls, "tool_calls"),
            (row.tokens_used, spec.budget.max_tokens, "tokens"),
            (row.wall_clock_seconds, spec.budget.max_wall_clock_seconds, "wall_clock"),
        ):
            if used > maximum:
                invalid.append(f"budget_violation_{label}:{row.case_id}")
    ...
    overlaps = tuple(value for value in (_overlap(row.forward_frontier_signature, row.backward_frontier_signature) for row in rows) if value is not None)
    ...
    metrics = ArmMetrics(
        ...
        forward_backward_frontier_overlap=sum(overlaps) / len(overlaps) if overlaps else None,
        false_root_progress_rate=_rate(claims - upheld, claims),
        repeated_known_failure_rate=_rate(retries, tests),
        representation_diversity=sum(row.distinct_representations_used for row in rows) / len(rows),
        expansions=sum(row.expansions_used for row in rows),
        tool_calls=tool_calls,
        tokens=sum(row.tokens_used for row in rows),
        wall_clock_seconds=sum(row.wall_clock_seconds for row in rows),
        verification_debt=sum(row.verification_debt for row in rows),
        cost_per_verified_solve=tool_calls / solved if solved else float("inf"),
    )
    contaminated: list[str] = []
    if metrics.false_root_progress_rate is not None and metrics.false_root_progress_rate > spec.ceilings.max_false_root_progress_rate:
        contaminated.append("LOCAL_WAYPOINT_AS_ROOT_PROGRESS:false_root_progress_above_ceiling")
    if metrics.connection_test_precision is not None and metrics.connection_test_precision < spec.ceilings.min_connection_test_precision:
        contaminated.append("UNVERIFIED_CONNECTION_AS_ROAD:connection_precision_below_floor")
    if metrics.repeated_known_failure_rate is not None and metrics.repeated_known_failure_rate > spec.ceilings.max_repeated_known_failure_rate:
    ...
    return ArmReport(ArmVerdict.ARM_ADMISSIBLE, metrics, ("matched_budget_respected", "gold_path_hidden_and_unexposed", "contamination_ceilings_respected", "arm_metrics_are_observations_not_capability_claims"))


def _ranking_key(report: ArmReport):
    if report.metrics is None:
        return (2, 0.0, 1.0, float("inf"), 1.0, "")
    metrics = report.metrics
    return (
        0 if report.admissible else 1,
        -metrics.verified_solve_rate,
        metrics.false_root_progress_rate or 0.0,
        metrics.cost_per_verified_solve,
        metrics.irrelevant_waypoint_rate or 0.0,
        metrics.arm.value,
    )
...
def compare_arms(spec: FrozenBenchmarkSpec, observations: Iterable[TaskArmObservation], *, baseline_arm: SearchArm = SearchArm.A_FORWARD_ONLY) -> ArmComparisonReport:
    ...
    return ArmComparisonReport(spec.benchmark_id, spec.spec_hash, ranked, baseline_arm, tuple(reasons))


__all__ = [
    "ArmComparisonReport", "ArmMetrics", "ArmReport", "ArmVerdict",
    "ContaminationCeilings", "FrozenBenchmarkSpec", "HiddenPathCase",
    "HiddenPathTaskFamily", "MatchedBudget", "SearchArm", "TaskArmObservation",
    "compare_arms", "evaluate_arm",
]
