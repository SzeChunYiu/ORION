"""Measurement-aware relation mechanics reused from RAKL.

Donor: SzeChunYiu/RAKL@70f5f7c4a6771ffd1158765b42ac9f8aee8a270f
       src/rakl/measurement.py

This keeps measurand, feature, unit, measurement model, observation operator,
procedure, instrument, calibration, traceability, uncertainty and resolution
separate before any SAME_OBSERVABLE / OBSERVATIONALLY_EQUIVALENT claim.
"""
...
def _traceability_status(
    spec: MeasurementSpecification, prefix: str
...
def evaluate_measurement_relation(
    trial: MeasurementRelationTrial,
    ...
    missing = list(_validate_spec("source", trial.source)) + list(
        _validate_spec("target", trial.target)
    )
    ...
    if missing:
        return MeasurementRelationReport(
            MeasurementRelationVerdict.CANNOT_CHECK, tuple(missing)
        )
    ...
    if (
        trial.witness.source_id != trial.source.observation_id
        or trial.witness.target_id != trial.target.observation_id
    ):
        return MeasurementRelationReport(
            MeasurementRelationVerdict.REJECT,
    ...
    if trial.source.measurand_id != trial.target.measurand_id:
        if trial.mapping.measurand_mapping_valid is False:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.REJECT, ("measurand_mismatch",)
            )
        if trial.mapping.measurand_mapping_valid is None:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.CANNOT_CHECK,
                ("measurand_mapping_unknown",),
            )
    if trial.source.feature_of_interest_id != trial.target.feature_of_interest_id:
        if trial.mapping.feature_mapping_valid is False:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.REJECT,
                ("feature_of_interest_mismatch",),
            )
        if trial.mapping.feature_mapping_valid is None:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.CANNOT_CHECK,
                ("feature_mapping_unknown",),
            )
    if trial.source.unit_id != trial.target.unit_id:
        if not trial.mapping.unit_transform_id:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.CANNOT_CHECK,
                ("unit_transform_missing",),
            )
        if trial.mapping.unit_transform_invertible is False:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.REJECT,
                ("unit_transform_not_invertible",),
            )
        if trial.mapping.unit_transform_invertible is None:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.CANNOT_CHECK,
                ("unit_transform_invertibility_unknown",),
            )
    if not set(trial.source.regime).intersection(trial.target.regime):
        return MeasurementRelationReport(
            MeasurementRelationVerdict.REJECT, ("measurement_regime_disjoint",)
        )
    if not set(trial.source.phenomenon_scope).intersection(
        trial.target.phenomenon_scope
    ):
        return MeasurementRelationReport(
            MeasurementRelationVerdict.REJECT, ("phenomenon_scope_disjoint",)
        )
    if trial.source.measurement_model_id != trial.target.measurement_model_id:
        if trial.mapping.measurement_model_compatibility is False:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.REJECT,
                ("measurement_model_mismatch",),
            )
        if trial.mapping.measurement_model_compatibility is None:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.CANNOT_CHECK,
                ("measurement_model_compatibility_unknown",),
            )
    if trial.source.observation_operator_id != trial.target.observation_operator_id:
        if trial.mapping.observation_operator_compatibility is False:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.REJECT,
                ("observation_operator_mismatch",),
            )
        if trial.mapping.observation_operator_compatibility is None:
            return MeasurementRelationReport(
                MeasurementRelationVerdict.CANNOT_CHECK,
                ("observation_operator_compatibility_unknown",),
            )

    source_reason, source_trace = _traceability_status(trial.source, "source")
    target_reason, target_trace = _traceability_status(trial.target, "target")
    if source_trace is False or target_trace is False:
        return MeasurementRelationReport(
            ...
            tuple(
                reason
                for reason, state in (
                    (source_reason, source_trace),
                    (target_reason, target_trace),
                )
                if state is False
            ),
        )
    ...
    if (
        trial.source.traceability_required
        and trial.target.traceability_required
        and trial.source.traceability_reference_id
        != trial.target.traceability_reference_id
    ):
        return MeasurementRelationReport(
            MeasurementRelationVerdict.REJECT,
    ...
    if (
        trial.source.standard_uncertainty is None
        or trial.target.standard_uncertainty is None
    ):
        return MeasurementRelationReport(
            MeasurementRelationVerdict.CANNOT_CHECK,
    ...
    if (
        trial.source.unit_id != trial.target.unit_id
        and trial.mapping.probes_in_common_coordinate is not True
    ):
        return MeasurementRelationReport(
            MeasurementRelationVerdict.CANNOT_CHECK,
    ...
    if (
        combined_uncertainty > tolerance
        or max(trial.source.resolution, trial.target.resolution) > tolerance
    ):
        return MeasurementRelationReport(
            MeasurementRelationVerdict.CANNOT_CHECK,
