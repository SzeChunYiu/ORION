...
def _lineages(hops: Tuple[BridgeHop, ...]) -> tuple[Tuple[str, ...], bool]:
    values = [value for hop in hops for value in hop.evidence_lineage_ids]
    return tuple(sorted(set(values))), len(values) != len(set(values))


def evaluate_bridge_path(path: BridgePath) -> BridgePathReport:
    """Port of RAKL bridge composition: navigation/transfer never mints endpoint equivalence."""
    ...
    for index, hop in enumerate(path.hops):
        if hop.witness.question_or_qoi != path.question_or_qoi:
            return _r(BridgePathVerdict.REJECT, (f"question_or_qoi_drift_at_hop:{index}",), hop_reports=hop_reports)
    ...
    for index, handoff in enumerate(path.handoffs):
        left, right = path.hops[index].witness, path.hops[index + 1].witness
        if left.target_id != right.source_id:
            return _r(BridgePathVerdict.REJECT, (f"intermediate_object_identity_mismatch:{index}",), hop_reports=hop_reports)
        if handoff.junction_id != left.target_id:
            return _r(BridgePathVerdict.REJECT, (f"handoff_junction_identity_mismatch:{index}",), hop_reports=hop_reports)
        if not handoff.role_pairs or not handoff.evidence_ids:
            return _r(BridgePathVerdict.CANNOT_CHECK, (f"handoff_contract_incomplete:{index}",), hop_reports=hop_reports)
        left_roles = {target for _, target in left.mapping_pairs}
        right_roles = {source for source, _ in right.mapping_pairs}
        for delivered, consumed in handoff.role_pairs:
            if delivered not in left_roles:
                return _r(BridgePathVerdict.REJECT, (f"handoff_left_role_not_delivered_by_prior_hop:{index}:{delivered}",), hop_reports=hop_reports)
            if consumed not in right_roles:
                return _r(BridgePathVerdict.REJECT, (f"handoff_right_role_not_consumed_by_next_hop:{index}:{consumed}",), hop_reports=hop_reports)
        if handoff.compatibility_passed is None:
            return _r(BridgePathVerdict.CANNOT_CHECK, (f"handoff_role_compatibility_unknown:{index}",), hop_reports=hop_reports)
        if handoff.compatibility_passed is False:
            return _r(BridgePathVerdict.REJECT, (f"handoff_role_compatibility_failed:{index}",), hop_reports=hop_reports)
    ...
    if not path.claimed_end_to_end_invariants:
        return _r(
            BridgePathVerdict.NAVIGABLE_ONLY,
            ("locally_witnessed_path_has_no_declared_end_to_end_invariant", "bridge_to_is_navigation_not_equivalence"),
            **base,
        )
    for invariant in path.claimed_end_to_end_invariants:
        for index, hop in enumerate(path.hops):
            if invariant in hop.witness.not_preserved:
                return _r(BridgePathVerdict.REJECT, (f"carried_invariant_explicitly_broken:{invariant}:hop:{index}",), **base)
            if invariant not in hop.witness.preserved:
                return _r(BridgePathVerdict.CANNOT_CHECK, (f"carried_invariant_unresolved:{invariant}:hop:{index}",), **base)
    ...
    semantics = tuple(hop.error_semantics_id for hop in path.hops)
    if any(not semantics_id for semantics_id in semantics):
        return _r(BridgePathVerdict.CANNOT_CHECK, ("one_or_more_hops_missing_error_semantics",), **base)
    if any(semantics_id != rule.error_semantics_id for semantics_id in semantics):
    ...
    for index, hop in enumerate(path.hops):
        ...
        if value is None:
            return _r(BridgePathVerdict.CANNOT_CHECK, (f"hop_error_bound_unknown:{index}",), **base)
        if value < 0:
            return _r(BridgePathVerdict.REJECT, (f"negative_hop_error_bound:{index}",), **base)
        errors.append(value)
    ...
    reasons = [
        ...
        "error_semantics_match_predeclared_composition_rule",
        "certified_accumulated_error_within_frozen_tolerance",
