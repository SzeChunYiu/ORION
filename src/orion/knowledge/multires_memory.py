...
def validate_memory_view(view_id: str, views: Iterable[MemoryView]) -> MemoryViewReport:
    """Validate reconstructable-memory metadata without minting new authority.

    Reused from RAKL@70f5f7c4. Derived views are caches/representations; they may
    preserve source authority but cannot strengthen it, and lossy erasures stay explicit.
    """
    ...
    def visit(current_id: str) -> None:
        ...
        if current.kind == MemoryViewKind.CANONICAL:
            ...
            for pin in sorted(current.source_pins, key=lambda item: item.record_id):
                ...
                visit(pin.record_id)
            for certificate in sorted(set(current.authority_certificates) - direct_authority):
    ...
    visit(view_id)
    for record_id in sorted(set(target.required_canonical_ids) - roots):
        issues.add(f"required_canonical_unreachable:{record_id}")
    if issues:
        return MemoryViewReport(view_id, MemoryViewVerdict.INVALID, tuple(sorted(roots)), tuple(order), tuple(sorted(issues)))
    ...
    return MemoryViewReport(view_id, verdict, tuple(sorted(roots)), tuple(order), ())


def canonical_source_closure(view_id: str, views: Iterable[MemoryView]) -> tuple[str, ...]:
