"""Bounded epistemic context compiler reused from RAKL.

Donor: SzeChunYiu/RAKL@70f5f7c4a6771ffd1158765b42ac9f8aee8a270f
       src/rakl/context_compiler.py

The algorithm and fail-closed H30 semantics are intentionally preserved. ORION
renames only the owning package; it does not claim this mechanic as a new ORION
invention.
"""
...
class ContextItem:
    """A materializable view over canonical memory.
...
def compile_epistemic_context(
    ...
    7. If relevant, coverage-bearing context exists and the budget admits NONE of
       ...
       empty working set reported as compiled (RAKL HOSTILE_TEST_MATRIX H30). An
    ...
    mandatory = tuple(
        sorted((item for item in pool if item.mandatory), key=lambda x: x.record_id)
    )
    mandatory_cost = sum(item.token_cost for item in mandatory)

    if mandatory_cost > request.budget_tokens:
        mandatory_ids = {m.record_id for m in mandatory}
        omitted = tuple(
            sorted(item.record_id for item in pool if item.record_id not in mandatory_ids)
        )
        mandatory_atoms = frozenset(
            atom for item in mandatory for atom in item.coverage_atoms
        )
        missing = tuple(
            sorted(set(request.required_coverage_atoms) - mandatory_atoms)
        )
        return ContextCompileReport(
            ...
            rehydration_record_ids=tuple(
                sorted({source for item in mandatory for source in item.source_record_ids})
            ),
    ...
    while True:
        fitting = [
            ...
            if used + item.token_cost <= request.budget_tokens
        ]
    ...
    missing_required = tuple(
        sorted(set(request.required_coverage_atoms) - set(covered))
    )
    all_ids = set(by_id)
    ...
    if missing_required:
        ...
        excluded_by_budget = [
            item
            for item in candidates
            if _marginal_value(item, covered, weights) > 0
            and used + item.token_cost > request.budget_tokens
        ]
