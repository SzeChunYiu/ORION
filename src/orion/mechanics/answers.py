...
def _resolve_supersession(
    ...
    if len(roots) != 1:
        return None, ()
    successor_counts = {
        identifier: sum(item.supersedes == identifier for item in group)
...
def _evidence_residual(
    ...
    if any(reference not in evidence_records for reference in record.evidence_refs):
        return (
            AnswerResidualKind.EVIDENCE_UNRESOLVED,
            "every evidence reference must resolve in the host-owned evidence index",
        )
    for reference in record.evidence_refs:
        ...
        if resolved.evidence_id != reference:
            return (
                AnswerResidualKind.EVIDENCE_UNRESOLVED,
                f"evidence index key does not match the resolved record identity: {reference}",
            )
        actual = evidence_record_fingerprint(resolved)
        if actual != expected:
            return (
                AnswerResidualKind.EVIDENCE_MISMATCH,
                f"evidence content does not match the declared digest: {reference}",
            )
    return None
...
def apply_answer_records(
    ...
    *,
    ...
    record_id_counts = {
        record_id: sum(record.record_id == record_id for record in records)
        ...
        if record_id_counts[record.record_id] > 1:
            residuals.append(
                AnswerResidual(
                    ...
                    record.dimension,
                    (record.record_id,),
                    "answer record ids must be globally unique",
                )
            )
            continue
    ...
    for (mechanic_id, dimension), group in sorted(
        ...
        if tip.waiver_reason:
            residuals.append(
                AnswerResidual(
                    AnswerResidualKind.UNAUTHORIZED_WAIVER,
                    tip.mechanic_id,
                    tip.dimension,
                    (tip.record_id,),
                    "V0 has no protected waiver authority path; waiver proposals cannot close questions",
                )
            )
            continue
        ...
        applied_evidence_bindings.extend(
            (tip.record_id, reference, digest)
            for reference, digest in tip.evidence_bindings
        )
    ...
    report = AnswerApplicationReport(
        ...
        evidence_bindings=tuple(applied_evidence_bindings),
    )
    ...
    if report.waiver_record_ids:
        residuals.append(
            AnswerResidual(
                ...
                "*",
                None,
                report.waiver_record_ids,
                "waiver-driven closure is not licensed without a protected authority path",
            )
        )
            ...
            records.append(
                AnswerRecord(
                    ...
                    evidence_bindings=tuple(
                        sorted(raw.get("evidence_bindings", {}).items())
                    ),
                    payload=tuple(
