...
Repeated failure is evidence of recurrence, not proof of cause. Multiple atomic observations from one root run are not independent recurrence. Candidate construction, registration, receipt minting and assessment all recheck independent root runs and the exact variation manifest; `FailurePatternCandidate` cannot self-assert promoted authority. A same-context pattern remains `CANDIDATE`. A guard becomes `VERIFIED_LOCAL` only after a distinct replay run matches a supporting task/problem/variation and records an actually invoked host-installed action `guard:<pattern_id>`. Conditional reuse additionally requires fresh run, task, split, evaluation and variation identities plus an independent protected verification receipt. The receipt is Ed25519-verified against a host-pinned public-key trust store and binds the exact candidate hash and complete support/replay/transfer episode contents. Process and evaluator independence are computed from content hashes, while evidence independence is compared with a lineage derived from canonical evidence-record hashes. An unrelated successful episode, caller-supplied independence boolean, forged receipt, same-ID evidence substitution or unprotected assessment call cannot promote the guard. A failed fresh transfer contradicts or narrows the lesson instead of being averaged away.
