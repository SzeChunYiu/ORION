...
class _AuthorityEscalationError(ValueError):
    pass
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            ...
            if key in _FORBIDDEN_AUTHORITY_TRUE and item is True:
                raise _AuthorityEscalationError(
                    f"capability attempted authority escalation: {key}"
                )
...
def run_campaign_cycle(
    ...
    *,
    ...
    except _AuthorityEscalationError:
        raise
