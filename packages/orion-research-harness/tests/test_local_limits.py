...
def test_process_timeout_uses_one_bounded_ceiling_with_legacy_env_compatibility(
    ...
    assert local_tools._validated_process_timeout(1_500) == 1_500
    ...
    assert local_tools._validated_process_timeout(9999) == 1_500
...
def test_descendant_holding_output_pipe_cannot_hang_completed_process(tmp_path: Path):
    ...
    result = local_tools.execute_local(workspace, "PYTHON", {"code": code, "timeout": 5})
...
def test_process_output_budget_can_be_raised_explicitly_but_stays_bounded(tmp_path: Path):
    project = tmp_path / "project"
    project.mkdir()
    workspace = ResearchWorkspace.initialize(
        project / "ws", project_root=project, allow_process_tools=True
    ...
    result = local_tools.execute_local(
        ...
        "PYTHON",
        {
            "code": "import sys; sys.stdout.write('x' * 150000)",
    ...
    with pytest.raises(ValueError, match="max_output_bytes"):
        local_tools.execute_local(
            workspace,
            "PYTHON",
            {"code": "print('blocked')", "max_output_bytes": 1_000_001},
        )
