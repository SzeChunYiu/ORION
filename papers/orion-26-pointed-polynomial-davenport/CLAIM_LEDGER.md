...
| 22 | Every dual `Q` with `Q(0) ≠ 0` is supported in `S`, minus the involution-pair of the third special length | `verify_dual_support_v5.py` (`p = 11,13,17,19,23`, all pairs, every basis dual) | **verified, not proved** — this single claim is the whole remaining gap in Observation D |
