...
TAU = 1.0              # V1 literal "expected < one new relevant" (kept for the control arm)
TAU_REL = 0.1          # V2: threshold = TAU_REL * dev relevance density (frozen in PROTOCOL_V2.md)
SPLIT_SEED = 20260830
...
def run_corpus(name, log):
    ...
    # ---- V2: dev relevance density (mean relevant documents per dev query) ----
    d_dev = sum(len(rel(q)) for q in dev) / max(len(dev), 1)
    stop_threshold = TAU_REL * d_dev
    ...
    out = {"corpus": name, "docs": len(corpus), "queries_scored": len(qids),
           ...
           "nonempty_titles": title_nonempty, "dev_marginals": marg,
           "d_dev_relevance_density": round(d_dev, 4), "stop_threshold": round(stop_threshold, 4),
           "by_depth": {}}
                ...
                if arm == "best_single":
                    ...
                    for i, r in enumerate(ROUTE_ORDER):
                        if arm == "route_aware_stop" and i > 0:
                            ...
                            if rem and max(rem.values()) < stop_threshold:
                                break
...
def main():
    logf = open(os.path.expanduser("~/beir/run_v2.log"), "a", buffering=1)
    def log(m):
    ...
    res = {"schema": "ORION12.BEIR_ROUTE_AWARE_STOPPING_DENSITY.v2",
           "zip_sha256": digests, "route_order": ROUTE_ORDER, "depths": DEPTHS,
           "rrf_k": RRF_K, "patience": PATIENCE, "tau": TAU, "tau_rel": TAU_REL, "split_seed": SPLIT_SEED,
           "corpora": {}}
    for c in CORPORA:
        res["corpora"][c] = run_corpus(c, log)
        with open(os.path.expanduser("~/beir/RESULTS_V2_DENSITY.json"), "w") as fh:
