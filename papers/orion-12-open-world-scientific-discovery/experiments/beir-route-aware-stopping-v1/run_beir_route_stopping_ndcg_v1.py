...
def run_corpus(name, log):
    ...
    def ndcg10(ex, g):
        dcg = sum((1.0 if x in g else 0.0)/__import__('math').log2(i+2) for i, x in enumerate(ex[:10]))
        idl = sum(1.0/__import__('math').log2(i+2) for i in range(min(10, len(g))))
...
def main():
    logf = open(os.path.expanduser("~/beir/run_ndcg.log"), "a", buffering=1)
    def log(m):
    ...
    res = {"schema": "ORION12.BEIR_ROUTE_AWARE_STOPPING.v1",
    ...
    for c in CORPORA:
        ...
        with open(os.path.expanduser("~/beir/RESULTS_V1_NDCG.json"), "w") as fh:
