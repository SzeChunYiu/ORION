...
1. **Route-aware coverage state.** Acquisition routes, route dependence, availability, exhaustion, and unresolved obligations are represented explicitly rather than inferred from ranking confidence.
2. **Fail-closed stopping semantics.** Completeness is withheld when route independence or unseen-population assumptions are not identified.
3. **A mixed external test.** TREC-COVID shows that early ranking can improve while the preregistered recall-and-cost discovery gate fails, making the distinction between ranking quality and authority to stop empirically visible.

The external failure is part of the result. The paper does not claim state-of-the-art retrieval, lower reading cost, or general discovery superiority.

## 2. Route-aware discovery state

### 2.1 Routes are acquisition mechanisms, not labels

A discovery episode maintains candidate acquisition routes. A route can correspond to a provider, citation expansion, restricted collection, or another materially distinct acquisition mechanism. Different names do not establish independence. If two nominal routes share a backend, seed, or censoring mechanism that makes their observations dependent, that dependence remains part of the search state.
...
Structured evidence memory is not claimed as a new primitive. Its role here is to bind evidence to the search decision whose completeness is being evaluated.
...
The stopping diagnostic therefore returns an unresolved terminal when material routes remain unavailable, route dependence cannot be established, or an estimator lacks information required by its assumptions. A budget limit or provider failure is never silently recoded as evidence that additional relevant literature does not exist.
...
These studies establish mechanism semantics. They do not establish field performance and are not substituted for the external benchmark.
...
The route-aware strategy improves nDCG@10 by 0.1488 relative to the matched comparator. The bootstrap 95% interval is 0.1010 to 0.1995, and the route-aware method is ahead on 42 of 50 topics.
...
The method may therefore be appropriate for settings where explicit coverage accounting matters more than minimizing reads, but that deployment tradeoff is not evaluated by the present benchmark.

## 6. Route dependence and fail-closed population reasoning

Open-world stopping rules often rely on overlap or diminishing discovery across routes. Such estimators are assumption-dependent. Shared providers, duplicated content, route censoring, and zero overlap can make the unseen population weakly identified or non-identifying.

The framework does not solve those identification problems by assertion. Route dependence is recorded explicitly and a closure estimate is withheld when its assumptions are not supported. This policy can increase search cost, as the external study demonstrates. The cost is part of the scientific result rather than an implementation inconvenience to hide.
...
Its residual contribution is the control interface among them: route identity and dependence are explicit; task completion and coverage closure are separate; stopping claims require evidence appropriate to their assumptions; and inaccessible routes remain unresolved. This makes the work a methods and critical system-design contribution rather than a ranking-superiority paper.
...
Coverage estimators remain assumption-dependent, and fail-closed behavior may be unnecessarily expensive in well-defined closed collections. The study also stops at discovery mechanics: it does not establish that route-aware search improves the truth, completeness, or calibration of downstream scientific synthesis.

A new multi-dataset successor would require a fresh identity and cannot retroactively repair the failed TREC-COVID terminal.
...
Reviewer-facing release surfaces should preserve the adverse preregistered gate and maintain blind/named separation where required. The journal package must be rebuilt against the current manuscript bytes; a clean PDF of an older surface is not a current submission package.
