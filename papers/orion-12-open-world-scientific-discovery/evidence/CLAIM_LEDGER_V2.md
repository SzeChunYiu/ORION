...
| V2-C7 | Source-grounded discovery morphology and failure-atlas records may be used as structure-conditioned acquisition/design evidence without becoming protected confirmatory gold or task-global closure authority. | Merged #506/#509 source-bound atlases; actual manuscript interface; authority boundary | `SUPPORTED_GOVERNANCE_BOUNDARY` — 51-episode morphology basis is frozen for descriptive use; 30-episode failure atlas reaches schema-stable only; neither is a ORION-12 retrieval/closure performance result. |

## Scientific-ascent addendum: new analytic identities, no empirical promotion

The 2026-08-23 scientific-ascent revision introduces a theory above the prior
acquisition/closure boundary.  These are new theorem and prospective-hypothesis
identities; they do not rename a historical empirical terminal and do not change
the authorized manuscript terminal `P2_NARROWED`.

| Identity | Claim | Authority and evidence |
|---|---|---|
| `ORION-12-T-ACQUISITION-CEILING` | Conditional on a frozen acquired pool, no downstream selector that cannot invent evidence can submit more gold/decision-relevant evidence than the pool contains. | `ANALYTIC_PROVED_IN_MANUSCRIPT`; set-inclusion proof in `manuscript/sections/acquisition_authority-envelope.tex`. |
| `ORION-12-P-CALLS-ARE-NOT-EXPOSURE` | Equal request counts alone impose no nontrivial bound on the difference between two arms' acquisition ceilings. | `ANALYTIC_PROVED_IN_MANUSCRIPT`; constructive counterexample. The historical one-versus-three admissible-route exposure is an instance, not the proof's basis. |
| `ORION-12-T-CLOSURE-FACTORIZATION` | A donor product can implement the contract-correct closure decision exactly iff closure truth is constant on each fibre of its retained observable state. | `ANALYTIC_PROVED_IN_MANUSCRIPT`; elementary factorization proof. ORION-12-X B3's 400/400 tie is a compatible exact-contract instance, not the proof's basis. |
| `ORION-12-T-INDISTINGUISHABLE-WORLDS` | False closure plus valid-closure shortfall is at least one minus total variation between the closable/open transcript laws. | `ANALYTIC_PROVED_IN_MANUSCRIPT`; binary-testing lower bound. It does not assert that any external world pair has been measured. |
| `ORION-12-T-MAXIMAL-AUTHORITY-COMPLETION` | Under the declared material-obligation and adversarial-realizability contract, closing exactly after all registered material obligations are discharged is maximally permissive among universally sound closure rules. | `ANALYTIC_CONTRACT_RELATIVE`; proof in the new theory section. It does not prove that finite open-web search enumerates every obligation. |
| `ORION-12-E-7-OF-229-EXPOSURE` | In the archived adverse external capture, all governed routes emitted 7/229 gold identifiers; therefore run-conditional recall could not exceed 0.030568 and 222/229 misses were upstream of selection. | `SUPPORTED_HISTORICAL_ADVERSE_DIAGNOSIS`; `evidence/successor_results/P2_ACQUISITION_CEILING_DIAGNOSIS_2026-08-22.{json,md}`. The associated scientific terminal remains adverse/non-promoted. |
| `ORION-12-H-ODAE-ENVELOPE` | Obligation-directed adaptive exploration can improve the guarded discovery/utility/optionality/closure frontier by valuing unresolved-obligation reduction and closure-fibre separation under one charged budget. | `PROSPECTIVE_UNTESTED_MECHANISM_FAMILY`; analytic policy definition only. Generic value-of-information, active search, coverage and adaptive stopping remain donor-owned. |
| `ORION-12-H-ENVELOPE-768` | A controller can exceed the guarded donor-complete acquisition--authority envelope across four source-disjoint arenas under matched admissible exposure. | `PROSPECTIVE_UNTESTED`; frozen shape only in `protocol/P2_TASK_WORLD_SUCCESSOR_V2.json`: 768 independent clusters, three primary comparators, intersection across contrasts, joint target power 0.90. Required external bindings and protected outcomes do not exist. |

The new theory treats all strong retrieval, coverage, stopping, obligation and
memory mechanisms as donor-composable.  It cannot be cited as evidence that the
...
The manuscript's structure-conditioned/source-grounded successor section is a governance/interface writeback, not a new ORION-12 empirical campaign. The abstract and existing Results were reviewed after #598 and are intentionally unchanged: inserting Jump, atom or atlas outcome numbers there would falsely make those studies ORION-12 performance evidence. The actual manuscript interface instead records the owner-relevant consequences: historical discovery/failure episodes remain source-bound design evidence; private cognition stays non-inferable; the 51-episode morphology basis is descriptive; the 30-episode failure atlas is schema-stable but not independently corpus-ready; and none can erase an unresolved acquisition obligation or become protected gold by curation.

At the time of that interface audit, the then-existing Limitations and Conclusion
remained valid because their claim ceiling was stricter than the successor
objects.  The 2026-08-23 scientific-ascent revision replaces that prose with the
acquisition--authority-envelope theory while retaining the same empirical
boundary: ORION-12-X is exact-contract evidence only, external retrieval/provider
generality remains bounded or `CANNOT_CHECK`, and no successor result is
promoted above `P2_NARROWED` by either writeback.
