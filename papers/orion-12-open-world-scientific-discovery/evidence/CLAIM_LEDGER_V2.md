...
|---|---|---|
| `P2-T-ACQUISITION-CEILING` | Conditional on a frozen acquired pool, no downstream selector that cannot invent evidence can submit more gold/decision-relevant evidence than the pool contains. | `ANALYTIC_PROVED_IN_MANUSCRIPT`; set-inclusion proof in `manuscript/sections/acquisition_authority-envelope.tex`. |
| `P2-P-CALLS-ARE-NOT-EXPOSURE` | Equal request counts alone impose no nontrivial bound on the difference between two arms' acquisition ceilings. | `ANALYTIC_PROVED_IN_MANUSCRIPT`; constructive counterexample. The historical one-versus-three admissible-route exposure is an instance, not the proof's basis. |
| `P2-T-CLOSURE-FACTORIZATION` | A donor product can implement the contract-correct closure decision exactly iff closure truth is constant on each fibre of its retained observable state. | `ANALYTIC_PROVED_IN_MANUSCRIPT`; elementary factorization proof. P2-X B3's 400/400 tie is a compatible exact-contract instance, not the proof's basis. |
| `P2-T-INDISTINGUISHABLE-WORLDS` | False closure plus valid-closure shortfall is at least one minus total variation between the closable/open transcript laws. | `ANALYTIC_PROVED_IN_MANUSCRIPT`; binary-testing lower bound. It does not assert that any external world pair has been measured. |
| `P2-T-MAXIMAL-AUTHORITY-COMPLETION` | Under the declared material-obligation and adversarial-realizability contract, closing exactly after all registered material obligations are discharged is maximally permissive among universally sound closure rules. | `ANALYTIC_CONTRACT_RELATIVE`; proof in the new theory section. It does not prove that finite open-web search enumerates every obligation. |
| `P2-E-7-OF-229-EXPOSURE` | In the archived adverse external capture, all governed routes emitted 7/229 gold identifiers; therefore run-conditional recall could not exceed 0.030568 and 222/229 misses were upstream of selection. | `SUPPORTED_HISTORICAL_ADVERSE_DIAGNOSIS`; `evidence/successor_results/P2_ACQUISITION_CEILING_DIAGNOSIS_2026-08-22.{json,md}`. The associated scientific terminal remains adverse/non-promoted. |
| `P2-H-ODAE-ENVELOPE` | Obligation-directed adaptive exploration can improve the guarded discovery/utility/optionality/closure frontier by valuing unresolved-obligation reduction and closure-fibre separation under one charged budget. | `PROSPECTIVE_UNTESTED_MECHANISM_FAMILY`; analytic policy definition only. Generic value-of-information, active search, coverage and adaptive stopping remain donor-owned. |
| `P2-H-ENVELOPE-768` | A controller can exceed the guarded donor-complete acquisition--authority envelope across four source-disjoint arenas under matched admissible exposure. | `PROSPECTIVE_UNTESTED`; frozen shape only in `protocol/P2_TASK_WORLD_SUCCESSOR_V2.json`: 768 independent clusters, three primary comparators, intersection across contrasts, joint target power 0.90. Required external bindings and protected outcomes do not exist. |
...
boundary: P2-X is exact-contract evidence only, external retrieval/provider
