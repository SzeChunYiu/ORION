...
**Open-World Scientific Closure: Dynamic Coverage, Search Saturation, and Structural Jumps beyond Nearest-Neighbour Retrieval**
...
Search nearby donors, digest them into typed nodes and hyperedges, measure marginal structural yield, and continue while the local frontier expands. When new descriptions become redundant and the robust residual stops shrinking, search structurally remote regions. A song or distant discipline is admissible only through a target-native structural map and a hidden consequence.
...
- Authoritative result SHA: `7c3b3211a01710a637a0aa0aac556f36d8a01d38`.
- Exact terminal: `CANNOT_CHECK_STRONG_DONOR_OR_TRANSFER_BINDING_UNAVAILABLE`.
- Attained internal evidence: 50 frozen topics, eight policies, 400 retained topic-policy rows, no dropped topics, matched 100-read/four-query maxima, and policy outputs frozen before qrels access.
- Boundary finding: zero licensed structural-jump topics; `STRUCTURAL_JUMP`, `RANDOM_REMOTE`, and `LOCAL_SATURATION_STOP` therefore tied the local policy on all 50 topics rather than demonstrating remote-jump value.
- Comparator finding: the registered primary contrast against the internal ideal donor product had mean `-0.004218` with 95% bootstrap interval `[-0.008444, -0.000505]`; this does not support superiority.
- Unattained hard conditions: a frozen material biomedical neural donor, independently licensed review/reminted transfer worlds, and independent external gold custody.
- Allowed manuscript claims: the open-world impossibility and reopening theory; the exact bounded internal TREC-COVID comparison; the no-licensed-jump and donor/transfer/custody boundaries.
- Not established: global completeness, structural-jump value, transfer benefit, external independence, or superiority.

## Claim boundary

No finite study establishes global completeness. The final paper should foreground the negative and identification result: a jump policy cannot be credited when no topic attains the jump predicate, and a local policy cannot be called superior when the strongest available internal donor product performs better under the frozen contrast.
