...
## Material contraction immediately before these rounds
...
Fresh primary-source search used:

- structural/relational representation + unseen-domain transfer;
- expressivity versus graph generalization;
- domain relations and domain-informed representations;
- compositional/domain generalization under controlled representation differences.

The round surfaced additional pressure that:

- more expressive graph representations can generalize worse under greater structural distance/complexity;
...
## No-material-change round 2 — process/method/schema vocabulary

A second fresh search deliberately left standard ML `domain generalization` phrasing and used:

- process/model/method matching;
- object-centric process relations and constraints;
- procedural representation transfer;
- schema/relational comparison;
- cross-domain policy/representation alignment;
- procedural-data inductive structure.

Close sources strengthen prior ownership of:

- explicit object/process relations and constraint checking;
...
3. explicit source-insufficient `UNRESOLVED`;
4. entire held-out procedural domain;
5. protected unseen combinations of coordinate corruptions;
6. untyped and same-information typed-serialization controls;
7. one prospectively frozen classical model-selection protocol;
8. independent pre-artifact reimplementation.

Terminal: `NO_MATERIAL_CHANGE_2`.

## Bounded saturation state

`P9_D1_POST_CONTRACTION_NOVELTY_SATURATED_BOUNDED`.

Allowed D1 scientific wording remains:

> On the frozen D1 benchmark, explicit typed relational comparison changes the transfer behavior of the selected classical learner: the relational arm reaches perfect held-out-domain performance while the same-information serialization does not.

This is not a universal representation theorem, a graph-architecture novelty claim, or discovery of serialization friction.
