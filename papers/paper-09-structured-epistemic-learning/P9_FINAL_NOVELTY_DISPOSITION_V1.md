...
3. give generic classical learners first right of refusal and invalidate any result exceeding its view ceiling;
4. when an information-sufficient view still underperforms, separate a computation residual from missing information using a prospectively frozen explicit-inference control;
5. test transfer on exact authored/procedural method structures under a whole-domain holdout, reminted surface identities, explicit `UNRESOLVED`, and protected multi-coordinate corruptions;
6. include a same-information typed serialization control so a relational effect is not reduced to hidden extra semantic information;
7. stop architecture escalation when simple learning plus exact computation close the bounded residual.

## Final result synthesis

- M1: `M1_GLOBAL_COMPOSITION_RESIDUAL`; semantic mechanic ranking = 1.0, semantic/current affine gluing = 0.5104167 at exact task information ceiling 1.0. The residual is computation, not information.
...
- neural algorithmic reasoning or symbolic-neural hybrid computation;
- representation sufficiency/minimal-sufficiency theory;
- state/observation aliasing, feature collisions, or underspecification in general;
- the general distinction between representation/perception failure and reasoning/computation failure;
- symbolic-oracle fallback or exact symbolic verification;
- abstention/unsolvability/missing-information evaluation;
- the principle that native structured relations can be degraded by serialization (`serialization friction`);
- generic domain/compositional generalization from relational structure;
- procedural-content transfer benchmarking;
- reusable program/module/mechanic discovery;
- recurrent/continuous latent reasoning, dedicated variable binding, causal-mechanistic discovery, method invention, LLM problem solving, or scientific authority.

The collision-derived ceiling formula is elementary empirical accounting over the frozen equivalence classes and is not presented as a theorem contribution.

## Post-result nearest-work saturation

The final search was repeatedly narrowed whenever a closer parent appeared.

Material contractions included:

- *When 2D Tasks Meet 1D Serialization: On Serialization Friction in Structured Tasks* (arXiv:2604.27272): owns the general same-information/native-structure-vs-serialization principle.
- *Temporal Reasoning Is Not the Bottleneck: A Probabilistic Inconsistency Framework for Neuro-Symbolic QA* (arXiv:2605.04243): directly owns a general representation-vs-reasoning failure separation in temporal QA.
- *Compositional Choke Points* (OpenReview, 2026): deterministic benchmark + symbolic-neural oracle isolating iterative-computation failure.
...
TMLR is a suitable target because its current scope explicitly includes experimental studies yielding insight into learning-system behavior, applications of existing techniques that expose strengths/weaknesses, formalization of new learning tasks/evaluation methods, and new analysis approaches. Its acceptance criteria prioritize well-supported claims and audience interest rather than requiring a novel model or state-of-the-art result.
...
Reopen the novelty/standalone disposition only if a current primary source is found that materially duplicates the combined final protocol/result, or if sibling-paper overlap would require reuse of archival text/results beyond the stated ownership boundary.
