...
D1 supplies the external procedural-transfer test. The typed relational arm reaches \DOneTypedAccuracy{} protected accuracy on transactional workflows, compared with \DOneUntypedAccuracy{} for untyped pair structure, \DOneSerializedAccuracy{} for the same-information typed serialization, and \DOneTranscriptAccuracy{} for the reminted transcript bag. The improvement over the same-information serialization is \DOneTypedMinusSerialized{}. The \DOneTypedMinusTranscript{} figure against transcript is reported separately and is not a transfer margin: the transcript comparator answers \textsc{aligned} on all 128 protected cases, so the quantity is $1-\text{prior}(\textsc{aligned})$. Its content is that a surface-action view has an empty transferable vocabulary over a whole-domain holdout, which is a stronger and narrower statement than outperforming it. The typed relational arm also preserves \DOneTypedDoubleAccuracy{} accuracy on double corruptions and \DOneTypedUnresolvedAccuracy{} on source-insufficient \textsc{unresolved} cases.
...
\begin{enumerate}
    ...
    \item typed relational method coordinates transfer to a held-out procedural domain better than untyped and same-information serialized comparators on D1, and the reminted transcript view carries no transferable vocabulary over that holdout.
\end{enumerate}
