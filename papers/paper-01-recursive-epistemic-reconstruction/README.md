...
The hidden-shift suite directly exercises parent-domain and representation changes plus negative controls that should **not** reframe. It exposed and repaired an over-broad control rule: singular `EVIDENCE`/`EXECUTION` responsibility previously could enter `REFRAME`; those cases are now routed away from formulation rewrite.
