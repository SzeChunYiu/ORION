...
conditions without pooling the runs. The separate verifier is
...
The bounded semantic-interface handoff is
\idt{../lane-handoffs/p1-source-native-action-adapter-v2/}. Its amended
protocol, compatibility matrix, certification result, partial adapter,
conditional certificate, taxonomy provenance and primary captures are bound by
the 47-entry \idt{SHA256SUMS} manifest. The manifest digest is
\idt{9f4503f693e155b12fb8c333f4777619cec1d66e387dcfc0e141fffa6933847d}.
An independent standard-library \texttt{hashlib.sha256} pass over every entry
recorded zero failures in
\idt{../lane-handoffs/p1-source-native-action-adapter-integration-v3/INDEPENDENT_SHA256_VERIFICATION_V1.json}.
That receipt checks byte integrity only; it is not external semantic review,
case authority or empirical verification.

The audit enumerates all 117,649 total maps: 0 are fully certified, 116,929 are
rejected, and 720 remain not disproved but uncertified. The only currently
certified partial map preserves \textsc{Unresolved}; the actionable mappings
remain unbound because exhaustive target denotations and an owner-approved
decision/action bridge are absent. The full-withdrawal counterexample is
explicitly conditional on later owner-ratification of proposed closed-world
target denotations. No case text, row-level outcome, protected gold or system
outcome was accessed, and the captured public taxonomies license semantic
development only, not an R7 transition.
