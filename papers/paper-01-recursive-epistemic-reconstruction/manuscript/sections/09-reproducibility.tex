...
The full 47-entry semantic-interface handoff and its recorded independent
checksum receipt remain in external custody and are not present in a clean
repository checkout. The manuscript retains only the recorded handoff manifest
digest,
\idt{9f4503f693e155b12fb8c333f4777619cec1d66e387dcfc0e141fffa6933847d}.
Because neither the manifest entries nor the independent receipt are
repository-available, clean-checkout byte verification is
\textsc{Cannot-check}; the historical zero-failure checksum statement is not a
current repository integrity result. No private or large payload is
redistributed here, and the unavailable handoff cannot serve as
repository-available evidence or external semantic authority.
...
outcome was accessed. These counts are bounded provenance statements recorded
from the unavailable handoff, not independently checkable current-repository
facts or external semantic authority. Licensing for the captured public
taxonomies remains uncertain; no redistribution right or R7 transition is
assumed.
