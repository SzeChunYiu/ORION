...
ORION's design explicitly separates authority promotion (Paper IV) from formulation reconstruction (Paper I). Paper I mechanisms can expose possible method failures but cannot authorize $M_t$ to rewrite itself. The reframe gate is mechanically bounded: only formulation and search-universe coordinates may trigger a reframe, and evidence/execution-only negative controls exercise the corresponding non-reframe behavior in the frozen local suite.

\subsection{Resource usage and environmental impact}
The repository does not yet contain an admissible clean-environment resource ledger for the full Paper I campaign. Numerical model-token, runtime, monetary-cost, or carbon-impact claims are therefore \texttt{CANNOT\_CHECK} in this revision. A future result-bearing run must retain actual resource usage under its frozen provider/environment bindings before those quantities are reported.
