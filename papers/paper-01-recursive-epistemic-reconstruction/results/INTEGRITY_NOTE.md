# P1 campaign integrity and recovery note
...
| P1-T2 table status | `OK` |
| P1-T3 table status | `OK` |
...
counts. Every P1-T2 row records `cannot_check_cases: 0`.
...
The P1-T2 digest includes the status-ontology correction documented by
`P1-T2_STATUS_ONTOLOGY_CORRECTION_V1.json`. The raw records, primary rates,
intervals, effects, and registered P1 hypothesis verdicts are unchanged.
...
P1-T2 was correctly `PARTIAL`. Those facts describe the failed attempt only.
...
narrower credential-free mechanical claim `P1.NECESSITY.V2.2.4`. The two claims
