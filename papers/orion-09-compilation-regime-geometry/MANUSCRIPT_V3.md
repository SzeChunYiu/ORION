...
Those mixed cells impose an irreducible 43/1,146 classification floor for any classifier using only that frozen vocabulary. The result is representational, not universal: donor exactness is **not a function of these features on this domain**. Enlarging the representation remains an open successor question.
