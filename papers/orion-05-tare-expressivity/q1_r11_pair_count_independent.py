...
def union_support(frames):
    out = set()
    for frame in frames:
        out.update(q for q, letter in enumerate(frame) if letter != 0)
    return tuple(sorted(out))


def tag_syndrome(tag, frames):
    return tuple(symp(tag, frame) for frame in frames)
...
def restricted_tag_minimum(n, frames, rhs, active):
    ...
    for letters in product(range(4), repeat=len(active)):
        tag = [0] * n
        for q, letter in zip(active, letters):
            tag[q] = letter
        tag = tuple(tag)
        if tag_syndrome(tag, frames) == rhs:
            w = support(tag)
...
def tag_dp_minimum(n, frames, rhs, active):
    """Minimum Tag support by exact syndrome DP over active coordinates only."""
    zero = (0,) * len(frames)
    dp = {zero: 0}
    for q in active:
        nxt = {}
...
def tag_constraint_rank(frames, active):
    """Rank of the six Tag symplectic equations in 2|active| binary variables."""
    rows = []
    for frame in frames:
        row = 0
...
def restore_cost_full(targets, frames):
    """Frozen two-branch all-three factor support from local letters."""
    n = len(targets[0])
    total = 0
    for q in range(n):
        for k in (0, 1):
            ...
            for block in range(3):
                idx = 2 * block + k
                letters.append(mul(targets[idx][q], frames[idx][q]))
            total += f3(*letters)
    return total


def restore_cost_active(targets, frames, active):
    """Precomputed identity-frame baseline plus <=9 active-coordinate corrections."""
    n = len(targets[0])
    baseline = 0
    for q in range(n):
    ...
    total = baseline
    for q in active:
        old = f3(targets[0][q], targets[2][q], targets[4][q])
        old += f3(targets[1][q], targets[3][q], targets[5][q])
        new = f3(
            mul(targets[0][q], frames[0][q]),
            mul(targets[2][q], frames[2][q]),
            mul(targets[4][q], frames[4][q]),
        )
        new += f3(
            mul(targets[1][q], frames[1][q]),
            mul(targets[3][q], frames[3][q]),
            mul(targets[5][q], frames[5][q]),
        )
        total += new - old
    return total
...
def deterministic_targets(n, salt):
    """Small target family with no hidden random state."""
    rows = []
    for t in range(6):
        rows.append(tuple(((q + 2 * t + salt * (t + 1)) % 4) for q in range(n)))
    return tuple(rows)
...
def check_pair_counts():
    ...
    for n in range(1, 7):
        ...
        for a, b in ordered_pairs(n):
            # Each member has support <=2 and anticommutation forces an overlap.
            assert len(union_support((a, b))) <= 3
...
def check_active_union_and_tag():
    ...
    for n in range(1, 5):
        ...
        idx = sample_indices(len(pairs))
        for ia in idx:
            for ib in idx:
                for ic in idx:
                    frames = pairs[ia] + pairs[ib] + pairs[ic]
                    active = union_support(frames)
                    ...
                    for orientation in ((0, 1), (1, 0)):
                        ...
                        if full is not None:
                            ...
                            assert full <= rank <= 6, (n, active, full, rank)
                            feasible += 1
                        checked += 1

                    # Candidate-dependent Restore cost is exactly baseline + active correction.
                    for salt in (0, 1, 3):
                        targets = deterministic_targets(n, salt)
                        assert restore_cost_full(targets, frames) == restore_cost_active(
                            targets, frames, active
                        )

    assert checked > 0 and feasible > 0
    print(f"tag_active_union_checks={checked} feasible_tag_cases={feasible}")

    # Hostile boundary: if one invents an extra constraint that explicitly reads
    # an outside coordinate, active-union restriction is no longer licensed.
    # The theorem is therefore grammar/constraint scoped, not a generic Tag claim.
    n = 2
    frames = ordered_pairs(n)[0] * 3
    active = union_support(frames)
    outside = next((q for q in range(n) if q not in active), None)
    if outside is not None:
        tag = [0] * n
        tag[outside] = 1
        assert tuple(tag)[outside] != 0


def main():
    check_pair_counts()
    check_active_union_and_tag()
    print("Q1_R11_INDEPENDENT_FINITE_CHECK_PASS")
