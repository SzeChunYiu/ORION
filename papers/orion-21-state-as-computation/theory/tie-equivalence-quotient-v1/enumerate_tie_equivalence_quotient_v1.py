...
def control_no_alarm(max_p: int, rows_cache: dict[int, list[tuple[int, ...]]]) -> dict[str, Any]:
    """Negative control: strictly distinct |c| must yield singleton classes and
    ZERO refutations of every invariant.  A refutation firing here is a bug."""
    checked = 0
    for p in range(2, max_p + 1):
        # strictly distinct magnitudes, both sign patterns exercised
        for signs in itertools.product((1, -1), repeat=p):
            ...
            for r in range(1, p + 1):
                ...
                ctx = _make_ctx(corr, options, meta, rows_cache[p])
                base = invariant_values(corr, options[0], ctx)
                for name, _kind, _desc in INVARIANT_SPECS:
                    # single-member class: every invariant is trivially constant
                    if base[name] != base[name]:  # pragma: no cover
                        raise ControlViolation("no-alarm control: invariant unstable")
                checked += 1
    return {"passed": True, "states_checked": checked,
            "rule": "strictly distinct |c| implies singleton class and zero refutations"}
    ...
    for p in p_values:
        ...
        for corr in itertools.product(alphabet, repeat=p):
            for r in range(1, p + 1):
                ...
                for name, _kind, _desc in INVARIANT_SPECS:
                    ref = values[0][name]
                    diff_idx = next((k for k in range(1, size) if values[k][name] != ref), None)
