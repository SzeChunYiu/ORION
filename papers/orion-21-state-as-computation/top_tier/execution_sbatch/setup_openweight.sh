...
payload = json.dumps({"model":"llama3.1:8b","prompt":prompt,"stream":False,"options":{"num_ctx":32768,"temperature":0.6,"top_p":0.9,"seed":42}}).encode()
t0=time.time()
req = urllib.request.Request("http://127.0.0.1:11434/api/generate", data=payload, headers={"Content-Type":"application/json"})
...
echo ---EMBED_END
nproc; nvidia-smi --query-gpu=name,memory.used,memory.total --format=csv 2>/dev/null | head -3
