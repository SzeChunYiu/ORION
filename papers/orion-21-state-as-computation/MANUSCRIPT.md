...
The terminal `P11I_HIGH_WIDTH_ADVANTAGE_REPLICATED_WIDE_PANEL` licenses a width-conditioned result only: across three independent RNG replicates and three fixed geometry strata, all nine prespecified high-width seed×geometry cells pass. A fresh two-subprocess revalidation under `P11I_REPLICATION_UNIT_AMENDMENT_V1_1.md` reproduces every cell byte-identically while recording `n=3`, not nine. The pooled attack wins in the narrow regime and the compiled-state advantage replicates in the registered high-width regime. `P11_ACTIVE_CLAIM_AUTHORITY_V2.json` supersedes V1 as the current authority while preserving this positive leaf byte-for-byte; P11D and P11H remain adverse historical results.

### 5.7 Ten-responsibility digits boundary

The frozen query-family phase study tests whether the earlier learned-compiler
result survives across all ten digit responsibilities under linear, RBF and KNN
access. It does not. With an inclusive per-query tolerance of `-0.02`, the
compiled 16-of-64 state is supported for **3/10** responsibilities under LINEAR
access and **5/10** under each of RBF and KNN, below the preregistered **8/10**
family gate in every access class. The terminal is
`P11_QUERY_FAMILY_PHASE_V1_GATE_NOT_MET`; thresholds were not retuned.

The adverse quality result does not invalidate the separately frozen resource
identities. Both implementations agree on all 70 phase rows: compiled state is
no larger than the 64-float universal state exactly when `U<=4`; the charged
LINEAR break-even horizon grows from 1,917 to 19,169 service steps; and a newly
arriving responsibility imposes a positive specialization cost on the compiled
route while the universal state requires no reconstruction. The resulting
boundary is responsibility-conditioned: retain raw state unless the family is
small (`U<=4`), every member is individually compile-tolerant, and the service
horizon exceeds the charged break-even.

This negative is bound in `P11_ACTIVE_CLAIM_AUTHORITY_V2.json` alongside, not
in place of, the width-conditioned P11H/P11I result. It does not refute the
`r=7` controlled replication, and that replication cannot be used to overwrite
the real-data family negative. The executed study identity is
`P11_QUERY_FAMILY_PHASE_V1`.
