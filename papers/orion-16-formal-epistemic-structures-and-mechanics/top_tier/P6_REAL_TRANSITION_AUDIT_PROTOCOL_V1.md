...
- complete old/new URI correspondence registered -> evidence meaning transports;
- lexical key/value preserved but correspondence missing -> `CANNOT_CHECK`;
...
- immutable model-output bytes preserved, only archived path discovery repaired, frozen analyzer unchanged, negative result replay agrees -> `ADMISSIBLE` for the bounded negative result;
- same path recovery but output digest mismatch -> `CANNOT_CHECK`;
...
- extraction completed but frozen eligibility yields 0/11842 -> `CANNOT_CHECK` for native superiority;
- same execution facts used only to claim extraction/coverage boundary -> `ADMISSIBLE` for that bounded negative/CANNOT_CHECK statement;
...
- normalized execution facts round-trip and the independent scientific/authority record is preserved -> `ADMISSIBLE`;
- provenance round-trip succeeds but scientific contract/authority record is absent -> `CANNOT_CHECK`;
- provenance round-trip changes an execution digest -> `CANNOT_CHECK`;
- provenance complete but independent science is invalid -> `REOPEN`/invalid scientific state, never admissible.

## Comparator

`EXECUTION_PROVENANCE_POLICY` receives the same execution/provenance/action-permission/source facts and admits a transition when:

- execution/provenance binding is valid;
- the source record exists/current;
- generic permission is satisfied.

It does **not** inspect responsibility-scoped evidence transport, open scientific obligations or claim-commit authority. This comparator represents the strongest relevant lower layer actually available in the selected records; if adding those scientific coordinates makes it extensionally equal to ETS, the correct result is assimilation/equivalence rather than novelty inflation.

`ETS` uses the four coordinates:

- computational/execution support;
- evidence meaning/transport;
- scientific obligations/coverage;
- scientific commit authority/scope.

Missing evidence/coverage yields `CANNOT_CHECK`; invalidated evidence/obligation yields `REOPEN`; commit scope violation yields `DENIED`.

## Primary endpoints

- exact disposition accuracy against independent frozen gold;
- unsafe false-admissible count;
- unnecessary reopen count on real admissible controls;
- correct `CANNOT_CHECK` count;
- scope/authority laundering count;
- per-family accuracy;
- source-token existence/currentness;
- deterministic replay;
- independent second checker agreement.

## Positive terminal

`P6_REAL_TRANSITION_AUDIT_V1_SUPPORTED` requires:

- all source receipts/standard records pass token binding;
- ETS exact accuracy `1.0` across every frozen real transition;
