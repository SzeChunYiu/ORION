...
The authority context contains active typed judgments, hard and soft obligations, grants, a protected coercion registry, revocation state, provenance/derivation lineage, and retained request/allow/deny/commit/revoke history.
...
Authorization requires all hard obligations discharged, no active blocker, a valid grant, fresh bindings, and matching effect identity/scope/content/epoch immediately before commit.
...
\begin{theorem}[Permission/scientific-discharge separation]
There exists a context in which a generic policy grant validly permits an operation while the corresponding scientific effect remains at the inability-to-check terminal because a mandatory scientific obligation is unresolved.
\end{theorem}
\begin{proof}
...
The normative checker exercises all 25 direct source-domain/target-domain combinations, valid and invalid typed coercion paths, scope mismatches, stale epochs, non-compensatory blockers, protected self-promotion boundaries, exact alternative-derivation revocation, and the three authority terminals. It also runs 160 shared-calculus/ideal-product equivalence cases. A frozen 17-case authority manifest contains clean authorized controls in each domain, paired blockers, five explicit cross-domain laundering attacks, an inability-to-check case, and a positive registered coercion.
...
\textbf{Competing interests and funding.} Personal competing-interest and funding declarations are entered in the submission system after direct author confirmation. This manuscript does not infer or fabricate private financial/non-financial declarations from repository data.
