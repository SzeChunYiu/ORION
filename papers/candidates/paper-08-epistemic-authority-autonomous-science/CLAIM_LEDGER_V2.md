# P8 claim ledger V2

**Paper:** A Theory of Epistemic Authority for Autonomous Science  
**Theory status:** `CLOSED_V2`  
**Novelty / separate-paper status:** `CANNOT_CHECK` pending literature and strong-baseline closure  
**Rule:** engulf authorization/effect/provenance/abstention structures before testing the scientific-obligation interface; do not claim centralization as inherent superiority.

| ID | Permitted claim | Authority | Evidence / proof | Forbidden upgrade |
|---|---|---|---|---|
| P8-C1 | P8 types every authority-relevant judgment by domain, kind, scope, content contract and epoch, and binds hard obligations to exact discharge types. | FORMAL_DEFINITION | `manuscript/FORMAL_CORE_V2.md` | “P8 invents typed authorization.” |
| P8-C2 | If ordinary inference preserves type and only protected coercions change type, foreign-domain authority cannot discharge a target obligation without a composable coercion path. | THEOREM | Thm. 1 | Novelty over all information-flow/authorization type systems. |
| P8-C3 | Coercion composition must match the full type; domain-only reachability can accept an invalid scope/kind/content interface. | COUNTEREXAMPLE / REPAIR | V2 coercion fixture | Claim that every existing propagation system is domain-only. |
| P8-C4 | Hard scientific obligations are non-compensatory under the P8 rule; soft utility/confidence cannot discharge them. | FORMAL_POLICY | Def. 10 | Claim that all scoring methods are unsafe. |
| P8-C5 | With unbounded extensible positive evidence, a finite additive blocker penalty can be overcome. | SCOPED_PROPOSITION | Prop. 2 | “Finite scalar policies can never encode vetoes.” |
| P8-C6 | `AUTHORIZED`, `DENIED`, and `CANNOT_CHECK` are distinct terminals with different epistemic/repair meanings. | FORMAL_SEMANTICS | Def. 11 + checker | Novel abstention taxonomy claim without donor credit. |
| P8-C7 | Alternative complete derivations are represented by support families; a certificate survives revocation iff at least one complete valid support set remains. | THEOREM | Thm. 4 + revocation checker | Claim that all authorization systems lack alternative-derivation semantics. |
| P8-C8 | Post-hoc refusal cannot make a prior irreversible commit pre-effect authorized; stale certificates need freshness/revalidation when relevant premises changed. | STRUCTURAL_BOUNDARY / DONOR-OWNED_CONSTRAINT | Props. 5–6 | Novel ongoing-authorization claim. |
| P8-C9 | Candidate-controlled admission predicate plus candidate-controlled evidence cannot guarantee an external promotion property. | COUNTERMODEL / P5 EMBEDDING | Prop. 7 | Universal impossibility of safe self-modification. |
| P8-C10 | A shared calculus and an ideal typed product of per-domain gates are extensionally equivalent when both use the same discharge/coercion/freshness/revocation semantics. | THEOREM / NEGATIVE_RESULT | Thm. 8 + 160 checker cases | “Centralization is inherently more expressive.” |
| P8-C11 | Generic action permission may be valid while a mandatory scientific obligation remains unresolved; generic permission therefore does not imply P8 scientific `AUTHORIZED`. | SEPARATION_THEOREM | Thm. 9 | “Security authorization is wrong/incomplete for its own purpose.” |
| P8-C12 | P1–P5 native authority gates are protected special cases to be reproduced, not overwritten. | INTERNAL_OWNERSHIP / EMBEDDING_CONSTRAINT | formal core + native embedding fixture | Claiming P1–P5 gate mechanisms as P8 novelty. |
| P8-C13 | The V2 checker covers 25 domain pairs, typed coercion, revocation, terminals, finite-blocker cases, 160 shared/product equivalence cases, positive coercion, epoch and self-promotion boundaries. | DETERMINISTIC_ARTIFACT_FACT | `formal/check_theory_closure_v2.py` | Real-agent performance claim. |
| P8-C14 | The donor-complete envelope attempts to combine authorization/trust management, UCON/ongoing authorization, multi-authority propagation, effects, provenance, abstention and scientific gates before narrowing. | PROGRAMME_DESIGN | donor-complete envelope | “ORION already outperforms all authorization systems.” |

## Donor ownership that must remain visible

P8 treats as donor/pre-existing: deontic/action/input-output logics; authorization/trust-management languages and proof systems; delegation/revocation; UCON/ongoing authorization; typed effects and trace policies; FAVA evidence-backed authorization; multi-authority and authorization-propagation systems; stale/commit-time authorization control; agent abstention; provenance/source attribution/execution provenance; runtime shielding; ORION P4 scientific-authority promotion and P5 protected self-promotion boundary.

## Allowed headline

> Cross-domain scientific authority requires full evidence-to-obligation typing: valid permission or success in one domain does not discharge another domain's scientific obligation without an explicit protected coercion, and a shared calculus is behaviorally equivalent to an equally typed donor product when their semantics are identical.

## Disallowed headlines

- “The first theory of agent authority.”
- “P8 invents ongoing/commit-time authorization.”
- “A centralized authority calculus is more expressive than independent gates.”
- “P8 outperforms UCON, ETAS, FAVA, multi-authority governance, abstention or provenance systems.”

Separate-paper superiority requires a frozen discriminator versus the ideal donor-product baseline, potentially on interface consistency, proof/audit cost, revocation coverage or implementation-defect rate.
