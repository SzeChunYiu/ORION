...
**Rule:** a formal theorem is authority only for the theorem in its declared model; deterministic bounded cases are regression/countermodel support, not empirical generalization.

Status vocabulary: `FORMAL`, `LOCALLY_TESTED`, `EXTERNALLY_SCOPED`, `NONCLAIM`, `CANNOT_CHECK`, `STRUCK`.
...
- status: `FORMAL + LOCALLY_TESTED`;
- authority: V2.1 root-inclusive safety theorem and exhaustive four-node forward-DAG/change-set checks;
- scope: support-sound abstraction, changed set as declared; permits conservative edges;
...
- status: `FORMAL + LOCALLY_TESTED`;
- authority: spurious-edge countermodel + affected-realizability theorem in `FORMAL_CORE_V2_1.md` and V2.1 checker;
- scope: minimax/information-model result for uniformly sound graph-only policies;
...
- status: `FORMAL + LOCALLY_TESTED`;
- authority: history-aware commutation theorem, hidden-ambient-read counterexample, checker;
- scope: deterministic admissible footprint-faithful mechanics satisfying full separation;
...
- status: `FORMAL + LOCALLY_TESTED`;
- authority: fixed-latent/refined-observation theorem + harmful-coarsening control/checker;
- scope: constructed worst-case deterministic stationary-policy family; not empirical performance of LLM agents;
...
- status: `FORMAL + LOCALLY_TESTED`;
- authority: evidence/closure separation theorem and finite witness;
- scope: existential nonimplication between evidence validity and target-obligation satisfaction;
...
- status: `FORMAL + LOCALLY_TESTED`;
- authority: positive transport theorem, ambiguity-conditioned negative theorem, 64-condition checker;
...
- status: `EXTERNALLY_SCOPED + NONCLAIM`;
- authority: 2026-08-18 literature delta explicitly absorbing categorical scientific-regime transport, sheaf transport/obstruction, open-world closure-gap work, planning abstraction, schema/ontology transport, goal/world-model evolution;
- novelty state: broad representation-transport novelty `STRUCK`; scoped interface stable on dated two-round search.
...
- status: `FORMAL + LOCALLY_TESTED`;
- authority: generic-permission/scientific-discharge separation theorem;
- nearest-work boundary: SAGE-Fin already establishes typed exact-effect governance and context/evidence-versus-authority pressure; action-evidence specifications own exact action/evidence binding;
...
- status: `FORMAL + LOCALLY_TESTED`;
- authority: product-decomposition theorem + 160 equivalence cases;
...
`SUBMISSION_HEADLINE_AUTHORITY = COMPLETE` subject to exact-head structural linter and repository CI. Any later manuscript headline change reopens this ledger.
