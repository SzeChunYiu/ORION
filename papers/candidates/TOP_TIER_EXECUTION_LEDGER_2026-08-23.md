...
- protected workflow run: `32644815733`;
- artifact id: `9494570298`;
- artifact ZIP SHA-256: `b45df33c73eb78e941b53d3ef0b0c4ca95970974ee56387b565ee358e23d0a08`;
- frozen cases SHA-256: `8868875a7d7e9db376543fe466409adf3b2f02bd03885c9fe3803d9c6b5d2ae2`;
- frozen gold SHA-256: `57cc7707cc24a243bf0c40f2908fc316e4d2f24272f6b30554ffabceacbdec16`;
...
- strongest declared same-facts donor product: exact accuracy `0.50`, `9` unsafe false-admissible decisions, `3` laundering false-admissible decisions, `0` unnecessary reopen errors on independent-support controls;
- bounded ETS checker: exact accuracy `1.00`, `0` unsafe false-admissible, `0` laundering false-admissible, `0` unnecessary reopen errors.

Executable theory layer:

- T6.1: only the all-preserved assignment among 16 four-factor assignments is admissible when computational support, evidence transport, scientific obligation and scientific authority are all required;
- T6.2: matching epoch/scope transport composes, while epoch mismatch, scope mismatch and an open obligation block composition;
- T6.3: `47` pairs share the donor decision signature while requiring different scientific dispositions.
...
- a closure-transport counterexample when any required preservation coordinate is absent: fact, evidence meaning, support, obligation or coverage;
- sequential assumption invalidation blocks witness composition even when local mappings are well-formed;
- interface mismatch blocks sequential witness composition;
- a frozen closed-world/open-world pair with identical observed history has best possible deterministic observed-only accuracy `0.5`, requiring an external coverage witness to separate the worlds.
...
- full-type coercion composes only under matching intermediate type and valid/protected coercions; ambiguity remains `CANNOT_CHECK`;
- revoking source A preserves independent derivation `d2`, whereas revoking A+B removes all surviving registered support;
- confidence up to `1.0` does not compensate for a hard scientific type mismatch.
...
Two repository-root conformance defects in the publication regeneration chain were repaired without changing any scientific data, estimator, threshold or endpoint. On head `5ac5a02d1b65143909788a1831eded5cfcdce256`:

- `p9-paper-package` run `32651708809`: focused 29-test science suite, paired D1 derivation, fail-closed official-vs-independent verification, evidence summary, macros/tables regeneration and manuscript audit all GREEN;
- evidence artifact id `9496386037`; artifact ZIP SHA-256 `edc2b5584446826c5407b8100e70a9bbe33f0a896fc89c384ccf4e3fe1558537`;
- `p9-tmlr-pdf` run `32651708939`: fail-closed PDF build plus structural/anonymity smoke GREEN;
- PDF artifact id `9496389682`; artifact ZIP SHA-256 `81ec241cbccb12fed7d6117b53503aed9d2605131f369ff4ed0a3cc2cd57e4fd`.
...
- protected workflow run: `32645458392`;
- artifact id: `9494736360`;
...
- frozen held-out transfer targets solved: `6`;
- false expansions on known-method controls: `0`.

Allowed claim movement: OCME is a falsifiable formal object with exact obstruction and outside-closure expansion witnesses. This does **not** establish autonomous method invention because the successful candidate primitives (`AND2`, `SQUARE`) were declared before execution.
...
- protected workflow run: `32645458435`;
- artifact id: `9494739942`;
...
- deterministic byte replay: GREEN;
- protocol/cases/gold were committed before the checker.

Protected 18-case result:
...
The protected corpus supplies explicit executable H15.1–H15.5 witnesses, including complete attributable/replayable execution with invalid science, agreement on invalid science, and lane disagreement with independent valid science.

Allowed claim movement: the execution-integrity/replay/agreement -> scientific-validity/authorization non-implications are executable and non-vacuous over the frozen fault model. This is not superiority over real W3C PROV/RO-Crate/workflow/attested-execution systems; real interoperability, broad host fault injection, overhead and independent production adjudication remain open.
