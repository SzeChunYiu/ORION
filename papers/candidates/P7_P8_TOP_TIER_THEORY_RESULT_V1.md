...
**Artifact ZIP SHA-256:** `63c099f0b67090649975d82759c688f43a96dbb2890f61df57ab805dc2d9e40d`  
...
- explicit closure-transport counterexamples when **any** required preservation coordinate is absent: fact, evidence meaning, support, obligation or coverage;
- sequential assumption invalidation blocks witness composition even when each local regime mapping is individually well-formed;
...
- a pair with identical donor action-authorization facts is permitted in both cases while the target scientific judgments are respectively `AUTHORIZED` and `DENIED`;
- unknown scientific target/evidence typing yields `CANNOT_CHECK`;
- exact protected coercion composition succeeds only under matching intermediate full type and valid/protected coercions;
- ambiguous coercion remains `CANNOT_CHECK`;
