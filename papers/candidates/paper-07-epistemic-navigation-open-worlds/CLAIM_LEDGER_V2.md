...
| ORION-17-C8 | Incomplete and target-ambiguous transport requires reopen or `CANNOT_CHECK`; incomplete but non-ambiguous transport does not imply semantic failure. | THEOREM / BOUNDARY | Thm. 6 + 64 transport combinations | “Missing transport proof proves the old result false.” |
...
| ORION-17-C11 | The V2 checker covers stopping ambiguity, certificate boundary, fixed-information refinement/coarsening, evidence-vs-closure, 64 transport combinations and stop terminals. | DETERMINISTIC_ARTIFACT_FACT | `formal/check_theory_closure_v2.py` | Real-agent performance claim. |
