# P6–P8 formal-assumption audit V2
...
The current P7 and P8 formal cores already state their main impossibility, transport, anti-laundering and revocation claims with appropriately explicit premises. The material defect found in this audit is in the **P6 minimal-reopening theorem boundary**, plus one assumption that should be made explicit in the P6 commutation theorem.

This addendum therefore does not replace the V1 formal cores. It narrows two P6 claims and adds outcome-executed contract suites for P7/P8.

## 2. P6 defect A — graph soundness does not by itself imply minimal reopening

### V1 pressure point

P6 V1 defines dependency soundness as an over-approximation condition: every semantic support whose change may affect a certified claim is represented by an ancestor path in the graph. It then claims that reopening every graph descendant is inclusion-minimal among strategies sound for every semantics compatible with that graph.
...
Under that premise, any strategy that sees only the graph and must remain sound uniformly over the admissible class must reopen every descendant. The corrected theorem is stated in the P6 V2 formal addendum.
...
## 3. P6 defect B — commutation needs footprint fidelity, not declarations alone
