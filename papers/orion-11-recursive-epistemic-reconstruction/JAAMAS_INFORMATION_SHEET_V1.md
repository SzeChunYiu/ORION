# ORION-11 — JAAMAS submission information sheet (V1)

JAAMAS requires a 1–2 page sheet with every submission, and returns submissions
whose sheet is incomplete or uninformative without review. It asks two questions
and warns that "We are the first to have done X" is not an acceptable answer to
the first without stating the importance of X.

## 1. What is the main claim, and why is it an important contribution?

**Claim.** Deciding *when* a representation must change is a distinct problem
from deciding *how* to change it, and coupling the licensing of a transition to
the responsibility that motivates it is what makes the decision correct. In a
finite registered construction, a licensed-transition policy separates cleanly
from strong assimilated parents that lack the coupling.

**Why it matters for agents.** An agent that reformulates its own
representation faces two failure directions and standard designs trade one for
the other: reformulate too readily and it churns through high-level changes
nothing required; reformulate too reluctantly and it carries a representation
that can no longer express the distinction now being asked of it. Both are
silent — neither produces a local error at the moment it occurs. The evidence
here is that the licensing coupling separates the two directions
simultaneously rather than trading between them, which is the property a
self-revising agent actually needs and the one an ablated interface loses.

**What is not claimed.** Not naturalistic superiority and not inherent
expressivity. The information-equivalent product *ties*, and the paper says so:
the result is a finite mechanism boundary, not evidence that the mechanism wins
where information is equal.

## 2. What evidence is provided? Be precise.

**Mechanical worlds.** Over generator runs of 2,882 mechanical worlds each, the
licensed-transition policy attains hidden-shift success **1.0000** against
**0.4938** and **0.4833** for the strongest assimilated parents, with **no**
unnecessary high-level changes on 2,402 controls per run. Both directions at
once: it finds the shifts that require a change and does not manufacture changes
that were not required.

**Authored exact contracts.** On 400 authored exact contracts the policy makes
**400** correct decisions against **275** for a donor-complete interface lacking
the licensing coupling. On the same battery an **information-equivalent product
ties at 400/400** — the tie is retained rather than omitted, and it is what
bounds the claim to a mechanism boundary.

**Retained negative.** The historical **48-case** precursor remains negative and
is reported separately from the narrower positive successor, not superseded by
it.

**Naturalistic programme, reported as it stands.** 117,649 source-to-target maps
yield **116,929 rejections, 720 unresolved maps and zero certifications**. This
is the arm that would carry a naturalistic claim, and it certifies nothing.

**External authority is open.** V13 makes external authority execution
content-addressed but has received **0/7** signed outputs and closed **0/4**
authority acts, so no external authority has been exercised.
