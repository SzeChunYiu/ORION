---
title: "Typed Evidence Licenses for Fail-Closed Nonpromotion in Finite Rule Systems"
short-title: "Typed Evidence Licenses for Nonpromotion"
author:
  - name: "Sze Chun Yiu"
...
Doyle's truth-maintenance system associates conclusions with justifications and revises beliefs when supporting assumptions change [@doyle1979]. Assumption-based truth maintenance computes, for each derived node, the assumption environments under which it holds [@dekleer1986]. Martins and Shapiro develop a belief-revision model that retains multiple support contexts [@martins1988]. The Alchourrón-Gärdenfors-Makinson (AGM) framework characterizes rational contraction and revision at a more abstract level [@agm1985]. These traditions own the general problem of dependency-sensitive update. The present system does not compete with their expressivity: an assumption-based encoding records where a derivation's support came from, and a query over computed environments can test that support after evaluation. The typed rule instead carries origin and evidence class as labels during propagation, so that a conclusion needing mixed-origin support never acquires the license at all. It fixes a small positive rule language and asks which declared evidence license survives every edge of a particular derivation after direct refutation?
...
The residual specialization lies in the interpretation and enforcement of licenses. A license denotes a bounded evidential permission such as theorem-level support, prospective support, post-outcome support, bounded computation, or external replay. A rule cap states which such permissions the rule is allowed to transmit. Intersection across conjunctive premises enforces a common end-to-end license, while the cap prevents a rule from upgrading that license. With addition taken as label union and multiplication as label intersection, the transfer algebra is annotated evaluation at one fixed annotation configuration, so the contribution claimed here is not expressive power. It is that nonpromotion is enforced inside the fixed-point computation, under a declared vocabulary and caps, rather than recovered by inspecting derivations after evaluation. The resulting algebra is intentionally transparent and is not presented as the only reasonable model of scientific authority.

## Authorization and the domain instantiations

Modern authorization languages provide analyzable policy semantics and native decision engines; Cedar is one example [@cutler2024]. X.509 certificate validation has its own detailed path and revocation semantics [@rfc5280]. Neither domain is reduced here to the evidence-license calculus. The first domain-transfer attempt used Cedar's official multi-policy fixtures but found no native evidence-authority, license, or retraction field with which to adjudicate the proposed residual. That attempt was retained as a null result. Authorization policies of this class adjudicate requests against declared principals, actions, and resources; they do not attach evidential permissions to derived conclusions or constrain which permission survives each inference step, and the fixtures exposed no independently authored field of that kind. The typed layer supplies exactly that binding, using native decisions as fixed inputs. The second attempt used native OpenSSL decisions as fixed per-store facts and asked only whether erasing store origin during a merge can authorize a result that neither store authorizes alone. The evidence-license layer preserves that origin distinction; it does not replace the native X.509 engine.
...
where $A_r\subseteq Q$ is a nonempty finite antecedent set, $h_r\in Q$ is the head, and $K_r\subseteq\Lambda$ is the rule's license cap. Let $\mathcal{R}$ be a finite set of such registered rules and let the declared system be $\mathcal{S}=(Q,\Lambda,\sigma,\mathcal{R})$. Rules with empty bodies are represented as seeds. For a label assignment $x\in(2^\Lambda)^Q$, the transfer made by rule $r\in\mathcal{R}$ is
...
3. Every internal node with claim $h_r$ applies a registered rule $r\in\mathcal{R}$, has one child tree for each claim in $A_r$, and satisfies $\lambda\in K_r$.
...
**Corollary 5 (exact retraction within the declared algebra).** The set $\operatorname{Ret}_\Lambda(R)$ contains exactly the claim-license pairs for which every previously valid typed proof tree is destroyed by the registered refutations. A pair with at least one surviving valid typed proof tree remains licensed.
...
A deliberate boundary concerns expressiveness. The relationship to annotated-logic, provenance-semiring, and assumption-based truth-maintenance frameworks is declared as a donor relationship, not demonstrated as a formal separation: no proposition here shows that those frameworks cannot express origin-witness nonpromotion, and simple encodings appear to reproduce parts of the behavior. The filed contribution is accordingly a formal license-propagation system together with one measured hybrid-authorization phenomenon — 46 hybrid authorizations among 1,962 third-party merge tasks — not an expressiveness theorem. Whether a separation exists at the level of propagation-time enforcement, per-decision evaluation cost, or vocabulary curation remains open.
