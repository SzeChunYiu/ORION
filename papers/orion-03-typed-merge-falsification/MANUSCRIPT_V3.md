...
**ORION-03 — scientific successor manuscript V3**  
**Supersedes for journal science:** `MANUSCRIPT_V2.md`  
**Preserves:** the R2 calculus, all three scientific cases, X.509 instantiation, and negative/analytic boundaries
...
3. every internal node applies a registered rule whose cap contains `lambda` and has one child proof tree for each antecedent.
...
**Corollary 5 (exact retraction relative to the declared algebra).** `Ret_Lambda(R)` contains exactly the claim-license pairs for which all valid typed proof trees are destroyed by the registered refutations. Any pair with at least one surviving typed proof tree remains licensed.
...
The domain-agnostic evaluator under `packages/typed-merge-evaluator/` represents seeds, capped rules, refutations and expected verdicts, computes the registered least fixed point, and reconstructs typed proof-tree authorization. Its tests re-derive the committed scientific cases and verify source bindings.
...
The registered typed witness policy returns exactly the parent-authorized set. Therefore its zero unsafe merges, zero needless rejections and corresponding perfect precision/recall are propositional consequences of the benchmark definition and **must not** be presented as detector-performance measurements.

In the registered `PARITY_PARTITION` family, flat union incurs 4 unsafe merges; intersection and reject-all incur 63 needless rejections each; the `M4_OURS_B` policy incurs 14. These are genuine corpus-dependent measurements.
