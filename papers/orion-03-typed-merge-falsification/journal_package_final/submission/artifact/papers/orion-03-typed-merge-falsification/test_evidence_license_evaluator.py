...
class EvidenceLicenseEvaluatorTests(unittest.TestCase):
    ...
    def test_route_refutation_differs_from_direct_conclusion_refutation(self):
        claims = [
            {"id": "forecast_route", "seeds": ["PROSPECTIVE"]},
            {"id": "post_outcome_route", "seeds": ["POST_OUTCOME"]},
            {"id": "conclusion", "seeds": []},
        ]
        rules = [
            {
            ...
            {
                "id": "post_outcome_support",
                "body": ["post_outcome_route"],
                "head": "conclusion",
                "cap": ["POST_OUTCOME"],
            },
        ]

        route_refuted = evaluate_document(system(claims, rules, ["forecast_route"]))
        self.assertEqual(route_refuted["final_labels"]["conclusion"], ["POST_OUTCOME"])

        conclusion_refuted = evaluate_document(system(claims, rules, ["conclusion"]))
        self.assertEqual(conclusion_refuted["final_labels"]["conclusion"], [])
