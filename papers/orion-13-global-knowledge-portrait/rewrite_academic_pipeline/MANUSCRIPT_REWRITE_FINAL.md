...
1. **Identity above compatibility.** Scientific identity is represented as a target-bound relation that can authorize merge, authorize distinction, or remain unresolved even when records are representation-compatible.
2. **Prospectively separated evidence lanes.** A public-reference holdout tests false merge/false split behavior, while a separate 400-case exact-contract study tests whether a stronger semantic product remains insufficient for the identity decision.
3. **Implementation-equivalence control.** An information-equivalent typed product ties exactly, locating the scientific object in the represented relation rather than in one software architecture.

The paper evaluates already-structured scientific representations. It does not claim raw-text extraction superiority, universal coordinate necessity, or end-to-end knowledge-graph improvement.

## 2. Representation compatibility and identity authority

Let two structured records carry fields describing constructs, measurements, contexts, provenance, missingness, and other integration-relevant coordinates. A representation layer can determine whether those fields are syntactically compatible or connected by declared mappings.
...
A conservative exact-coordinate control supplies the false-split comparison. The typed rule and the conservative control have identical false-split outcomes on the registered holdout, giving a difference of 0 with interval [0,0]. The predeclared confirmatory rule therefore passes: the stronger identity semantics reduce false integration without buying that reduction through additional false separation on this panel.
...
## 5. An information-equivalent implementation must tie
...
This tie rules out an architectural interpretation. The paper does not claim that scientific identity requires a centralized calculus or proprietary state representation. If another system represents the same decision-relevant relation, the scientific prediction is equivalence.
...
The result is therefore a semantic-integration methodology contribution rather than a general claim about knowledge-graph quality.

## 8. Anti-confounding boundary

The structured identity result should not be read as proof that every recorded semantic coordinate is individually necessary. In the broader corpus, a polarity coordinate is sufficient to solve a confounded slice, which prevents a universal coordinate-necessity interpretation.

The present paper therefore claims the **target identity relation** and its tested decisions, not a universally minimal basis of semantic features. Establishing coordinate-level necessity would require a new anti-confounded ontology study with matched-polarity opposite-verdict pairs and native semantic gold.

This limitation is scientifically useful: exact performance of the identity rule and necessity of each representation coordinate are different questions.

## 9. Deliberately out-of-scope successor science

A broader programme considers raw-text extraction, expert annotation over multiple scientific families, global-portrait construction, downstream answer quality, and leave-one-ontology-out transfer. Those studies are not part of the current claim surface.
...
## 10. Reproducibility and release contract

The final submission package should bind the exact manuscript, public-reference holdout definitions, mapping outputs, 400-case successor contracts, result receipt, independent reconstruction, and claim ledger to one release. Reviewer-facing prose should use scientific descriptions rather than development identifiers.

The current journal package must be rebuilt against the final manuscript bytes; an earlier render or manifest is not a current submission artifact. Exact-subject CI and manuscript audit are release checks, not new scientific evidence.

## 11. Limitations

Both evidence lanes operate on structured inputs. The 32-case public-reference holdout is small, and the 400-case successor is an exact finite contract rather than a sample from deployed scientific integration systems. The identity relation is only as sound as the coordinates and scientific responsibility encoded by the contract; omitted load-bearing semantics can make an apparently exact rule wrong for a richer external task.

The study does not establish that the chosen relation is universally minimal. An information-equivalent product ties, as it should, and a different representation may encode the same scientific distinction more efficiently.
