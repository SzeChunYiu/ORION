...
Where third-party source text cannot be redistributed, the package must retain the legally shareable source identity/version/locator and content commitment or retrieval instructions rather than copying restricted text. The public-reference study must not be described as containing expert annotations that are not present in the frozen external source authorities.

Final insertion fields:

- submission commit: `[UNBOUND_FINAL_SUBJECT]`;
- permanent archive/release URL: `[UNBOUND]`;
- archive DOI, if minted: `[UNBOUND / NOT YET AVAILABLE]`;
- exact checksum manifest: `journal_package/SHA256SUMS` plus the frozen public-reference archive checksum files;
