...
- Frozen manuscript subject: `3c97b87f4f4c8c0365226019236c83d3c4c7bb37`
- Authoritative result integration: PR #1355, merge `906ad9d752ea088caf8c074fd8d117b8e142d62c`
- Code-only conformance repair: PR #1353, merge `134c77a2f4ffd838806b0bbf8621d7064f4a8c45`
- Integration head checked by the computation lane: `dc5a36c9c5256dbc3a35ceed955e28c7cd8d2717`
- Result packets: 25/25 complete
- Writing owner: `P1_P15_REWRITE_LANE`
- Computation-session paper-authority delta: `NONE`

This plan treats every positive, adverse, invalid, censored, donor-unavailable, and `CANNOT_CHECK` terminal as a scientific result. It does not turn completion of an execution packet into top-tier, novelty, submission, or external authority.

## Programme-level findings that may enter manuscripts

### Finite internal results supported
...
1. **Census is resource-censored.** The frozen source tree yielded 11,434 entries, 316,842 occurrences, 63,017 unique labels, and 121,985 retained unclassified occurrences. This is an internal source-tree census, not a complete semantic inventory.
2. **Ideal-donor superiority is unearned.** Metadata discovery covered 75 atoms but no matched runnable ideal-donor product was attained. Fifty-five of 75 arXiv calls failed; metadata is not substituted as executable donor evidence.
3. **Remote structural-jump value is unearned.** The material donor and independently bound transfer worlds were unavailable.
4. **Responsibility-identification superiority is unearned.** No externally scored unseen incident was available.
5. **Historical rediscovery is unearned.** Chronology or external custody remained unresolved.
6. **Frontier-mathematics discovery is unearned.** No externally escrowed target was supplied.
7. **External novelty is unearned.** Inaccessible work may absorb the exact residuals; no independent novelty review was completed.

## Paper-by-paper writing disposition
...
- `SUPPORTED_BOUNDED_CONTENT_BOUND`
- `SUPPORTED_INTERNAL_FINITE`
- `NEGATIVE_BOUNDARY_SUPPORTED`
- `INVALID_EXECUTION_RETAINED`
- `ACQUISITION_OR_CUSTODY_BLOCKED`
- `CANNOT_CHECK_EXTERNAL`
- `THEORY_TARGET_NOT_EMPIRICALLY_PROMOTED`

Never map:

- `CANNOT_CHECK` to support;
- internal replay to external independence;
- packet completion to top-tier readiness;
- metadata discovery to ideal-donor execution;
- a frozen design denominator to executed evidence;
- an invalid denominator to an empirical result.

## Manuscript integration contract

For each paper, the final abstract, Results, Discussion, claim ledger, figures, and journal package must bind to the exact authoritative result SHA and claim ceiling. Existing historical evidence remains separately identified. These successor packets add no paper authority by themselves.

```text
P1_P15_REWRITE_LANE_ACKNOWLEDGES_25_OF_25_COMPUTATION_LEDGER
```
