...
SCHEMA = "ORION.FiberGuard.PriorArtMatrix.R19.v2"
...
def build_result() -> dict[str, Any]:
    ...
    assert table_rows[0].startswith("| Area |")
    assert table_rows[1].startswith("|---|")
    ...
    lowered = text.lower()
    assert "absence of a direct title or phrase match is never treated as proof of novelty" in lowered
    assert "novelty_not_established__residual_candidate_explicit" in lowered
    assert "not an external novelty opinion or a novelty certificate" in lowered
