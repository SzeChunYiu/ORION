...
def verify_receipt(
    receipt: Mapping[str, Any], *, expected_manifest_sha256: str
) -> None:
    ...
    if (
        ...
        or payload.get("terminal") not in ALLOWED_INTERNAL_TERMINALS
    ):
        raise ReceiptMismatch(
            "receipt terminal is not an allowed Engine B internal terminal"
        )
    ...
    if terminal == "NQ_ENGINE_B_STRUCTURAL_EXECUTION_COMPLETE":
        if payload.get("processed_records") != payload.get("total_records"):
            raise ReceiptMismatch(
                "structural execution terminal has an incomplete denominator"
            )
        if payload.get("unsat_proofs_requiring_external_check") != 0:
            raise ReceiptMismatch(
                "structural execution terminal retains unchecked UNSAT proofs"
            )
