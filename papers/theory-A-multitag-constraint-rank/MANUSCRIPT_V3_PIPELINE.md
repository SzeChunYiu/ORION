...
\(\operatorname{zsf}(H; A)\) for the largest length of a word over \(A\) with no
nonempty zero-sum subsequence, where a subsequence is selected by an arbitrary
set of positions and need not be contiguous. We prove that every finite
instance of a deletion-closed compiler grammar has an exact optimum of support at most
...
We instantiate the theorem in an arbitrary-block MultiTag Tag-and-Restore
Encoding (TARE) grammar. With
...
necessity claim for any specialization or outside the sufficient cone. The result separates the
...
Tag-and-Restore Encoding (TARE) is an upstream block-encoding construction that exposes frame, Tag,
...
4. a one-Tag, three-block finite record illustrating the support-two ceiling
   without promoting it to an intrinsic theorem; and
...
## 8. One-Tag, three-block finite record

For the one-Tag, three-block grammar,
...
two. An internal exact \(n=2\) record reports unrestricted optimum 5 and
support-at-most-one optimum 6 over 13,824 feasible configurations. The public
source package currently supplies the result record but not a standalone
enumerator; accordingly, this finite observation is not used here to claim an
...
separate one-Tag, three-block finite record.
...
Restore-sensitivity records together with the one-Tag, three-block result
record. The displayed arguments carry the all-size upper claim; no intrinsic
lower claim depends on the packaged record. These files are distributed in the
