...
nonempty zero-sum subword. We prove that every instance of a deletion-closed
compiler grammar has an exact optimum of support at most
...
=\operatorname{rank}(H_R)\le s+1\) for every frame \(R\). The one-Tag,
three-block specialization is sharply intrinsic, with
\(\kappa_{\mathrm{1T3B}}=2\). We make no necessity claim for
...
4. a sharp one-Tag, three-block control with intrinsic support two; and
...
## 8. Sharp one-Tag, three-block specialization
...
two. The all-size deletion argument gives the matching upper bound. For the
lower bound, an exhaustive exact \(n=2\) production instance has unrestricted
optimum cost 5, attained with maximum frame support two, whereas exhaustive
minimization over all 13,824 feasible support-at-most-one configurations gives
cost 6. This finite witness excludes a uniform support-one normal form. Thus

\[
\kappa_{\mathrm{1T3B}}=2.
\]
...
separately sharp one-Tag, three-block control.
...
The arXiv ancillary package contains deterministic checks for the finite-group
identities, Restore sensitivity, and the exact one-Tag, three-block lower
witness. They are
corroborative: the displayed arguments carry the all-size upper claim, while
the exhaustive finite witness supplies the lower claim. A permanent archival
identifier will replace the ancillary-package reference in the journal
version.
...
support. The one-Tag, three-block case aligns certificate and intrinsic
complexity at two.
