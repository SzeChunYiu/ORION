...
def _elapsed_milliseconds(started: float, *, observed: float | None = None) -> int:
    """Return canonical integer timing, never a float-bearing receipt value."""

    ended = time.monotonic() if observed is None else observed
    return max(0, int((ended - started) * 1000 + 0.5))
...
def generate_d2_records(
    *,
    ...
    metrics = {
        ...
        "d2_wall_milliseconds": _elapsed_milliseconds(started),
    }
...
def generate_d3_records(
    ...
    *,
    ...
    metrics = {
        ...
        "d3_wall_milliseconds": _elapsed_milliseconds(started),
    }
    return records, metrics
...
D2_GENERATION_TERMINAL = "NQ_CR_B_D2_CENSUS_GENERATED_INDEPENDENTLY"
D3_GENERATION_TERMINAL = "NQ_CR_B_D3_CENSUS_GENERATED_FROM_CHECKPOINTED_D2"
...
def load_checkpointed_d2_records(input_root: Path) -> tuple[tuple[tuple[int, ...], ...], str]:
    """Load D2 only from its byte-bound, already-durable input bundle."""

    manifest_path = input_root / "input_manifest.json"
    try:
        if not manifest_path.is_file() or manifest_path.is_symlink():
            raise batch.InputManifestMismatch("input manifest is unavailable or unsafe")
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        verified = batch.verify_input_manifest(input_root, manifest)
        records = tuple(batch.iter_records(verified.stream_path))
    except (
        OSError,
        json.JSONDecodeError,
        ValueError,
        batch.InputManifestMismatch,
        batch.InputRecordMismatch,
        batch.CoverageIncomplete,
    ) as error:
        raise CensusGenerationMismatch(f"checkpointed D2 bundle is invalid: {error}") from error
    spec = fm.D2_SPEC
    if verified.scope != spec.scope or verified.record_count != spec.expected_record_count:
        raise CensusGenerationMismatch("checkpointed D2 scope or frozen denominator mismatch")
    for ordinal, record in enumerate(records):
        if (
            record.record_id != fm.record_identifier(spec, ordinal)
            or record.scope != spec.scope
            or record.required_bins != spec.required_bins
            or len(record.sequence) != spec.sequence_length
        ):
            raise CensusGenerationMismatch(
                f"checkpointed D2 record identity mismatch at ordinal {ordinal}"
            )
    return tuple(record.sequence for record in records), verified.manifest_sha256
...
def build_generation_receipt(
    ...
    *,
    ...
    requested_scope: str = "both",
    d2_input_manifest_sha256: str | None = None,
) -> dict[str, Any]:
    terminals = {
        "d2": D2_GENERATION_TERMINAL,
        "d3": D3_GENERATION_TERMINAL,
        "both": GENERATION_TERMINAL,
    }
    receipt: dict[str, Any] = {
        "schema": GENERATION_RECEIPT_SCHEMA,
        "terminal": terminals[requested_scope],
        ...
        "lunarc_execution": execution_context,
        "scientific_authority_delta": "NONE",
        "orbit_completeness": "NOT_CLAIMED",
        "normalization_completeness": "CANNOT_CHECK",
        "d2_input_manifest_sha256": d2_input_manifest_sha256,
    }
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--scope", choices=("d2", "d3", "both"), default="both")
    parser.add_argument("--d2-input-root", type=Path)
    ...
    parser.add_argument(
        "--execution-context",
        choices=("LOCAL_NOT_SUBMITTED", "AUTHORIZED_LUNARC_REPLAY"),
        default="LOCAL_NOT_SUBMITTED",
    )
    args = parser.parse_args(argv)
    if args.scope == "d3" and args.d2_input_root is None:
        parser.error("--scope d3 requires --d2-input-root")
    if args.scope != "d3" and args.d2_input_root is not None:
        parser.error("--d2-input-root is valid only with --scope d3")
    return args
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    terminal = success_terminals[args.scope]
    error: str | None = None
    scope_reports: list[dict[str, Any]] = []
    metrics: dict[str, int] = {}
    d2_input_manifest_sha256: str | None = None
    try:
        if args.scope in {"d2", "both"}:
            ...
            d2_records, d2_input_manifest_sha256 = load_checkpointed_d2_records(args.d2_input_root)
            metrics["d2_checkpoint_records_loaded"] = len(d2_records)
        if args.scope in {"d3", "both"}:
    ...
    if error is not None:
        ...
        receipt["counts_match_frozen_denominators"] = False
        receipt["error"] = error
        receipt["generation_receipt_sha256"] = _digest_of(
            {key: value for key, value in receipt.items() if key != "generation_receipt_sha256"}
        )
