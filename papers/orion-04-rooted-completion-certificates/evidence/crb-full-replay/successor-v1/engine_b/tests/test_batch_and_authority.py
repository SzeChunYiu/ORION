...
def test_successor_slurm_script_uses_frozen_envelope_and_canonical_lane() -> None:
    script = (
        Path(__file__).resolve().parents[1] / "slurm" / "job_orion04_crb_full_replay.slurm"
    ).read_text()
    assert "#SBATCH --cpus-per-task=48" in script
    assert "#SBATCH --mem=192G" in script
    assert "#SBATCH --time=24:00:00" in script
    assert "#SBATCH --partition=hep" in script
    assert "SUBJECT_COMMIT=" + eb.SUBJECT_COMMIT in script
    assert "ORION04_CRB_AUTHORIZED_COMMIT" in script
    assert "orion-04-rooted-completion-certificates" in script
