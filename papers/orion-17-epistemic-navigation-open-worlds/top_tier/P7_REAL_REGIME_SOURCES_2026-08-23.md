# ORION-17 real regime source record — 2026-08-23
...
The announcement explicitly warns RDF consumers to handle previous URIs when reading older crates. This makes the transition useful for ORION-17: JSON keys/value payloads can appear unchanged while canonical RDF identity changes, so value preservation alone is weaker than semantics-aware support transport.
...
PROV already owns version/aspect relations such as `specializationOf` and `alternateOf`, and provenance constraints for evolving entities. ORION-17 does not claim generic version linking. The residual tested here is whether a declared scientific/evidence closure witness survives a representation/ontology regime change, including when a complete alias map is or is not available.
...
The original responsibility uses the three observed class identities `{0,1,2}`. The frozen coarse responsibility is `class0_vs_other`. This is a real observed dataset; ORION-17 uses labels as evidence atoms and does not claim a new wine classifier.
