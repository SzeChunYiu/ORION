...
| P11G's terminal was reachable in both directions under its own freeze | **NEGATIVE / FALSE** | `orion.study.p11.attack_audit`, 48 of 48 admissible worlds | all four scientific gates hold in every world the freeze admits; the survival was fixed before the seed was drawn. `UNWINNABLE_ATTACK_PREDETERMINED_SURVIVAL`. Permanent for P11G and retained; re-asked under P11H, whose hypothesis gates were shown reachable in both directions before its seed was drawn |
...
| P11H's terminal was reachable in both directions before its seed was drawn | **SUPPORTED / PREREGISTERED** | `P11H_PREFLIGHT_ATTAINABILITY_V1.json`, `assess_threshold_panel` + `measure_terminal_reach` | panel `PASS`: nothing `THRESHOLD_UNATTAINABLE`, no hypothesis gate `THRESHOLD_UNCONDITIONAL`, two discriminating; `distinct_terminals` = 2 over 15 admissible draws of which 3 clear every gate — all published before execution |
| the compiled-state low-sample advantage holds at `r=3` against the pooled universal decoder | **NEGATIVE / FALSE** | P11H terminal `P11H_POOLED_UNIVERSAL_ATTACK_PREVAILED` | drawn cells (14,2,3) and (19,3,3); the pool reaches 0.95 at `n=128` in both and `delta64` is +0.0506 against P11G's unedited 0.20 bar. Never state the advantage without a state-width condition |
...
`orion.study.p11.attack_audit` established that P11G's four scientific gates hold in every world its own freeze admits, so its survival was decided before its seed was drawn. That is permanent, it is not repaired here, and the audit still exits `3` on it.

P11H is the successor the record says is the only thing that can retire the finding. It is a new protocol identity: nothing of P11C–P11G is edited, and P11G's scientific payload still digests to `a2b0c33ce3c39e54ca1aa400a2b7d52d019fc4503f6cd5eb726c7b8bbe79a7cc`. P11H registers all three universal-state arms and freezes the best-of-arms combination rule inside its **own** positive gate, draws two protected regimes by its fresh seed from a frozen 2×3 ladder of state widths × bank geometries, and carries P11G's `0.95` and `0.20` thresholds over **unedited** — the P14A → P14C move, where the bars stay and the instrument changes.
