# ORION Paper 2 V2 — maximal widening readiness

**Current authorized scientific terminal:** `P2_NARROWED`.

**Target terminal:** `P2_TRANSFER_SUPPORTED`.

**Rule:** V2 may strengthen the paper only prospectively. It cannot rewrite, delete, or relabel V1 evidence.

## Completed widening infrastructure

- [x] stronger manuscript thesis: **Acquisition Is Not Closure**;
- [x] donor-composable architecture section and Figure P2-7;
- [x] 2026 nearest-work donor matrix covering SAGE, AgentIR, SIEVE, bibliography expansion, HALT, Search-R1, DeepControl, decision-theoretic stopping and stage-separated evaluation;
- [x] prospectively frozen widening protocol;
- [x] machine-readable V2 system/baseline registry;
- [x] 12-case hostile closure-authority suite;
- [x] machine-readable claim-promotion state;
- [x] prospective V2 claim ledger;
- [x] fail-closed V2 validator and contract tests;
- [x] manuscript CI gated on the V2 validator/tests.

## Development lane

The known AutoResearchBench Deep `0/600` result is development-only. Use it to repair query derivation and candidate generation, not as final confirmation.

Development families are frozen in `protocol/P2_V2_SYSTEM_REGISTRY.json`:

1. `ORION_ACQ_LEXICAL_PLUS`;
2. `ORION_ACQ_REASONED_HYBRID`;
3. `ORION_ACQ_STRUCTURED_EXPAND`;
4. `ORION_MAX_COMPOSED`.

A final `ORION_MAX_COMPOSED` identity must be frozen only from development evidence, before confirmatory outcome access.

## Required external promotion sequence

### L1 — `P2_EXTERNAL_MECHANISM_SUPPORTED`

- [ ] denominator-valid external task family;
- [ ] matched resource budgets;
- [ ] strongest runnable donor baseline pressure;
- [ ] direct ablation of the named ORION residual;
- [ ] uncertainty and absolute misses;
- [ ] independent rescore.

### L2 — `P2_EXTERNAL_DISCOVERY_SUPPORTED`

All L1 requirements plus:

- [ ] positive discovery effect under the predeclared margin/decision rule;
- [ ] no frozen stopping-safety violation.

### L3 — `P2_OPEN_WORLD_STOPPING_SUPPORTED`

All L2 requirements plus:

- [ ] direct external/fresh evidence on premature closure or unresolved-obligation errors;
- [ ] fixed-depth, verification/sufficiency and utility continuation baselines;
- [ ] censored/unavailable-route hostile cases retain fail-closed behavior.

### L4 — `P2_TRANSFER_SUPPORTED`

All L3 requirements plus:

- [ ] a separately frozen second evaluation changes at least one independent axis;
- [ ] no tuning on transfer labels/outcomes;
- [ ] directionally consistent benefit and no predeclared safety regression.

## Preferred benchmark route

AutoResearchBench remains the first external target because its released benchmark/evaluation pipeline directly tests Deep target-finding and Wide set-coverage scientific literature discovery. Wide uses set-based IoU/precision/recall over released identifiers; Deep uses a judging path for target-paper verification. Any ORION adapter must preserve scorer-native identity rather than retrospectively translating a non-matching output into an easier metric.

A second transfer axis should be chosen only after the first positive external result and frozen before outcome access. A temporal/rolling scientific-retrieval collection is preferred over another development-adjacent slice because it gives stronger evidence against benchmark-specific tuning.

## Publication strategy

The current narrowed manuscript remains submission-safe while V2 is incomplete. Do not hold or weaken it merely because V2 is ambitious.

If V2 reaches L1/L2, update title/abstract/results/conclusion to an external discovery claim. If it reaches L3, make open-world stopping authority a primary empirical contribution. If it reaches L4, rerun venue fit and promote transfer/generality language.

## Hard blockers that automation must not invent away

- provider/model credentials needed for any selected live or judge-backed campaign;
- any benchmark corpus/evaluator that is not actually released or runnable;
- human author metadata, funding/conflict/CRediT declarations and final AI-use disclosure;
- permanent DOI/archive deposit;
- literature refresh within 14 days of actual submission.

These remain explicit human/external gates, not scientific zeros.

## Done definition

`P2_V2_COMPLETE` means either:

1. the highest evidence-supported L1-L4 terminal is archived, independently verified, and reflected in the manuscript without exceeding it; or
2. the campaign reaches a stable negative / `CANNOT_CHECK` terminal while the existing `P2_NARROWED` paper remains intact and submission-ready.
