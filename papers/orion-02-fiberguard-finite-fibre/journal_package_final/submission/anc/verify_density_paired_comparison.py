#!/usr/bin/env python3
"""Paired test of the STUDY_E Hamming backoff against its own lexical negative control.

STUDY_E reported certified coverage as two bare counts - geometry 32/44 and the
...
supplies it. It reads only the frozen STUDY_E results artifact and recomputes the
...
C_STUDY_E_PMLB_BACKOFF_COVERAGE_IMPROVED_BELOW_GATE stands on its own.

Usage: verify_r23_control_paired_test.py --results <STUDY_E_RESULTS.json> [--emit out.json]
"""
from __future__ import annotations
import argparse, json, random, sys
from math import comb

GEOM = "STUDY_E_HAMMING_BACKOFF_K2"
LEX = "STUDY_E_LEXICAL_BACKOFF_K2_NEGATIVE_CONTROL"
...
def analyse(results: dict) -> dict:
    ...
    return {
        "schema": "ANON.STUDY_E_CONTROL_PAIRED_TEST.v1",
...
def main() -> int:
    ...
    print(text, end="")
    c = out["contingency"]
    print(f"STUDY_E_CONTROL_PAIRED_TEST geometry={out['geometry_certified']}/44 "
