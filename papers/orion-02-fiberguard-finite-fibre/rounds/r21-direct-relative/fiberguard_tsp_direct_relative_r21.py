#!/usr/bin/env python3
"""Frozen ORION-02 R21 TSP direct-relative joint-route experiment.

The scientific contract is FIBERGUARD_TSP_DIRECT_RELATIVE_R21_PROTOCOL.md.
...
SCHEMA = "ORION.FiberGuard.TSPDirectRelative.R21.v1"
...
ALGORITHM_CUTOFF = 3600.0
PAR10 = 36000.0
ALPHA = 0.10
TOL = 1e-9
BOOTSTRAP_REPLICATES = 20_000
BOOTSTRAP_SEED_TEXT = "ORION02_R21_TSP_DIRECT_RELATIVE_BOOTSTRAP_V1"

EXPECTED_BLOBS = {
    "README.md": "bbae808cc2f718b15b379b30ef6a9909933fc3d5",
    f"{SCENARIO}/readme.txt": "54ad2771d1da0b04105b3bffa0bdd8b0b8d8eb72",
    f"{SCENARIO}/description.txt": "923b7e0cb7cf354af398875f536717e21a8c7388",
    f"{SCENARIO}/algorithm_runs.arff": "7ec9db6394c52a2b62d1c44fcd84df47b10ba7b9",
    f"{SCENARIO}/cv.arff": "9d9bee231caaa9cfa017cb176999a36f04d163e5",
    f"{SCENARIO}/feature_costs.arff": "d6649105f201e9155f5646ff5191e46a824a0ee3",
    f"{SCENARIO}/feature_runstatus.arff": "259e00133d47da914d9cc9f7435f9c97a4346bb9",
    f"{SCENARIO}/feature_values.arff": "8c62bc84e319d721691d7b1ef1326f1d08437641",
}
EXPECTED_SIZES = {
    "README.md": 3035,
    f"{SCENARIO}/readme.txt": 1226,
    f"{SCENARIO}/description.txt": 7997,
    f"{SCENARIO}/algorithm_runs.arff": 3990796,
    f"{SCENARIO}/cv.arff": 531764,
    f"{SCENARIO}/feature_costs.arff": 1091481,
    f"{SCENARIO}/feature_runstatus.arff": 841300,
    f"{SCENARIO}/feature_values.arff": 47853407,
}
EXPECTED_STEPS = ("eax_probing", "tspmeta", "ubc_all", "ubc_cheap")
K_VALUES = (1, 3, 5, 9)
ROUTE_K = 9
...
def load_description(path: Path) -> tuple[list[str], dict[str, list[str]]]:
    ...
    if len(algorithms) != 4:
        raise ValueError(f"expected four declared solvers, found {algorithms}")
...
def load_algorithm_runs(
    ...
    ok_histogram: collections.Counter[int] = collections.Counter()
    ...
    for (instance, algorithm), values in grouped.items():
        ...
        if len(values) != 10 or len(set(repetitions)) != 10:
            raise ValueError(
                f"expected ten unique repetitions for {(instance, algorithm)}, "
                f"found {len(values)}/{len(set(repetitions))}"
            )
        ok_count = sum(status == "ok" for _, status, _ in values)
        ok_histogram[ok_count] += 1
        raw_status.update(status for _, status, _ in values)
        successful = ok_count >= 6
    ...
    return (
        ...
        dict(timeouts),
        {
            ...
            "ok_repetition_histogram": {
                str(key): value for key, value in sorted(ok_histogram.items())
            },
            "aggregation": "at_least_6_of_10_ok_then_median_all_10_else_PAR10",
            "par10": PAR10,
        },
    )
...
def cost_for(
    ...
    for step in steps:
        ...
        if not isinstance(value, float) or not math.isfinite(value) or value < 0:
            raise ValueError(f"missing/invalid cost for {instance}/{step}: {value!r}")
        costs.append(value)
...
def execute(subject_repo: Path) -> dict[str, Any]:
    ...
    if legal_pairs_product != legal_pairs_nested or len(legal_pairs_product) != 112:
    ...
    for test_fold in range(1, 11):
        ...
        for steps in representations:
            ...
            for k in K_VALUES:
                ...
                for group, ids in groups.items():
                    ...
                    costs = np.array(
                        [cost_for(x, steps, step_costs) for x in ids], dtype=np.float64
                    )
                    losses = np.array(
                        [
        ...
        for algorithm in algorithms:
            ...
            for group, ids in groups.items():
                costs = np.array(
                    [cost_for(x, ("ubc_cheap",), step_costs) for x in ids],
                    dtype=np.float64,
                )
        ...
        if len(pair_records) != 112 or len(pair_state) != 112:
    ...
    if certified_fraction < 0.01:
        terminal = "C_R21_TSP_DIRECT_RELATIVE_NO_CERTIFIED_LEARNED_COVERAGE"
    elif material:
        terminal = "C_R21_TSP_DIRECT_RELATIVE_JOINT_ROUTE_VALUE"
    elif difference < -TOL:
        terminal = "C_R21_TSP_DIRECT_RELATIVE_STRICT_BUT_NOT_MATERIAL"
    elif abs(difference) <= TOL:
        terminal = "C_R21_TSP_DIRECT_RELATIVE_NULL"
    else:
        terminal = "C_R21_TSP_DIRECT_RELATIVE_ADVERSE"
    ...
    hostile = {
        "complete_cartesian_pairs_evaluated_each_fold": len(all_pair_records) == 1120,
...
def self_test() -> None:
    assert representation_registry() == [
        ("eax_probing", "ubc_cheap"),
        ("tspmeta", "ubc_cheap"),
        ("ubc_all", "ubc_cheap"),
        ("eax_probing", "tspmeta", "ubc_cheap"),
        ("eax_probing", "ubc_all", "ubc_cheap"),
        ("tspmeta", "ubc_all", "ubc_cheap"),
        ("eax_probing", "tspmeta", "ubc_all", "ubc_cheap"),
    ]
    assert [fold_role(10, offset) for offset in (1, 2, 3)] == [1, 2, 3]
    assert nearest_rank([1, 2, 3, 4], 0.95) == 4
    assert len(list(itertools.product(range(28), range(4)))) == 112
