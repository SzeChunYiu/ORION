...
The paper contributes (i) an exact sufficiency condition for finite target maps, (ii) two protected campaigns showing why strong generic verification can remain target-insufficient, and (iii) an information-equivalent control that ties exactly and thereby rules out a proprietary-architecture interpretation.

## 2. A sufficiency boundary for promotion

Let the underlying scientific state be \(s\in S\), the visible representation be \(r(s)\), and the target promotion terminal be \(y(s)\) in a declared output alphabet. The representation induces fibres containing states that are indistinguishable to any downstream decision rule using only \(r\).
...
This theorem also gives a strong equivalence control. If a competing implementation is enriched until it receives the same target-defining scientific coordinates and the same terminal semantics, it should tie extensionally. A tie is therefore expected evidence of representation independence, not a failed superiority test.
...
One prespecified hypothesis in the same campaign is non-discriminating because its eligible family is saturated by construction. That null remains a null. Later work with a more informative interface does not retroactively convert the frozen test into positive evidence.
...
A compensatory rule that allows favorable dimensions to offset missing hard obligations performs substantially worse, illustrating the specific failure mode the contract is designed to expose.
...
This result removes an architectural interpretation. The scientific object is the target-sufficient information and decision relation, not where that information is stored or which software component evaluates it. Any information-equivalent implementation should be able to realize the same terminal map.
...
Provenance, source-aware verification, artifact identity, evaluator auditing, abstention, authorization, assurance cases, and reproducibility infrastructure are established mechanisms. They are treated here as donor capabilities rather than as inventions of the paper.

The residual question appears one layer above those mechanisms: after their native signals are available, what information is still necessary to determine a particular scientific-promotion terminal? The fibre condition provides an exact answer for the declared finite model, while the protected campaigns show how missing target distinctions can survive even in a rich generic product.
...
The registered target coordinates are not proved universally minimal. A different domain may require fewer, more, or differently organized distinctions. A naturalistic successor study remains unresolved because source eligibility, rights, pairing, and independent custody have not all been established at the required scale; it therefore contributes no empirical authority to the current paper.
...
The release package should bind the finite formal definitions, protected-case generators, comparator contracts, exact target tables, result records, and independent reconstruction to one versioned archive. Numeric claims in the abstract and main text should be generated from the same bound result objects used by the reviewer-facing tables.

For TMLR, the submission and supplementary archive must remain double-blind. A named preprint can be maintained as a separate surface without linking identity-bearing material from the anonymous submission.
