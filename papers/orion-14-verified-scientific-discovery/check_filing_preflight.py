...
def parse_checksum_manifest(root: Path, paper: Path, manifest: Path) -> list[Finding]:
    ...
    for line_no, raw in enumerate(manifest.read_text(errors="replace").splitlines(), 1):
        ...
        if not match:
            findings.append(
                Finding(
                    "checksum_manifest_syntax",
                    False,
                    f"unparseable line {line_no}",
        ...
        if target is None:
            findings.append(
                Finding(
                    "checksum_manifest_target",
                    False,
                    f"missing target at line {line_no}: {name}",
                    str(manifest.relative_to(root)),
                )
            )
            continue
        ...
        findings.append(
            Finding(
                ...
                f"{name}: expected={expected.lower()} observed={observed}",
                str(manifest.relative_to(root)),
            )
        )
    findings.append(
        Finding(
            "checksum_manifest_nonempty",
            parsed > 0,
            f"parsed_rows={parsed}",
            str(manifest.relative_to(root)),
        )
    )
    return findings
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="filing/preflight date in YYYY-MM-DD",
    )
    ...
    findings.append(
        Finding(
            "bounded_scientific_terminal",
            EXPECTED_SCIENTIFIC_TERMINAL in readiness_text,
            EXPECTED_SCIENTIFIC_TERMINAL,
            str(readiness.relative_to(root)) if readiness.is_file() else None,
        )
    )
    ...
    findings.append(
        Finding(
            "filing_time_literature_window",
            within_window,
            f"as_of={args.as_of.isoformat()} valid={FRESHNESS_DATE.isoformat()}..{FRESHNESS_LAST_VALID_DATE.isoformat()}",
            str(readiness.relative_to(root)) if readiness.is_file() else None,
        )
    )
    ...
    findings.append(
        Finding(
            "readiness_commit_denominator",
            bool(cited_commits),
            f"cited_full_commits={len(cited_commits)}",
            str(readiness.relative_to(root)) if readiness.is_file() else None,
        )
    )
    for commit in cited_commits:
        ...
        findings.append(
            Finding(
                "readiness_commit_resolves",
                ok,
                f"{commit}: {out or 'commit exists'}",
                str(readiness.relative_to(root)) if readiness.is_file() else None,
            )
        )
    ...
    findings.append(
        Finding(
            "tracked_paper_inventory",
            bool(tracked),
            f"tracked_files={len(tracked)}",
            str(paper.relative_to(root)),
        )
    )
    ...
    current_sources = [
        ...
        for p in tracked
        if p.is_file()
        and p.suffix.lower() in {".tex", ".md", ".bib"}
        and any(part in {"manuscript", "journal_package"} for part in p.parts)
    ]
    ...
    findings.append(
        Finding(
            "submission_source_placeholders",
            not placeholder_hits,
            "none" if not placeholder_hits else ", ".join(placeholder_hits[:50]),
        )
    )
    ...
    manifest_paths = sorted(
        ...
        and "journal_package" in p.parts
    )
    findings.append(
        Finding(
            "journal_checksum_manifest",
            bool(manifest_paths),
    ...
    state_files = sorted(
        ...
        and any(token in p.name.upper() for token in ("MANIFEST", "STATE", "CLOSURE"))
    ...
    for path in state_files:
        ...
        findings.append(
            Finding(
                "current_package_not_stale",
                not bad,
                "no stale terminal" if not bad else f"stale_states={bad}",
    ...
    result = {
        ...
        "human_filing_attestations_required": [
            ...
            "acknowledgements",
            "AI_use_declaration",
            "no_parallel_submission",
            "real_portal_submission_id",
        ],
    ...
    if args.write_md:
        ...
        lines = [
            "# ORION-14 repository filing preflight",
            "",
