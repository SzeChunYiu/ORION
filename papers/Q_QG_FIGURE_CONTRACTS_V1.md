# ORION-Q / ORION-QG main-figure contracts V1

Date: 2026-08-21
Method: `nature-figure` scientific-contract pass.

Every proposed display below has a scientific job, a data/evidence source, an authority boundary and a forbidden visual shortcut. No figure may create a stronger claim than the manuscript/claim ledger. Exact target production dimensions are deferred to venue-specific packaging; scientific content and accessibility are fixed here first.
...
7. **Timings are descriptive unless a frozen inferential design exists.**
8. **Every quantitative figure has a machine-readable source table generated from committed receipts.**
9. **No screenshot of terminal output as a scientific figure.** Receipts are data sources, not visual design.
10. **No figure copied from an analogue paper.** Analogue papers may guide evidence roles only.
...
**Panel structure:**
- A: nested families `D/R6L ⊂ D+ ⊂ D++ ⊂ unrestricted`;
- B: exact counterexample I `8 < 9` opens D→D+;
- C: exact counterexample II `5 < 6` opens D+→D++;
- D: all-n R6S theorem closes `C_DP=C_D++`;
- E: later QG5/QG7 counterexamples refine named subfamilies *inside* D++ without changing the support theorem.

**Sources:** R6N, R6O, R6P, R6S, QG5, QG7 receipts.

**Authority label:** counterexample / finite evidence / all-n theorem shown separately.
...
**Content:** explanatory F2^2 class diagram for the R6S exchange, the proper zero-sum subset at support≥3, and the four exact failing support-2 class tuples.

**Sources:** `MAX_R6S_ALL_N_COMPOSITION_RESULTS.json` Lemma B/E fields.

**Numbers shown:** 43,688 class tuples checked; 18,432 Lemma-E cases; zero violations for the load-bearing inequality; four support-2 failing patterns.
...
**Panel A:** split-anchor, spread-Tag witness with cost 8 vs 9.
**Panel B:** central support-two frame-for-Tag borrow with cost 5 vs 6.
...
**Content:**
- finite R6Q panels and 15-match prospective subject as bounded evidence;
- later QG5 `10<11` and QG7 fourth-regime witnesses as explicit refutations of universal closed-form extrapolation;
- R6S support theorem shown as unaffected.

**Forbidden:** ROC/AUC-style presentation; there is no stochastic learned classifier inference here.
...
States: frozen hypothesis → executed result → `{positive, negative, donor-absorbed, cannot-check}` → responsibility localization → donor first refusal → successor freeze or stop → replay/verification → retained lineage.
...
**Source:** machine-readable `Q2_TRANSITION_GRAPH_V2.json` plus future total eligible-chain manifest/checker.

Node encoding: receipt terminal class, not rhetorical importance.
Edge encoding: predeclared successor authority.
...
**No success rate.** The goal is process coverage/retention, not to imply negatives are failures or positives are wins.
...
Show immutable question packet → independent Lane A and Lane B → sealed diagnosis/move receipts → later external/frontier outcome → coordinate-by-coordinate scoring.
...
Cells must display categorical labels as text/symbols; color is secondary.
...
Center: typed/scoped partial-knowledge state.
Six spokes: unknown feasibility/VOI; stale failure reopening; interval verification; transport chain; experiment selection; remint/representation transport.

Each spoke names:
- identical visible information across compared arms;
- rule difference;
...
Examples:
- utility in N4-A/B/E/F3;
- scalarized regret in N4-C;
- laundering recall/FPR in N4-D.
...
- exact tie/donor absorption where it does not.
...
Source: `INFORMATION_PARITY_AND_ARTIFACT_MAP_V2.md` plus future machine-readable parity manifest.
...
Five components:
1. donor-optimal region;
2. elementary trade/counterexample families;
3. support/sufficiency bounds;
4. membership/boundary predicate or representation;
5. prospective forecast/refutation.
...
- all-n support-two theorem;
- finite-domain exactness identity/predicate;
- forecast-only/unverified rows.
...
Caption: 9,545 exact + 1 refutation; the single exact witness is logically sufficient to refute universal identity, irrespective of the aggregate rate.
...
# Figure regeneration deliverables

Before a paper is submission-ready:
- [ ] one source-data table/JSON/CSV per figure;
- [ ] one deterministic generation script per figure or one paper-level figure script;
- [ ] script records exact input receipt hashes;
- [ ] SVG/PDF vector output where appropriate, plus journal-required raster only when needed;
- [ ] final-size visual inspection;
- [ ] captions state n/denominator, evidence class and claim boundary;
- [ ] accessibility check (non-color redundancy, readable labels, alt-text/long description where target requires it);
- [ ] no main claim depends only on supplementary display.

No AI-generated schematic is needed for the quantitative main figures. If a graphical abstract is later requested by a venue, it is downstream communication material and must be reviewed against the manuscript evidence contract.
