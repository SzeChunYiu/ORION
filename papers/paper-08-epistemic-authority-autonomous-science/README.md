...
How can local authorization, delegation, provenance, verifier certificates, release gates and evidence ledgers be composed without laundering local permission into unsupported scientific authority?
...
### Correction to the old “169 heterogeneous chains” reading

The original X4 artifact reports 169 successful ordered donor-pair compositions and 169 widening countermodels. The mechanized interpretation shows that **169 is not 169 heterogeneous scientific-chain results**:

- the shipped loop iterates 13×13 donor names but its body ignores both donor variables;
- all 169 successful pairs compose to **one distinct clean/reflexive state** under the shipped interpretation;
- reading the thirteen families as scientifically type-distinct without protected bridges yields 13 successes, not 169;
- 8/8 deliberately wrong composition operators reproduce the 169 success count exactly;
- only the stronger exhaustive composition-soundness identity discriminates all eight wrong operators.

Therefore the pair count is retained as historical artifact multiplicity, not as breadth evidence.

### Stronger composition evidence

The interpretation checks an exact reduced state space:

- 3,072 states reduced to 192 verdict classes;
- 36,864 representative pairs checked;
- representing 9,437,184 state pairs;
- 0 unsound pairs;
- 81 discharging composed pairs over 9 discharging representatives;
- every widening hop blocks;
- both terminal verdicts are exercised;
- every tested wrong composition operator changes the soundness count.

This is the meaningful composition result. It tests the composition operator, not empirical deployed-agent performance.

### Real evidence-discharge study

`top_tier/P8_REAL_EVIDENCE_DISCHARGE_RESULT_RECEIPT_V1.md` binds 20 frozen real-domain cases across empirical, formal, multiple-support and systems settings:
...
- full support revocation blocks;
...
No deployed-agent safety, no universal minimality of the registered type coordinates, no global literature saturation, no claim that native action denial is scientific falsification, no inherent centralization advantage, no empirical claim from the 169 multiplicity count, and no `TOP_TIER_SUBMISSION_READY` self-promotion.
