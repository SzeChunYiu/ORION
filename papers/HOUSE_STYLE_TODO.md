...
cross-references to "Paper II", "ORION-12-X" or a shared programme.

## What that means per manuscript

1. **Remove the system name from the body.** In ORION-12 it appears 38 times as the
...
2. **No machine tokens in prose.** `CANNOT_CHECK`, `TIER_B_committed`,
   `P2_WIDE_EXTERNAL_CANNOT_CHECK` and similar are internal vocabulary. ORION-12's
...
3. **No repository paths or artifact filenames in the narrative.** ORION-12's body
...
5. **No table of open-literature checks.** Discuss the neighbouring work and
   cite it. (Present in ORION-11's `sections/06-related-work-boundary.tex`; ORION-12 carries
   the prose equivalent inside "Nearest work".)

6. **Internal identifiers out of the prose.** "ORION-12-X" is an experiment; call it
   one and give it a name a reader can hold, e.g. "the exact-contract battery".

## Scope

ORION-12 first, reviewed, then the same treatment paper by paper as each is finished.
