...
\urlstyle{same}
\hypersetup{
  pdftitle={A Width-One Corridor for Generalized Davenport Constants of \textbackslash(C\_5\^{}3\textbackslash)},
...
\title{A Width-One Corridor for Generalized Davenport Constants of \(C_5^3\)}
...
constants of \(C_5^3\) satisfy \(5k+10\le D_k(C_5^3)\le5k+11\) for every
\(k\ge4\).
...
The exact value remains \(D_4(C_5^3)\in\{30,31\}\). A larger bounded
...
here. The exact values used below are
...
length 32, whereas Property C for \(C_5^3\) forces every length-32
...
\textbf{Computer-assisted proof.} Multiplicity at most four first
...
Both engines reject a branch exactly when adding a term creates a
zero-sum subsequence of one of the lengths one through five; one stores
explicit byte reachability by weight and group sum, while the other
stores translation masks in a 128-bit representation. Their source,
build instructions, expected rows, and machine-readable results are
included as ancillary files. Thus every support stratum through ten is
exhausted. ∎
