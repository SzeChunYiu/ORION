...
\title{Restore-Sensitive Certificate Realization: Normal Forms and Intrinsic Support in Quantum Compilation}
\author{Sze Chun Yiu}
\date{}
\hypersetup{pdftitle={Restore-Sensitive Certificate Realization: Normal Forms and Intrinsic Support in Quantum Compilation},pdfauthor={Sze Chun Yiu}}
\begin{document}
\maketitle
\begin{abstract}
...
\end{abstract}
\noindent\textbf{Keywords:} quantum compilation; Pauli strings; support normal forms; certifiable support; intrinsic support; sparse optimization
...
\section*{2. Intrinsic and certifiable support}\label{intrinsic-and-certifiable-support}
...
\section*{3. Fixed alphabets and zero-sum-free sequences}\label{fixed-alphabets-and-zero-sum-free-sequences}
...
\section*{4. Simultaneous deletion theorem}\label{simultaneous-deletion-theorem}
...
\textbf{Theorem 2 (simultaneous alphabet ceiling).} Every admitted instance has an exact optimum \(x^\star\) such that, simultaneously for every constrained generator \(R\),
...
\section*{5. Binary specialization and the rank-only calculus}\label{binary-specialization-and-the-rank-only-calculus}
...
\textbf{Theorem 3 (binary generated-span identity).} Let \(A\subseteq \mathbb{F}_2^d\), and let \(H=\langle A\rangle\). Then
...
\textbf{Corollary 5 (exact rank-only budget).} For every finite \(A\subseteq\mathbb{F}_2^d\),

\[
\beta_{\mathrm{rank}}(A)=\operatorname{rank}(\langle A\rangle).
\]

\textbf{Proof.} Theorem 3 bounds every terminal word by the rank of the generated span and provides a basis word attaining that length. The identity derivation leaves the basis word terminal, so no smaller uniform conclusion holds in the declared state language. \(\square\)
...
\section*{6. Phase-quotiented MultiTag-TARE parity grammar}\label{phase-quotiented-multitag-tare-parity-grammar}
...
\subsection*{6.1 Deletion semantics}\label{deletion-semantics}
...
\subsection*{6.2 Restore incidence and objective}\label{restore-incidence-and-objective}
...
\section*{7. Restore sensitivity and the MultiTag normal form}\label{restore-sensitivity-and-the-multitag-normal-form}
...
\textbf{Lemma 6 (one-argument sensitivity).} Replacing one argument of \(F_b\) can increase its value by at most \(b-1\), and this bound is attained.
...
\textbf{Theorem 7 (MultiTag support normal form).} Under the stated semantic and incidence contracts, if \(\mu\geq (b-1)t_R\), every admitted instance has an exact optimum satisfying
...
\textbf{Proposition 8 (matched equality control).} Consequently
...
\section*{9. Dependent-triple family: a rank-only separation}\label{dependent-triple-family-a-rank-only-separation}
...
\textbf{Theorem 9 (dependent-triple separation).} The exact separation is
...
\section*{10. Declared product amplification}\label{declared-product-amplification}
...
\textbf{Proposition 10 (componentwise product budgets).} For every integer \(t\geq 1\),
...
\subsection*{10.1 Direct-enumeration corollary}\label{direct-enumeration-corollary}
...
\section*{11. Discussion and relation to prior work}\label{discussion-and-relation-to-prior-work}
...
\section*{12. Reproducibility and adverse transfer record}\label{reproducibility-and-adverse-transfer-record}
