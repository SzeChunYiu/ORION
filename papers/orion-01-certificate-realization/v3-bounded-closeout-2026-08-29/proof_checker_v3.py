...
def sequence_sum(sequence: Sequence[T], add: Add[T], zero: T) -> T:
    total = zero
...
def zero_sum_indices(
    sequence: Sequence[T], add: Add[T], zero: T, *, proper: bool = False
) -> tuple[int, ...] | None:
...
def is_zero_sum_free(sequence: Sequence[T], add: Add[T], zero: T) -> bool:
    return zero_sum_indices(sequence, add, zero) is None


def zsf_bruteforce(alphabet: Sequence[T], add: Add[T], zero: T, upper: int) -> int:
    """Return the exact zsf value when ``upper`` is a proven forcing length."""
    best = 0
...
def delete_indices(sequence: Sequence[T], indices: Sequence[int]) -> tuple[T, ...]:
    removed = set(indices)
    return tuple(value for i, value in enumerate(sequence) if i not in removed)
...
def check_f2_restricted_bound() -> dict[str, object]:
    ...
    for dimension in range(1, 4):
        ...
        for mask in range(1 << len(nonzero)):
            ...
            value = zsf_bruteforce(alphabet, lambda a, b: a ^ b, 0, dimension + 1)
            assert value <= dimension
            if basis.issubset(alphabet):
                assert value == dimension
                basis_equalities += 1
            families += 1
    assert families == 138
...
def check_cyclic_rank_boundary() -> dict[str, object]:
    ...
    for modulus in range(2, 11):
        add = lambda a, b, n=modulus: (a + b) % n
...
def check_restore_sensitivity() -> dict[str, object]:
    comparisons = 0
    sharpness_cases = 0
    paulis = (0, 1, 2, 3)  # identity and three distinct nonidentity letters
    for arity in range(2, 8):
        ...
        witness = [1] * arity
        witness[0] = 2
        assert restore_functional(witness) - restore_functional([1] * arity) == arity - 1
        sharpness_cases += 1
...
def check_deletion_terminal_equivalence() -> dict[str, object]:
    fixtures: list[tuple[str, tuple[int, ...], Add[int], int, int, int]] = [
        ("F2^2_full", (1, 2, 3), lambda a, b: a ^ b, 0, 2, 4),
        ("F2^3_basis", (1, 2, 4), lambda a, b: a ^ b, 0, 3, 5),
        ("Z5_singleton", (1,), lambda a, b: (a + b) % 5, 0, 4, 6),
    ]
    for _name, alphabet, add, zero, ceiling, maximum_length in fixtures:
        ...
        for length in range(ceiling + 1, maximum_length + 1):
            for original in itertools.product(alphabet, repeat=length):
                if sequence_sum(original, add, zero) == zero:
                    continue
                sequence = tuple(original)
                original_total = sequence_sum(sequence, add, zero)
                while len(sequence) > ceiling:
                    indices = zero_sum_indices(sequence, add, zero, proper=True)
                    assert indices is not None
                    reduced = delete_indices(sequence, indices)
                    assert len(reduced) < len(sequence)
                    assert sequence_sum(reduced, add, zero) == original_total
                    sequence = reduced
                assert is_zero_sum_free(sequence, add, zero)
                assert len(sequence) <= ceiling
    return {
        "name": "proper_deletion_and_terminal_equivalence",
        "status": "PASS",
...
def check_global_descent() -> dict[str, object]:
    ...
    for left, right, left_ceiling, right_ceiling in fixtures:
        ...
        while True:
            violating = next(
                (i for i, sequence in enumerate(state) if len(sequence) > ceilings[i]),
                None,
            )
            if violating is None:
                break
            indices = zero_sum_indices(state[violating], lambda a, b: a ^ b, 0, proper=True)
            assert indices is not None
            other = 1 - violating
            ...
            previous_phi = phi
            steps += 1
            assert steps <= sum(map(len, (left, right)))
        assert all(len(sequence) <= ceiling for sequence, ceiling in zip(state, ceilings))
    return {
        "name": "simultaneous_global_fixed_point_descent",
        "status": "PASS",
        "scope": "three two-generator fixtures under the stated monotonicity premise",
        "fixtures": len(fixtures),
    }
...
def check_soundness_and_product_arithmetic() -> dict[str, object]:
    ...
    for k1, b1, k2, b2 in fixtures:
        assert k1 <= b1
        assert k2 <= b2
        product_kappa = k1 + k2
        product_budget = b1 + b2
        assert product_kappa <= product_budget
    # Frozen bounded comparison from the manuscript definitions.
    assert 2 == 2
    assert 1 < 5
    return {
