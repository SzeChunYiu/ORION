...
**Status:** `EXECUTION_FROZEN` / candidate launch blocked pending signed-green main attestation  
**Outcome accessed:** `false`

The earlier V1 420-case protected campaign remains valid evidence for its frozen subject `46977ea...`, but that subject predates the later authority support/influence/host-telemetry repair. The 39-case live GLM arm is exploratory only and is not publication-authorizing evidence because its live labels/denominators and hidden-family execution path failed a recursive audit. V2 therefore uses a fresh post-repair split and never reuses either outcome for tuning.
...
- [x] **12. Immutable V2 binding artifact recorded**
  ...
  - `status=EXECUTION_FROZEN`, `outcome_accessed=false`, `campaign_execution_authorized=false`
- [ ] **13. GitHub-verified main freeze commit + green exact-main CI**
  - this checklist, `PROTECTED_RUN_BINDINGS_V2.json`, and `p4_protected_campaign_v2.yml` must merge to `main`
  - merge commit must have `verification.verified=true` / reason `valid`
  - ordinary CI must pass on that exact main SHA
  - **no `CAMPAIGN_TRIGGER_V2.txt` is included in this freeze change**
  - a later trigger-only PR records the exact freeze SHA + CI run; the campaign workflow independently verifies both via GitHub API before handoff
...
Any change to the subject, hidden set, split, V2 comparator/ablation config, frozen harness files, epoch, or eligible-opportunity definitions voids this freeze. Any protected-label access by the candidate/comparator jobs, external IP connection during scored execution, failure to verify the signed freeze commit/CI, or reuse of this split after an outcome-dependent subject repair also voids the campaign.
