...
H3 was not supported, and the reason is a property of the battery rather than of the systems. Every one of the eleven panel members, ORION included, correctly selected \cannot on all 30 genuinely insufficient-evidence cases, so the difference is zero with 95\% CI $[0,0]$. The correct-\cannot\ rate takes a single distinct value across the panel, and the ablations do not report it at all, so no configuration measured in this study places the metric anywhere other than $1.0$. The comparison therefore has no resolving power on this axis: a difference could not have been observed had one existed. This is reported as an inability to discriminate, not as evidence that no difference exists. Table~\ref{tab:p4_t3} reports abstention and false-negative cost rather than treating the null as a win.
...
saturates. Clean coverage is $1.0$ for all eleven panel systems and for all eight ablations,
