# A Compositional Calculus for Cross-Domain Scientific Authority — V5

V5 is V4 with one correction and no change to any theorem, count, model or
reported result. V4's abstract described the X4 model as exhausting "39,936
exact authority states" while, three lines later, stating plainly that the
distinct state count is 3,072 and that 39,936 is the number of evaluations --
the thirteen donor families being a replication factor, not a state dimension.
Theorem V3.10 carried the same conflation. Both now report 3,072 states and
39,936 evaluations, matching the exact-bounded-support section, which was
already correct. The arithmetic is unchanged: 3,072 x 13 = 39,936, and the
239,616 donor-sibling pairs are 3,072 x 78.
...
A decentralized donor product supplied with the same scientific type, narrowing, blocker, support-family, coercion, and composition rules agrees extensionally with ORION-18 over all **3,072** registered states, across all **39,936** donor-replicated evaluations.
