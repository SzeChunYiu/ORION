...
Every abstract, results, limitations and conclusion claim in this revision maps to an archived artifact. A row is `SUPPORTED` only when the cited artifact exists and the stated authority matches that artifact. **H1–H4 remain `CANNOT_CHECK`.** The diagnostic attribution archive is **21/24**, not a perfect score. Merged discovery/failure successor results are bounded negative/subsumption evidence and do not upgrade P5 empirical authority.
...
| Abstract / successor interface | Scoped failure knowledge is useful but standard failure-learning/memory parents are sufficient for the tested incremental effect; broad tension/imagination/concept labels decompose; the protected zero-error ORION-JUMP candidate ties the verified representation-regime parent on both disjoint splits. | MERGED BOUNDED NEGATIVE / SUBSUMPTION; NO P5 H1-H4 AUTHORITY | `development/jump-programme-closure-v1/README.md`; `research/extensions/orion-jump-recursive-atoms/zero_error_jump/ZERO_ERROR_JUMP_EXPECTED_V2.json`; `manuscript/sections/04a-recursive-failure-discovery-interface.tex` | `SUPPORTED` |
...
| Claim boundary | The #454 structural-assimilation programme is frozen as a non-authorizing process. WorldEvolver and Self-Revising are `MECHANISM_COMPARATOR_BOUND`; MDA remains `CANNOT_CHECK` under `MDA_PRIMARY_FULL_TEXT_NOT_SOURCE_GROUNDED_2026-08-19`. No comparator is thereby an official reproduction. | SOURCE/STRUCTURAL BINDING BOUNDARY | `research/self-orion-v3/BASELINE_STRUCTURAL_BINDINGS_V1.json`; `development/structural-assimilation-454-v1/CLOSURE.md` | `SUPPORTED` |
| Claim boundary | T8 implements a separate prospective revision-level V3 benchmark with candidate/protected custody, matched mechanism comparators and feedback interventions, an independent scorer, and a confirmatory preflight that remains `CANNOT_CHECK` while final identities and the MDA structural baseline remain unresolved. The public development panel is instrumentation only and supplies no H1–H4 evidence. | IMPLEMENTED PROSPECTIVE PROTOCOL ONLY; SCIENTIFIC OUTCOME NOT RUN | `protocol/SELF_ORION_V3_REVISION_LEVEL_PROTOCOL_V1.json`; `research/self-orion-v3/development/PROTECTED_DEVELOPMENT_SUITE_V1.json`; `research/self-orion-v3/BASELINE_STRUCTURAL_BINDINGS_V1.json`; `src/orion/study/p5/revision_level_v3_preflight.py` | `SUPPORTED` |
| Conclusion | The merged failure/atom/Jump results require parent subsumption rather than preserving unsupported new-operator labels; they do not fill P5 H1–H4. | MERGED BOUNDED NEGATIVE / GOVERNANCE CONSEQUENCE | `manuscript/sections/08-claim-boundary.tex`; `papers/ADDON_INTEGRATION_MATRIX_2026-08-19.md` | `SUPPORTED` |
...
No row marked `CANNOT_CHECK` may be rewritten as positive evidence. The 21/24 attribution result is descriptive-only and does not support H1–H4. The V3 T1–T7 mechanics are implementation/formal contracts only, and the T8 development fixture is instrumentation only. The merged successor results are scientific outcomes for their own bounded questions, but their P5 consequence is subtractive: standard-parent sufficiency, decomposition and parent equivalence remove unsupported Self-ORION ownership rather than establish fresh-task self-improvement.

T8 confirmatory execution remains blocked until final subject/split/evaluator/protected-suite identities are concrete and every required comparator is structurally bound; at present MDA remains the explicit structural `CANNOT_CHECK` baseline. A stale perfect-score report is an `EXECUTION_IDENTITY_BOUNDARY_MIXUP`-class failure (`research/failures/2026-08-p5-live-artifact-branch-identity-mismatch/`) and is refused by `python -m orion.study.p5.tables`.

A new live P5 result remains `CANNOT_CHECK` in this revision: the original P5 V1 execution bindings remain unbound, and the additive V3 protocol is implemented but not confirmatory-frozen or executed.
