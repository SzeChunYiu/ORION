...
\subsection{Prospective successor design}
...
The successor experiment is a separate, protected-custody revision-level benchmark rather than a rewrite of the frozen design above. Its development-only cause-confusable panel exposes the same candidate-visible symptom under different protected revision responsibilities; it evaluates matched no-revision, broad-self-edit, method-open-only, world-model, representation-regime, generic-diagnosis and fully composed policies, and it includes feedback-permutation and no-feedback controls. A separate scorer evaluates protected revision responsibility, false broad revision, unresolved decisions, and later fresh-transfer and harm records. This establishes only that the experiment can be run and audited. Its confirmatory preflight is undetermined: the final subject, split, evaluator and protected-suite identities are unbound, and the load-bearing donor comparators do not yet have frozen structural bindings. The development panel therefore supports no hypothesis and closes no study.

\subsection{Wide revision-level successor}

The widest frozen successor preserves the revision-level target rather than
retreating to cause-label accuracy. It plans 768 independent,
source-disjoint project/issue-family clusters: twelve clusters in each of eight
domains crossed with eight revision classes (evidence, measurement,
within-class model, model-class expansion, representation regime, execution,
evaluator, and unresolved). Episodes are blocked so that different protected
revision responsibilities share the same candidate-visible symptom. Six
comparator families are frozen, and the global endpoint requires the correct
minimal revision, preservation, protected fresh-transfer success, and every
non-compensatory harm gate. The planning model targets joint power 0.90 for an
absolute 0.15 planning difference beyond a 0.05 superiority margin at
familywise level 0.05. These are design quantities, not observed effects.

The three residual-error studies are an earlier identification stage. Their
fixed two-factor interventions must separate retrieval from representation,
environment from implementation, and representation from method basis without
using the protected final panel. A successor interface is eligible for the wide
campaign only if its registered tests cover every cross-decision pair in its
development fibre; that structural cover is necessary but does not predict a
positive external outcome. Source snapshots, final split commitment, the
strongest runnable external comparator, subject/model/epoch identities,
independent fresh-transfer evaluators and external verifier custody remain
missing. The wide design is therefore ready for those bindings, not a completed
scientific study.
