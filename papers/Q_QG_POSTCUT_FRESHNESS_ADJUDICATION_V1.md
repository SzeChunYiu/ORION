...
Q1's load-bearing theorem is about the **R6M three-block grammar under the raw support objective**. Post-cut QG-9/QG-16 concern the distinct R6I rank-2 dependent-triple grammar.

QG-6 independently recovers the R6M support-2 scale from a rank-2 conserved-syndrome quotient, but it does not strengthen Q1's theorem authority beyond R6S and carries no novelty authority. It is useful explanatory/companion evidence for QG1, not required for Q1's support-ceiling paper.

**Disposition:** keep Q1 on the original evidence cut. Optionally cite QG-6 in Discussion as later independent structural interpretation only if doing so does not distract from the exact R6S proof.

### Q2 — NO REOPEN FOR SCIENTIFIC CLAIM

The QG-9 ladder is an additional example of Q2's successor discipline: progressively stronger proof systems first stop at looser support bounds, then whole-system Tag relocation closes support 1. However Q2 already has enough independent negative/donor/successor chains to establish its bounded methodological case.
...
**Disposition:** do not reopen Q2 merely to add another success story. Mention post-cut continuation only in a dated repository note or future revision after the transition inventory is regenerated.

### Q3 — PROSPECTIVE FREEZE REMAINS VALID, WITH CUSTODY RECHECK REQUIRED

The reviewed current-main additions do not include a QG7D or QG15C result receipt. QG-17 has a protocol but no result receipt and is not one of the two frozen Q3 additional instances.

**Disposition:** Q3-V1/QG-7d and Q3-V2/QG-15c remain prospectively frozen relative to the original cut and the reviewed current-main point. Before either Lane A/B execution, recheck that no result-bearing QG7D/QG15C receipt has appeared on any authority-bearing branch intended to supply the deferred outcome. If an outcome becomes visible before instrument receipts freeze, that instance is contaminated and must terminate rather than be backdated.

### Q4 — NO REOPEN

The post-cut additions are quantum compiler theory and do not change Q4's exact-synthetic typed/scoped-state experiments or their donor boundary.

### QG2 — NO REOPEN

QG2's load-bearing object is static forecasting for the R6M/TARE support-two family and the exact 10<11 refutation/repair sequence. QG-9/QG-16 concern R6I and do not alter the R6M R6S theorem, the QG5 refutation, or the B-prime/F2 repair.

QG-6's R6M rank-2 quotient is an explanatory cross-check, but QG2's certificate does not require it.

**Disposition:** keep QG2 on original cut; QG-6 may be a companion structural citation, not certificate authority.

### QG1 — **REOPEN REQUIRED**

QG1 is explicitly a cross-family regime-geometry/framework paper. The new R6I results materially change the strongest available story:
...
These additions strengthen QG1's framework claim that a regime-geometry object contains multiple logically distinct components: tight intrinsic support, proof-system-dependent upper bounds, objective-indexed validity regions, exact counterexamples and representation-dependent boundary descriptions.

**Disposition:** QG1 must receive a V3/current-main synchronization before second review. Do not submit V2 as the final scientific manuscript.

## Publication-cut policy after adjudication

A single portfolio cut is no longer scientifically optimal.

- Q1/Q2/Q3/Q4/QG2 retain the original frozen cut for their current manuscript evidence, with explicit later-work freshness notes where needed.
- QG1 receives a paper-specific refresh cut at `main@c5ba39fef4f25c46de5fb69bf07f50530f4693ca` and must bind only the named new receipts above plus its previously authorized evidence.
...
By that rule, only QG1 reopens in this freshness round.
