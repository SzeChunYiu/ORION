"""Execute frozen P13B in two fresh subprocesses."""
...
OUT = HERE / "P13B_AUTHENTICATED_CERTIFICATE_CORRUPTION_RESULT_V1.json"
...
def _supervise(path: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="p13b-replay-") as directory:
        ...
        if not all(run.returncode == 0 for run in runs):
            raise RuntimeError("P13B protected worker failed")
