# ORION-23 top-tier promotion V1 — Responsibility-Scoped Sufficiency and Certified Reuse
...
Handwritten digits are shared programme infrastructure (ORION-19's cubic/`D-A` diagnostic and ORION-21's 64→32 compiler use the same public dataset). The frozen quantities owned here are ORION-23's own 17,970 parity→exact-digit responsibility episodes and CNF transport witnesses. The parity responsibility shares its protected target family with ORION-21's T11.2 parity checker witnesses, but the ORION-23 episodes and the ORION-21 witnesses are separately frozen corpora with no shared protected outcomes.
...
Define how a responsibility certificate transports under representation/compiler version change, source/evidence update, context/epoch change, responsibility upgrade and loss of raw-recovery availability. Compose with ORION-16/ORION-17/ORION-18 only through frozen interfaces; ORION-23 owns the state-reuse contract, not their general transition/transport/authority claims. Within-regime source-level revocation semantics (revoking a source while independent derivations survive) are donor-owned by ORION-18, extended here only by the responsibility-indexed part (`papers/SYNC_CONTRACT.md`). The digits responsibility upgrade is the question-changes-under-fixed-map dual of ORION-17's wine coarsening (map-changes-under-fixed-question); ORION-17 is cited as donor of the transport-witness machinery, and ORION-23's residual is the query-level sufficiency/reuse contract.

Recovery and reopen costs are priced through ORION-21's optionality/recovery costing (donor, `papers/SYNC_CONTRACT.md`); ORION-23's endpoint remains the non-compensatory unsafe-reuse contract — optionality cost-optimality alone certifies nothing here.
