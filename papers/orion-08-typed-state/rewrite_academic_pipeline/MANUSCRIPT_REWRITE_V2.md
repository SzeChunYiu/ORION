...
We answer this through six exact-synthetic studies. Each freezes a world, a decision problem, a comparator, and a scoring rule. Primary non-oracle arms see matched factual information; the manipulated object is the relation that makes some facts applicable, reusable, or decision-relevant. Each family also contains a no-value or hostile regime so that a mechanism is not rewarded merely because the benchmark was built around it.

The paper contributes an evaluation design as much as a set of results. It separates **state-representation value** from **policy novelty**. If a strong known decision rule supplied with the same structured state reproduces the gain, the scientific residual belongs to the state distinction rather than to the proposed allocation policy.
...
This restriction matters. A method that wins because it sees more information has not isolated the value of epistemic structure. Likewise, a policy that beats only a weak baseline should not retain a broad algorithmic novelty claim after a stronger donor receives the same structured inputs.

The six studies were not preregistered as components of one grand theory. They were frozen and executed separately, then synthesized after completion. We therefore report their original outcomes at their native scales. Bootstrap intervals reconstructed from fixed-seed paired differences are secondary uncertainty summaries; exact constructed-chain studies are reported as complete counts.
...
The result is not a new value-of-information algorithm. It isolates type as information that can alter the output of an otherwise unchanged planner.

## 4. Applicability scope prevents unnecessary reopening
...
A stronger deterministic proxy, however, performs much closer. The paired difference relative to that proxy is 0.277 with a 95% interval of 0.119 to 0.412. This donor result changes the interpretation. Most of the large gain belongs to avoiding entropy-driven acquisition, not to a broad claim that the proposed policy dominates reasonable decision-aware heuristics.

The residual contribution is therefore the decision-role representation and the matched-information test that exposes when policy novelty has been absorbed.
...
This donor-absorption rule is part of the contribution: representation value and policy novelty should not borrow authority from one another.
...
Effect scales differ across studies and should not be pooled. The bootstrap intervals are secondary analyses of fixed-seed episodes rather than independent replications. One applicability comparison remains compatible with the conservative never-reopen policy and is reported that way.

The decisive extension is a real-domain matched-information study in which competing systems receive genuinely equivalent scientific evidence while the epistemic bindings are manipulated or ablated prospectively.

## 12. Reproducibility and availability

The release package should expose each study's frozen protocol, exact or fixed-seed generator, comparator contract, outcome record, and paired analysis. Reviewer-facing materials should use scientific descriptions rather than development-repository identifiers and should make the six studies independently reproducible.

For TMLR, the submission manuscript and supplementary archive should remain anonymized even if a separate named preprint exists.

## 13. Conclusion

Scientific decisions can depend on more than factual payload. Across six exact-synthetic matched-information studies, explicit type, applicability, decision-role, lineage, and transport relations alter decisions in the regimes where those relations carry value. Strong donors and no-value controls simultaneously narrow the interpretation: the evidence supports a bounded claim about epistemic state, not universal policy superiority or real-agent transfer. The benchmark's central discipline is to let structured state earn only the scientific residual that remains after information parity and donor absorption are enforced.