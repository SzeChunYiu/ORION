...
Third, the intervals are per-comparison and unadjusted for multiplicity. Twelve comparisons are reported, nine of which exclude zero. Any family-wise adjustment widens all of them. The one direction that follows without further computation is that the two undetermined contrasts can only remain undetermined. We do not assert which of the nine would survive adjustment: the committed artifact stores summary statistics rather than the paired difference vectors, so corrected intervals cannot be derived from it, and we report the unadjusted intervals with this caveat rather than an estimate we cannot compute. Comparisons whose interval lies close to zero relative to its width --- N4-C and N4-E against the LLM proxy --- are the ones a family-wise correction would put at risk.
