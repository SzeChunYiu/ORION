...
Reported intervals are per-comparison. Twelve N4 comparisons are reported without a family-wise correction, and the committed publication analysis stores summary statistics rather than the paired difference vectors, so corrected intervals cannot be recomputed from it. Comparisons whose interval sits close to zero relative to its width --- N4-C, and N4-E against the LLM proxy --- should be read as the ones most exposed to that correction.
