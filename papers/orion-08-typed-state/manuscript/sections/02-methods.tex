...
The synthetic studies use frozen seeds and deterministic decision paths. Canonical receipts encode result and scope. Arms labelled fixed-heuristic proxies are fixed heuristics by construction, not measurements of any deployed model. The result boundary is exact-synthetic mechanism isolation.
...
Synthetic paired intervals are percentile-bootstrap intervals over the registered paired episodes. They are reported per comparison without family-wise correction of the mean; the family-wise question is addressed by exact sign tests over the win/loss counts with Holm--Bonferroni adjustment across the twelve-comparison family, reported with the results. The real-data project and dataset counts are descriptive; rows, tests, bugs, and records are not treated as independent replications of cross-domain transfer.
