...
This V2 supersedes ``verify_five_math_extensions_r4.py``.  The first file had
one transcription error in its expected non-quantum ledger: along
``s+c4=25`` the multiplicity formula gives ``c1=19`` in all three rows.  The
written theorem and manuscript addendum were unaffected.

The script is not a proof assistant and does not replace the written proofs or
external donor theorems. It checks finite algebraic consequences vulnerable to
...
def check_nonquantum() -> dict[str, object]:
    ...
    diagonal = [row for row in rows if row[0] >= 23 and row[0] + row[3] == 25]
    expected = [
        (23, 19, 2, 2),
        (24, 19, 4, 1),
        (25, 19, 6, 0),
