"""Execute the prospectively frozen P12B successor in two fresh processes."""
...
OUT = HERE / "P12B_EQUAL_ACTION_SIGNAL_COMPLEMENTARITY_RESULT_V1.json"
...
def _supervise(path: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="p12b-replay-") as directory:
        ...
        if not all(run.returncode == 0 for run in runs):
            raise RuntimeError("P12B protected worker failed")
...
def main() -> None:
    ...
    print(json.dumps({"terminal": result["terminal"], "summary": result["summary"], "gates": result["gates"]}, indent=2, sort_keys=True))
