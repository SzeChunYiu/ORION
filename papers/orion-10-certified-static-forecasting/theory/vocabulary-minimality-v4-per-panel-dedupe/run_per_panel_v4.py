...
VERBATIM_CAP = 50
# V3 CAP LIFT. V2's caps truncated every one of the ten panels (`cap_hit: true`
# for all), so its 740 rows are an early prefix of the enumeration order rather
# than a census.  These caps are 20x V2's and are the ONLY change to the frozen
# generator: the grammar, routes, skeletons, ordering, regime rule and terminal
# selection are byte-identical to V2.  The multiplier was fixed before the run
# and is not tuned against any outcome.
CAPS = {
    ("H1", 3): 2400, ("H2", 3): 3200, ("H3", 3): 1800, ("H4", 3): 1800,
    ("H5", 3): 2400,
    ("H1", 4): 800, ("H2", 4): 800, ("H3", 4): 480, ("H4", 4): 640, ("H5", 4): 480,
}
PANEL_ORDER = [
...
def main() -> dict[str, Any]:
    ...
    # V4 PER-PANEL DEDUPE. V3 shared ONE dedupe set across all ten panels, so a
    # panel's enumeration depended on the caps of the panels before it in
    # PANEL_ORDER: raising H1_n3 from 120 to 2400 inserted ~2280 extra
    # canonical_key entries before H2_n3 ran, and H2_n3 then skipped instances it
    # would otherwise have evaluated. That broke the prefix property the
    # cap-lift comparison rests on, and V3 terminated
    # CANNOT_CHECK_PREFIX_CONTROL_FAILED on 5 of 10 panels.
    #
    # Giving each panel its own set makes enumeration depend only on that panel's
    # own cap, so V2's rows are a prefix of V4's by construction. Dedupe still
    # removes duplicates WITHIN a panel, which is the only place canonical_key
    # collisions were ever scientifically meaningful.
    dedupe: set = set()
    ...
    for hname, n in PANEL_ORDER:
        dedupe = set()   # V4: per-panel, see note above
        t0 = time.monotonic()
    ...
    result = {
        ...
        "schema_v4": "ORION10.BPRIME_FIBRE_CRITERION_PER_PANEL_DEDUPE.v4",
    ...
    Path(__file__).with_name("PER_PANEL_RESULTS_V4.json").write_text(
